...
export function PublishedQuestionsSection({ delivery, lessonId, userId, onPracticeMore }: {
  ...
  useEffect(() => {
    if (loaded[page] || !delivery.pages[page]) return;
    const controller = new AbortController();
    setError('');
    const descriptor = delivery.pages[page];
    void fetchPublishedJson<Array<{ index: number; question: OfficialQuestionView }>>(
      ...
      if (records.length !== descriptor.count || records.length > 5) throw new Error('Página de questões incompleta.');
      if (!controller.signal.aborted) setLoaded(current => ({ ...current, [page]: records.map(r => r.question) }));
  ...
  return <div>
    ...
    {delivery.availableCount > 0 && <nav aria-label="Páginas de questões" className="mt-4 flex flex-wrap items-center gap-3">
      <span aria-live="polite">Página {page + 1} de {delivery.pages.length} · {delivery.availableCount} questões</span>
      ...
      <button type="button" disabled={page >= delivery.pages.length - 1} onClick={() => setPage(n => n + 1)} className="min-h-11 rounded-lg border px-4 disabled:opacity-50">Próxima página</button>
    </nav>}
