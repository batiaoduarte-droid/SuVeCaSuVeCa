...
const EVIDENCE_LABELS: Record<MacroMasteryEvidenceState, string> = {
  no_evidence: 'Sem evidência',
  acquiring: 'Em aquisição',
  needs_review: 'Precisa de revisão',
  immediate_transfer_confirmed: 'Transferência imediata confirmada',
  retention_confirmed: 'Retenção confirmada',
};
...
export const MacroMasterySummary: React.FC<MacroMasterySummaryProps> = ({
  ...
  return (
    <section className="rounded-xl border border-indigo-200 bg-indigo-50/60 p-4" aria-labelledby="macro-evidence-title">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 id="macro-evidence-title" className="m-0 flex items-center gap-2 text-sm font-black text-indigo-950">
            <Target className="h-4 w-4" /> Evidência de aprendizagem por competência
          </h3>
          <p className="mt-1 text-xs text-indigo-900">
            Sem média geral: uma fragilidade continua visível mesmo quando outras competências estão retidas.
          </p>
        </div>
        {vector.reviewDueCount > 0 && (
          <span className="rounded-full border border-amber-300 bg-amber-100 px-2.5 py-1 text-[11px] font-bold text-amber-950">
            {vector.reviewDueCount} revisão{vector.reviewDueCount === 1 ? '' : 'ões'} vencida{vector.reviewDueCount === 1 ? '' : 's'}
          </span>
        )}
      </div>
      {loading ? (
        <p className="mt-3 text-xs text-indigo-800" role="status">Carregando evidências PBL…</p>
      ) : vector.totalCompetencies === 0 ? (
        <p className="mt-3 text-xs text-indigo-800">Nenhuma competência de prática foi projetada para esta entrada.</p>
      ) : (
        <>
          <dl className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {(Object.keys(EVIDENCE_LABELS) as MacroMasteryEvidenceState[]).map((state) => (
              <div key={state} className="flex items-center justify-between gap-2 rounded-lg border border-indigo-100 bg-white px-3 py-2 text-xs">
                <dt className="text-slate-700">{EVIDENCE_LABELS[state]}</dt>
                <dd className="m-0 font-black text-indigo-950">{vector.counts[state]}</dd>
              </div>
            ))}
          </dl>
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-indigo-200 pt-3">
            <p className="m-0 text-xs font-semibold text-indigo-950">
              {vector.allRetentionConfirmed
                ? 'Retenção confirmada em todas as competências.'
                : recommended
                  ? `Próximo passo recomendado: ${presentCompetencyTitle(recommended.title).title}`
                  : 'Há evidência pendente neste percurso.'}
            </p>
            {recommended && onPracticeCompetency && (
              ...
              >
                <RotateCw className="h-3.5 w-3.5" /> Praticar ponto recomendado
              </button>
            )}
          </div>
        </>
