# Figuras de Linguagem - Aprofundamento
...
`mermaid
graph TD
 FL[Figuras de Linguagem: Recursos Semântico-Estilísticos]

 FL --> EixoComp[Eixo de Comparação e Semelhança]
 FL --> EixoSubst[Eixo de Substituição e Contiguidade]
 FL --> EixoPens[Eixo de Pensamento e Oposição]
 FL --> EixoSint[Eixo de Sintaxe e Construção]
 FL --> EixoSens[Eixo Sensorial e Sonoro]

 EixoComp --> CompEx[Comparação Explícita: 2 elementos + conectivos 'como', 'tipo', verbos 'parecer']
 EixoComp --> Meta[Metáfora: 2 elementos + comparação implícita / direta]
 EixoComp -.-> FGVComp[Abordagem FGV: Metáfora como hiperônimo da base comparativa]

 EixoSubst --> Meton[Metonímia: 1 elemento em tela substituindo outro por contiguidade / opcional]
 Meton --> Sined[Sinédoque: Relação parte pelo todo / singular pelo plural]
 Meton --> RelMet[Relações: Autor/Obra, Continente/Conteúdo, Abstrato/Concreto, Marca/Produto]
 EixoSubst --> Catac[Catacrese: Metáfora cristalizada por falta de termo próprio no léxico]

 EixoPens --> Ant[Antítese: Oposição lexical de termos / Lógica preservada]
 EixoPens --> Parad[Paradoxo / Oxímoro: Contradição de ideias inconciliável / Quebra lógica]
 EixoPens --> Iron[Ironia: Expressar o oposto do que se pensa]
 Iron --> Sarc[Sarcasmo: Viés negativo, deboche, pejorativo]
 Iron --> Hum[Humor: Viés positivo, riso, descontraído]
 EixoPens --> Euf[Eufemismo: Suavização de ideia desagradável / tabu]
 EixoPens --> Proso[Prosopopeia / Personificação: Atos humanos a inanimados ou animais]
 EixoPens --> Hipe[Hipérbole: Exagero expressivo para ênfase]

 EixoSint --> Anac[Anacoluto: Quebra sintática / Termo inicial solto sem função]
 EixoSint --> Pleo[Pleonasmo: Repetição / Redundância]
 Pleo --> PleoEst[Estilístico: Reforço expressivo literário]
 Pleo --> PleoSint[Sintático: Objeto Pleonástico formal anteposto]
 Pleo --> PleoVic[Vicioso: Redundância condenável na norma culta]
 EixoSint --> Elip[Elipse: Omissão de termo NÃO mencionado antes]
 Elip --> Zeug[Zeugma: Omissão de termo JÁ mencionado antes]
 Zeug --> VirgVic[Vírgula Vicária: Pontuação obrigatória da omissão verbal]
 EixoSint --> Silep[Silepse: Concordância ideológica com a ideia subentendida]
 Silep --> SilG[Gênero: Concordância com 'cidade', etc.]
 Silep --> SilN[Número: Sujeito coletivo -> Exige distanciamento verbal]
 Silep --> SilP[Pessoa: Inclusão do emissor com 'nós']
 EixoSint --> Hipa[Hipérbato: Inversão da ordem direta canônica S-V-C]

 EixoSens --> Sines[Sinestesia: Fusão de órgãos dos sentidos distintos]
 EixoSens --> Onom[Onomatopeia: Imitação gráfica e fonética de ruídos]
 EixoSens --> Alit[Aliteração: Repetição rítmica de fonemas consonantais]
 EixoSens --> Asso[Assonância: Repetição harmônica de fonemas vocálicos]

 FL -.-> AlertaRed[ALERTA DE PROVA DISCURSIVA: Proibição absoluta em redações técnicas/oficiais]
```text
...
- **Comportamento em Questões de Metáfora:** Se o enunciado exigir a identificação de uma metáfora e a única opção de teor analógico trouxer a conjunção *como* (ex.: *"A felicidade é como a pluma..."*), o candidato deve assinalá-la, pois as demais alternativas costumam ser relações sintáticas literais (consecutivas, proporcionais, explicativas).
- A distinção estrita e acadêmica entre metáfora e comparação só deve ser aplicada perante a FGV quando o próprio comando for explícito: *"Assinale a alternativa que traz metáfora e NÃO comparação"*.
...
1. **Autor pela Obra:** Emprego do criador no lugar da criação (*"Lia Machado de Assis"* = lia os livros de Machado).
2. **Continente pelo Conteúdo:** O recipiente ou espaço delimitador substitui o conteúdo ou população interna (*"O Brasil chorou"* = os brasileiros choraram; *"Ele bebeu duas garrafas"* = bebeu o líquido).
3. **Abstrato pelo Concreto:** A qualidade ou conceito substitui os indivíduos concretos portadores dessa qualidade (*"A juventude é impetuosa"* = os jovens).
4. **Parte pelo Todo (Sinédoque):** Fração física representando a integralidade (*"O bonde passa cheio de pernas"* = pessoas; *"Quero ter meu próprio teto"* = casa). As bancas modernas tratam a sinédoque sob o hiperônimo metonímia.
5. **Singular pelo Plural (Gênero pela Espécie):** O termo singular representa a coletividade (*"O homem é corruptível"* = os seres humanos; *"O cidadão foi às urnas"* = os cidadãos).
...
- **Exemplos Canônicos de Catacrese:** "Braço da cadeira", "boca do fogão", "dente de alho", "pé da mesa", "asa da xícara", "cabeça de prego", "embarcar no trem/avião" (extensão do radical *barco* por ausência de verbo específico).
...
- **Distinção frente à Concessão Sintática:** Orações subordinadas concessivas introduzidas por *embora* ou *apesar de* (ex.: *"Ninguém ouviu os gritos, embora a noite estivesse calma"*) representam oposição contextual de fatos, mas **não configuram paradoxo**, pois não há anulação lógica intrínseca.
...
- **Modalidades de Pleonasmo:**
 ...
 2. *Pleonasmo Sintático (Legítimo):* Anteposição de termo preposicionado que exerce função de objeto (direto ou indireto) e é formalmente reiterado por pronome oblíquo correspondente (*"A ela não lhe dei meu amor"* -> *A ela* = OI, *lhe* = OI pleonástico).
 3. *Pleonasmo Vicioso (-):* Redundância tautológica inútil que viola a norma culta (*"subir para cima"*, *"prever com antecedência"*, *"protagonista principal"*, *"avançar para a frente"*).
- **Anacoluto (Quebra de Sintaxe):** O elemento inicial não possui vínculo gramatical com o verbo da oração principal (fica "solto" / "sobra" na análise sintática). Ex.: *"Joana, eu não sei o que dizer a ela."* / *"A velha hipocrisia, recordo-me dela com vergonha."* / *"Portugal, quanta saudade sinto."*
...
- **Regra da Vírgula Vicária:** Quando a omissão for verbal (zeugma do verbo), a inserção da vírgula vicária no ponto da elipse é **estritamente obrigatória pela norma culta** (*"Deste lado da estrada, montanhas; e daquele, [há] rios"*).
...
- **Onomatopeia:** Reprodução gráfica e fonética de ruídos da realidade física (*"Passa, tempo, tic-tac"*, *"zum-zum"*).
- **Aliteração:** Repetição rítmica e sistemática de fonemas consonantais idênticos (*"Vozes veladas / veludosas vozes"* — /v/, /l/, /s/; *"Chove chuva, chove sem parar"* — /ch/).
- **Assonância:** Repetição harmônica de fonemas vocálicos ou ditongos (*"Essa desmesura de paixão / É loucura do coração"* — /ão/; *"Foz do Iguaçu / Polo sul, meu azul"* — /u/).
...
| Conceito / Regra | Categoria | Formulação Canônica Compacta |
| :----------------------------- | :---------- | :------------------------------------------------------------------------------------------- |
| **Metáfora** | Palavra | Comparação implícita direta entre 2 elementos sem conectivo formal. |
| **Comparação** | Palavra | Analogia explícita entre 2 elementos com conectivo (*como, tipo*) ou verbo (*parecer*). |
| **Metáfora (FGV)** | Semântica | Hiperônimo de linguagem figurada com base comparativa. |
| **Metonímia** | Palavra | Substituição opcional por contiguidade objetiva (1 termo expresso). |
| **Sinédoque** | Palavra | Subespécie de metonímia pela relação parte/todo ou singular/plural. |
| **Catacrese** | Palavra | Empréstimo semântico forçado por ausência de termo próprio no léxico. |
| **Antítese** | Pensamento | Oposição lexical de palavras antônimas sem quebra da lógica factual. |
| **Paradoxo** | Pensamento | Fusão de conceitos inconciliáveis que gera contradição lógica insuperável. |
| **Ironia** | Pensamento | Expressão formal do inverso semântico do que se pensa. |
| **Eufemismo** | Pensamento | Suavização intencional de ideia rude, ofensiva, dolorosa ou tabu. |
| **Prosopopeia** | Pensamento | Atribuição de atos ou sentimentos humanos a seres inanimados ou animais. |
| **Hipérbole** | Pensamento | Exagero expressivo desmedido para conferir ênfase. |
| **Sinestesia** | Sensorial | Mescla de diferentes órgãos dos sentidos humanos em uma mesma expressão. |
| **Anacoluto** | Sintaxe | Ruptura da frase na qual o termo inicial fica isolado sem função sintática. |
| **Pleonasmo Sintático** | Sintaxe | Reiteração formal de objeto anteposto por pronome oblíquo. |
| **Pleonasmo Vicioso** | Sintaxe | Redundância desnecessária e condenável na norma culta padrão. |
| **Elipse** | Sintaxe | Omissão de termo que não figurou anteriormente no trecho. |
| **Zeugma** | Sintaxe | Omissão contextual de termo que já foi mencionado anteriormente. |
| **Vírgula Vicária** | Pontuação | Sinal gráfico obrigatório que marca a supressão do verbo em oração coordenada. |
| **Silepse de Número** | Sintaxe | Concordância ideológica no plural que exige distanciamento do sujeito coletivo. |
| **Hipérbato** | Sintaxe | Inversão da ordem direta canônica dos termos da oração (S-V-C). |
...
- **Elemento X (Primeiro Termo):** É o vocábulo que está **materialmente escrito na frase da prova**.
- **Elemento Y (Segundo Termo):** É o referente real que foi **omitido / substituído**.
- *Aplicação:* "Abstrato pelo concreto" -> A prova escreveu "juventude" (abstrato) para representar "jovens" (concreto).
...
| Par Conceitual | Critério Decisivo | Limite / Condição de Validade | Exemplo Canônico |
|:------------------------------------------- |:---------------------------------------------------------------------------------------------------------- |:-------------------------------------------------------------------------------- |:--------------------------------------------------------------------------------------------------------- |
| **Metáfora vs. Comparação** | Presença de conectivo comparativo formal (*como, tipo*) ou verbo (*parecer*). | Ambos exigem 2 termos em relação de semelhança. | *"Catarina é uma flor"* (Metáfora) vs. *"Catarina é como uma flor"* (Comparação). |
| **Metáfora vs. Metonímia** | Relação de similaridade subjetiva (2 termos) vs. substituição por contiguidade real (1 termo expresso). | Metonímia exige termo ausente associado (*autor/obra*, *marca/produto*). | *"Ela é um doce"* (Metáfora) vs. *"Comprei Bombril"* (Metonímia). |
| **Metonímia vs. Catacrese** | Critério da opcionalidade vocabular. | Catacrese não possui termo próprio no léxico; metonímia é escolha de estilo. | *"Li Machado"* (Metonímia - opcional) vs. *"Braço da cadeira"* (Catacrese - compulsória). |
| **Antítese vs. Paradoxo** | Coerência lógica veritativa vs. contradição conceitual insuperável. | Antítese aproxima palavras antônimas; paradoxo choca ideias excludentes. | *"Sua alma subiu, seu corpo desceu"* (Antítese) vs. *"Uma inocência maliciosa"* (Paradoxo). |
| **Paradoxo vs. Concessão** | Quebra lógica conceitual vs. oposição sintática de expectativas reais. | Oração com*embora* relata fatos reais empíricos, sem paradoxo. | *"Amor é fogo que arde sem se ver"* (Paradoxo) vs. *"Não ouviu, embora fosse calma"* (Concessão). |
| **Ironia vs. Crítica Direta** | Emissão do oposto do pensamento vs. emissão literal do pensamento. | Se o emissor pensa o que declarou, é afirmação sincera, não ironia. | *"Que gênio!" [para um tolo]* (Ironia) vs. *"Políticos deveriam trabalhar mais"* (Opinião sincera). |
| **Anacoluto vs. Pleonasmo Sintático** | Ausência total de função sintática vs. reiteração formal de objeto. | No pleonasmo, o termo anteposto é preposicionado e ligado ao verbo. | *"Joana, não sei o que dizer"* (Anacoluto) vs. *"A Joana, não lhe disse"* (Pleonasmo Sintático). |
| **Elipse vs. Zeugma** | Termo omitido NÃO apareceu antes vs. termo omitido JÁ apareceu antes. | Todo zeugma é uma elipse; a elipse verbal exige vírgula vicária. | *"[Nós] Chegamos"* (Elipse) vs. *"Ele estuda; eu, [estudo] trabalho"* (Zeugma + Vírgula Vicária). |
...
3. *"Os olhos dela eram como duas jabuticabas."* -> **Comparação Explícita** (presença da conjunção *como*).
4. *"Estou morrendo de fome."* -> **Hipérbole** (exagero enfático).
5. *"Ela não sabe escrever mesmo... Tirou 10 na redação!"* -> **Ironia** (o marcador contextual desmente a declaração inicial).
6. *"Dirigiu-me um comentário duro e amargo."* -> **Sinestesia** (cruzamento de tato *duro* e paladar *amargo*).
7. *"Meu pensamento é um rio subterrâneo."* -> **Metáfora** (relação direta entre pensamento e rio sem conectivo).
8. *"Se quiser me prender, vai ter que saber me soltar."* -> **Paradoxo (Oxímoro)** (contradição conceitual de ações).
9. *"A fome chegou ao grupo e atacaram logo o buffet."* -> **Silepse de Número** (verbo no plural concordando com os indivíduos de *grupo* distanciado).
10. *"Ele já não está mais entre nós."* -> **Eufemismo** (suavização da morte).
11. *"Ele contava os dias para chegar à sua amada Belo Horizonte."* -> **Silepse de Gênero** (adjetivo feminino concordando com a ideia implícita de *cidade*).
12. *"A Casa Branca influencia outros países."* -> **Metonímia** (lugar pelo governo norte-americano).
13. *"A protagonista principal do filme era Julia Roberts."* -> **Pleonasmo Vicioso** (redundância semântica).
14. *"Cuidado com a boca do fogão!"* -> **Catacrese** (empréstimo vocabular por falta de termo próprio).
15. *"Chove chuva, chove sem parar"* -> **Aliteração** (repetição do fonema consonantal /ch/).
16. *"Deste lado da estrada, montanhas e daquele, rios."* -> **Elipse / Zeugma** (omissão do verbo *há*, marcada por vírgula vicária).
17. *"Leis são como salsichas."* -> **Comparação Explícita** (dois elementos + conectivo *como*).
18. *"Teus olhos são meus livros."* -> **Metáfora** (dois elementos + relação direta implícita).
...
- **B) CORRETA:** *"O dono do mercado disse que sua sogra é uma cobra e, por isso, não se pode confiar nela."* (Metáfora pura entre *sogra* e *cobra* sem conectivo comparativo).
- **C)** *"Para não repetir os erros, é preciso avançar para a frente e esquecer o passado."* (Incorreta: pleonasmo vicioso em *avançar para a frente*).
- **D)** *"Quando olhamos para o Universo, vemos que o homem é nada; mas, apesar disso, o homem é tudo."* (Incorreta: antítese entre *nada* e *tudo*).
- **E)** *"Goste ou não, esse seu estilo de roupas não diz muita coisa, né!"* (Incorreta: personificação ao atribuir o ato de dizer a *roupas*).
- **Gabarito:** **B**
...
- **B)** *"O amor é um grande laço/ Um passo pr'uma armadilha/ Um lobo..."* (Metáfora: identidade analógica direta).
- **C)** *"Ela é um filme de ação com vários finais..."* (Metáfora: equivalência direta).
- **D)** *"Há escolas que são gaiolas e há escolas que são asas."* (Metáfora: analogia conceitual).
- **E) CORRETA:** *"Vendo as pedras que choram sozinhas/ no mesmo lugar."* (Contém **prosopopeia / personificação**, pois pedras não realizam o ato biológico de chorar).
- **Gabarito:** **E**
...
- **Associação de Colunas:**
 ...
 2. *"Mas quantas vezes a insônia é um dom."* -> **( 2 ) Metáfora** (*insônia = dom* sem conector).
 3. *"E tomo café com gosto, toda sozinha no mundo."* -> **( 3 ) Eufemismo** (suavização histórica de abandono social).
 4. *"E sinto-me feliz por nada, por tudo."* -> **( 4 ) Paradoxo** (contradição lógica insuperável simultânea).
 5. *"[...] a casa vai acordando..."* -> **( 5 ) Prosopopeia** (atribuição de ação humana a uma casa).
- **Sequência e Gabarito:** `1, 2, 3, 4, 5` (Alternativa A).
...
- **A)** *"Ninguém é tão velho que não creia poder viver mais um ano."* (Incorreta: oração adverbial consecutiva *tão... que*).
- **B)** *"Está morto: podemos elogiá-lo à vontade."* (Incorreta: oração explicativa causal).
- **C) CORRETA:** *"A felicidade é como a pluma que o vento vai levando pelo ar..."* (Única opção com relação analógica entre dois termos; a FGV unifica a base comparativa sob o hiperônimo metáfora).
- **D)** *"O cisne, quanto mais envelhece, mais belo fica."* (Incorreta: correlação proporcional *quanto mais... mais*).
- **E)** *"A gente não deveria crescer nunca."* (Incorreta: pleonasmo estilístico de negação *não... nunca*).
- **Gabarito:** **C**
...
- **B) Incorreta: Paradoxo —** *"Ninguém ouviu os gritos da vítima, embora a noite estivesse silenciosa."* (A oração concessiva *embora* relata fatos reais, sem contradição conceitual intrínseca).
- **C) Incorreta: Ironia —** *"Quem dera os políticos dessem mais atenção às reclamações dos eleitores."* (Expressa um desejo literal e sincero, não ironia).
- **D) Incorreta: Hipérbole —** *"A lua abriu, sobre as ondas, a sua cortina de prata."* (A expressão *cortina de prata* é metáfora, não exagero quantitativo).
- **E) Incorreta: Eufemismo —** *"Isso foi repetido mil vezes nas últimas eleições, mas nada foi feito."* (*Mil vezes* é hipérbole, não eufemismo).
- **Gabarito:** **A**
...
Ao concluir o estudo do grupo `esta unidade`, o estudante deve ser capaz de:

1. Identificar, classificar e distinguir qualquer figura de linguagem em provas de concurso público, dominando com precisão os eixos de palavras, pensamento, sintaxe e som.
2. Aplicar a regra dos dois elementos para discernir de imediato entre metáfora, comparação explícita e metonímia.
3. Compreender e antecipar o padrão epistemológico da banca FGV, reconhecendo quando a metáfora é tratada como base comparativa genérica.
4. Diferenciar antíteses lexicais factíveis de paradoxos lógicos insuperáveis, sem confundir orações subordinadas concessivas (*embora*) com paradoxos.
5. Isolar o termo inicial em análises sintáticas para distinguir pleonasmos sintáticos legítimos de anacolutos.
6. Aplicar obrigatoriamente a vírgula vicária diante da elipse verbal em construções coordenadas paralelas.
