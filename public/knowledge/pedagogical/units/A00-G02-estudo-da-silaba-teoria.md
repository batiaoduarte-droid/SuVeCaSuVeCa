...
```text
 ...
 ┌──────────────────────────────────────┼──────────────────────────────────────┐
 ▼ ▼ ▼
ESTRUTURA DA SÍLABA ENCONTROS FONÉTICOS REPRESENTAÇÃO GRÁFICA
 (Vogal como Núcleo) │ vs. FONÊMICA
 │ │ │
 ...
 │ (Zero consoantes soltas) │ (Ambas Consoantes │ ├─ Consonantal (CH, RR, QU...)
 │ ├─ Ditongo soam na palavra) │ └─ Vocálico (AM, EN, IN...)
 ├─ Vogal (V): Pico de força │ ├─ DC (SV + V) │ └─ Dífono (1L = 2F, soma 1)
 │ (Letra "A" é sempre V) │ │ └─ DC = H ├─ Perfeito (Grafema X = /ks/)
 │ │ └─ DD (V + SV) │ (blu-sa)
 └─ Semivogal (SV): Som brando │ └─ DD ≠ H └─ Imperfeito
 de /i/ ou /u/ (E, I, O, U) ├─ Tritongo (SV+V+SV) (af-ta)
 └─ Hiato (V + V)
```
...
- A letra *E* vira semivogal quando assume som brando de `/i/` (ex.: *mãe* $\rightarrow$ som de "mãi"; *área* $\rightarrow$ som de "ária").
...
3. Se houver encontro de *I* e *U* na mesma sílaba $\rightarrow$ a letra articulada com maior sustentação será a vogal e a outra será a semivogal (ex.: em *fui* $\rightarrow U = V, I = SV$; em *tranquilo* $\rightarrow U = SV, I = V$).
...
- O tritongo pode ser oral (ex.: *U-ru-guai*, *quais*) ou nasal (ex.: *quão*, *en-xá-guem*).
...
- **Encontro Consonantal Perfeito:** Consoantes ouvidas na mesma sílaba (ex.: *BL* em `blu-sa`, *BR* em `que-bran-to`).
- **Encontro Consonantal Imperfeito:** Consoantes ouvidas em sílabas separadas (ex.: *FT* em `af-ta`, *CT* em `pac-to`).
- **Dígrafo Consonantal:** Duas letras emitindo 1 só som (ex.: *CH* em `cha-ve`, *NH* em `se-nhor`, *RR* em `car-ro`, *SC* em `des-cer`). Como uma das letras não soa individualmente, inexiste encontro consonantal no par.
...
3. Som de $/ss/$: *auxílio*, *máximo* ($1L = 1F$).
...
$$
\text{Fonemas } (F) = \text{Letras } (L) - (\text{Dígrafos Consonantais} + \text{Dígrafos Vocálicos} + \text{H inicial}) + (\text{Dífonos } X_{/ks/})
$$
...
- **Vogal Canônica:** Centro obrigatório da sílaba; 1 por sílaba .
- **Semivogal Canônica:** Som brando de `/i/` ou `/u/` em apoio a uma vogal .
- **Ditongo Decrescente ($DD$):** $V + SV$, inseparável ($DD \neq H$) .
- **Ditongo Crescente ($DC$):** $SV + V$, passível de virar hiato / proparoxítona eventual ($DC = H$) .
- **Tritongo Canônico:** Estrutura fixa $SV + V + SV$ na mesma sílaba .
- **Hiato Canônico:** $V + V$ em sílabas separadas; não possui semivogal .
- **Encontro Consonantal:** Consoantes simultaneamente pronunciadas; divide-se em perfeito e imperfeito .
- **Dígrafo Consonantal:** Duas letras gerando 1 fonema consonantal .
- **Dígrafo Vocálico:** Vogal nasalizada por $M/N$ na mesma sílaba .
- **Dífono:** Uma letra ($X$) gerando 2 fonemas ($/ks/$) .
...
```text
 ...
 ┌────────────────────────────┴────────────────────────────┐
 ▼ ▼
 [1. Divisão Silábica Fonética] [2. Varredura de Dígrafos]
 • Falar pausadamente (Abertura de boca) • Dígrafos consonantais (CH, RR, SS, SC...)
 • Consoante sem vogal recua (af-ta) • Grupos QU/GU (U é mudo?)
 • Dígrafos separáveis vs. inseparáveis • Dígrafos vocálicos (Vogal + M/N na sílaba)
 │ │
 └────────────────────────────┬────────────────────────────┘
 ▼
 [3. Encontros Vocálicos e Consonantais]
 • Letras mudas consom-se no dígrafo
 • V + SV = Ditongo Decrescente (DD)
 • SV + V = Ditongo Crescente (DC)
 • V | V = Hiato (H)
 • Consoantes audíveis = Encontro Consonantal (EC)
 │
 ▼
 [4. Cálculo de Grafemas vs. Fonemas]
 F = Letras - (Dígrafos + H inicial) + (X dífono)
```
...
| Critério | Encontro Consonantal | Dígrafo Consonantal | Ditongo Crescente | Ditongo Decrescente | Hiato |
|:--------------------------------------------------------------------------------------------------------------------------------------------------- |:-------------------- |:------------------- |:---------------------------------- |:---------------------------------- |:------------------------------ |
| **Composição Fonética** | Consoante + Consoante | 2 letras = 1 fonema | $\text{Semivogal} + \text{Vogal}$ | $\text{Vogal} + \text{Semivogal}$ | $\text{Vogal} + \text{Vogal}$ |
| **Alocação Silábica** | Mesma ou separada | Mesma ou separada | Mesma sílaba | Mesma sílaba | Sílabas distintas |
| **Comportamento ($= H$)** | Inaplicável | Inaplicável | **Pode virar Hiato ($DC=H$)** | **NUNCA vira Hiato ($DD \neq H$)** | É o próprio Hiato | | | | |
| **Presença de Semivogal** | Não | Não | Sim (1 semivogal) | Sim (1 semivogal) | **NÃO possui semivogal** |
| **Exemplo Típico** | `BLUSA`, `AFTA` | `CHAVE`, `CARRO` | `SÉRIE`, `LÍRIO` | `PAI`, `ÓRFÃO` | `JUIZ`, `SAÚDE` |
...
|:-: |:------------ |:----------------- |:------------------------------------------------------------------- |:------------------------------: |:-: |
| 1 | `Navio` | `na-vi-o` | Hiato ($V+V$) | 5 | 5 |
| 2 | `Heroico` | `he-roi-co` | Ditongo Decrescente (`ROI` = $V+SV$) | 7 | 7 |
| 3 | `Lua` | `lu-a` | Hiato ($V+V$) | 3 | 3 |
| 4 | `Tranquilo` | `tran-qui-lo` | Dígrafo Vocálico (`AN`) + Ditongo Crescente (`QUI` = $SV+V$) | 9 | 8 |
| 5 | `Álcool` | `ál-co-ol` | Hiato ($O+O$) | 6 | 6 |
| 6 | `Gênio` | `gê-nio` | Ditongo Crescente (`NIO` = $SV+V$) | 5 | 5 |
| 7 | `Noite` | `noi-te` | Ditongo Decrescente (`NOI` = $V+SV$) | 5 | 5 |
| 8 | `Raiz` | `ra-iz` | Hiato ($V+V$) | 4 | 4 |
| 9 | `Irmão` | `ir-mão` | Ditongo Decrescente Nasal (`ÃO` = $V+SV$) | 5 | 5 |
| 10 | `Senhor` | `se-nhor` | Dígrafo Consonantal (`NH`) | 6 | 5 |
| 11 | `Peixe` | `pei-xe` | Ditongo Decrescente (`EI`) + $X=/x/$ | 5 | 5 |
| 12 | `Cadeira` | `ca-dei-ra` | Ditongo Decrescente (`EI`) | 7 | 7 |
| 13 | `Juiz` | `ju-iz` | Hiato ($V+V$) | 4 | 4 |
| 14 | `Hora` | `ho-ra` | $H$ inicial mudo (sem fonema) | 4 | 3 |
| 15 | `Chave` | `cha-ve` | Dígrafo Consonantal Inseparável (`CH`) | 5 | 4 |
| 16 | `Tampa` | `tam-pa` | Dígrafo Vocálico (`AM`) | 5 | 4 |
| 17 | `Barril` | `bar-ril` | Dígrafo Consonantal Separável (`RR`) + $L$ consonantal | 6 | 5 |
| 18 | `Descer` | `des-cer` | Dígrafo Consonantal Separável (`SC`) | 6 | 5 |
...
- **Pegadinha da Dupla Separação de Paroxítonas em Ditongo Crescente:** Em bancas de alto nível (como Cebraspe), vocábulos como *série, história, memória, área* podem ser legitimamente classificados como proparoxítonas eventuais/relativas por abertura do ditongo em hiato (`sé-ri-e`). Ditongos decrescentes (*órfão*), contudo, **nunca** admitem divisão.
- **Erro Conceitual no Tritongo:** Dizer que tritongo são "três vogais" anula a questão. É rigorosamente $SV + V + SV$ (apenas 1 vogal).
...
- **Vogal:** Fonema de máxima intensidade que constitui o núcleo obrigatório da sílaba .
- **Semivogal:** Fonema brando com timbre de `/i/` ou `/u/` que apoia a vogal na mesma sílaba .
- **Ditongo:** Encontro de Vogal e Semivogal na mesma sílaba; pode ser Crescente ($SV+V$) ou Decrescente ($V+SV$) .
- **Tritongo:** Encontro indivisível de $SV + V + SV$ na mesma sílaba .
- **Hiato:** Encontro de duas vogais plenas em sílabas separadas ($V+V$) .
- **Proparoxítona Eventual:** Paroxítona terminada em ditongo crescente reclassificada como proparoxítona por abertura do ditongo em hiato .
- **Encontro Consonantal:** Sequência de consoantes em que todas são pronunciadas .
- **Dígrafo Consonantal:** Duas letras representando um único fonema consonantal .
- **Dígrafo Vocálico:** Vogal nasalizada por $M/N$ em fechamento de sílaba .
- **Dífono:** Grafema único ($X$) que emite dois fonemas consonantais ($/ks/$) .
- **Grafema:** A unidade física da escrita (letra) .
- **Fonema:** A unidade sonora distintiva mínima da fala .
...
1. A percepção da **Vogal** como pilar estruturante de qualquer sílaba em português.
2. A distinção precisa entre **Vogais e Semivogais** por meio do teste da tonicidade sonora e da soberania da letra *A*.
3. A identificação imediata e classificação de **Ditongos** (Crescentes e Decrescentes), **Tritongos** e **Hiatos**.
4. O domínio da regra do divórcio ($DC = H$), compreendendo por que palavras como *série* admitem dupla divisão silábica em bancas de concursos.
5. A separação categórica entre **Encontros Consonantais** (sons independentes) e **Dígrafos** (som único).
6. O teste prático de audibilidade do *U* nos grupos *QU/GU* e a identificação de dígrafos vocálicos com *M/N*.
7. As regras formais de partição silábica de dígrafos separáveis/inseparáveis e consoantes desacompanhadas.
