...
**Método central:** A escolha entre pronomes pessoais decorre da função de complemento e da regência do verbo, não da posição ou da referência a pessoa.
...
- Distinção entre classes morfológicas (substantivo, verbo, pronome, preposição) e funções sintáticas (sujeito, predicado, complementos verbais, complementos nominais, adjuntos).
...
1. Série O, A, OS, AS (e alomorfes -lo, -la, -los, -las, -no, -na, -nos, -nas): Atuam exclusivamente como Objeto Direto (O.D.), completando verbos transitivos diretos (VTD) ou a parcela direta de verbos bitransitivos (VTDI). Rejeitam qualquer preposição regente.
...
- \*Obedeci-o prontamente (Incorreto — emprego de pronome direto em VTI).
...
1. Como Objeto Indireto (O.I.): Subordina-se a um verbo transitivo indireto (VTI) ou bitransitivo (VTDI), preenchendo a vaga de complemento verbal regido por preposição (obedeci-lhe = obedeci ao menino).
...
- Na oração "Beijou-lhe a mão", o verbo beijar é transitivo direto e tem como objeto direto a expressão "a mão". O pronome lhe indica a quem pertence a mão (a mão dele), vinculando-se sintaticamente ao substantivo mão.
- A regra aplica-se de forma geral a todos os pronomes oblíquos átonos: "Puxou-me os cabelos" (os meus cabelos $\rightarrow$ me = Adjunto Adnominal).
...
$$
\text{VTDI} \iff \text{1 Objeto Direto (O.D.)} + \text{1 Objeto Indireto (O.I.)}
$$
...
- \*Dei-o [O.D.] um romance [O.D.]. (Incorreto — duplicidade de O.D.; forma correta: Dei-lhe um romance).
- \*Dá-lo [O.D.] ordens [O.D.]. (Incorreto — forma correta: Dar-lhe ordens).
...
1. Caso Inválido (Regências Distintas):
...
1. Procedimento de Correção ("Cada Um na Sua"):
...
| --- | --- | --- | --- | --- |
| Acarretar | VTDI | Rege O.D. de coisa + O.I. de pessoa | O.D. =o/a \ | O.I. = lhe |
| Desobedecer | VTI | Rege preposiçãoa | Exigelhe / lhes | Não lhe desobedecerei jamais. |
| Ajudar | VTD | Sem preposição (norma tradicional) | Exigeo / a / os / as | Eu o ajudei na tarefa. |
| Incomodar | VTD | Sem preposição | Exigeo / a / os / as | O barulho incomodou-os. |
| Satisfazer | VTD | Sem preposição (com pessoa) | Exigeo / a / os / as | A resposta não o satisfez. |
| Presentear | VTD / VTDI | Presentear alguém (O.D.) com algo (Adj.) | Exigeo / a (a pessoa) | Presenteou-o com o carro. |
| Namorar | VTD | Sem preposição (rejeitacom) | Exigeo / a | Eu a namorei. (não: namorei com ela) |
...
1. Construção A (Pessoa como O.D. + Coisa como O.I.):

$$
\text{\textit{Informar alguém [O.D.] de / sobre alguma coisa [O.I.]}}
$$

Exemplo: "Informou Rosinha [O.D.] de sua partida [O.I.]."\

1. Construção B (Coisa como O.D. + Pessoa como O.I.):

$$
\text{\textit{Informar alguma coisa [O.D.] a alguém [O.I.]}}
$$

Exemplo: "Informou sua partida [O.D.] a Rosinha / informou-lhe [O.I.]."\
...
- Como lhes não possui desinência feminina, ele assume a função de Objeto Indireto sem distinção de gênero:

$$
\text{\textit{Os chefes deram as ordens, por isso }} \mathbf{lhes} \text{\textit{ obedeci / obedeci }} \mathbf{a\ elas}\text{\textit{.}}
$$
...
$$
\text{em} + \text{o qual} = \mathbf{no\ qual} \iff \text{em} + \text{que} = \mathbf{em\ que}
$$
...
$$
\text{\textit{Seu pai [sujeito/início], que é libanês... [intercalada], }} \mathbf{o}\text{\textit{ presenteou com o Maverick.}}
$$
...
| Conceito / Regra Canônica | Formulação Sintética |
| --- | --- |
| Seleção de 3ª Pessoa | Complemento sem preposição (O.D.)$\implies$ o/a/os/as; com preposição $\implies$ lhe/lhes. |
| Tríplice Função do LHE | lhe pode ser Objeto Indireto (verbo), Complemento Nominal (nome) ou Adjunto Adnominal (posse). |
| Pronomes Coringas | me, te, se, nos, vos não têm função fixa; espelham a transitividade do regente (O.D., O.I. ou C.N.). |
| Bitransitividade (VTDI) | VTDI exige obrigatoriamente 1 O.D. + 1 O.I. É vedado duplo O.D. ou duplo O.I. na norma padrão. |
| Coordenação Verbal | Verbos de mesma regência compartilham objeto; verbos de regências distintas exigem "cada um na sua". |
| Invariância de LHE | lhe / lhes não varia em gênero; lhes substitui complementos masculinos ou femininos (às ordens). |
| Preposição com Relativo | A preposição antes do pronome relativo é exigida pelo termo regente posposto na oração adjetiva. |
| Dupla Regência vs. VTDI | Dupla regência = opções alternativas; VTDI = complementos simultâneos (rejeita troca de OI por OD). |
| Alomorfia e Próclise | Alomorfia (-lo/-no) ocorre apenas em ênclise; palavra atrativa impõe próclise e restaura a forma base. |
...
1. Em verbos bitransitivos (VTDI), verifique o outro complemento expresso na frase para garantir a alternância obrigatória entre O.D. e O.I.
...
3. Se a frase mantiver sentido idêntico ("Beijou-lhe a mão" $\rightarrow$ "Beijou a mão dele / a sua mão"), o pronome exerce valor possessivo.
4. Conclusão: Todo pronome oblíquo com valor possessivo é sintaticamente um Adjunto Adnominal.
...
2. Analise a transitividade individual de cada verbo ($V1$ e $V2$).
3. Compare as regências:

- Se $\text{Regência}(V1) = \text{Regência}(V2)$, o compartilhamento é válido ("Conheço e respeito aquele professor").
- Se $\text{Regência}(V1) \neq \text{Regência}(V2)$, condene a frase como vício sintático ("Vi e gostei do filme").

1. Aplique a técnica "cada um na sua": mantenha o substantivo pleno no primeiro verbo e insira no segundo verbo o pronome ou forma preposicionada correspondente ("Vi o filme e gostei dele").
...
3. Coluna 3 (Alomorfia e Concordância): Se a ênclise for válida, verifique a terminação (-R/-S/-Z $\rightarrow$ -lo; nasal $\rightarrow$ -no). Em próclise, confirme se o pronome retornou à sua forma padrão.
...
| Critério | O / A / OS / AS | LHE / LHES | Pronomes Coringas (ME, TE, SE, NOS, VOS) |
| --- | --- | --- | --- |
| Exigência Preposicional | 100% Sem Preposição | 100% Com Preposição | Neutro (com ou sem preposição) |
| Funções Sintáticas | Exclusivamente Objeto Direto | Objeto Indireto, C.N. ou Adj. Adnominal | Objeto Direto, Objeto Indireto ou C.N. |
...
| --- | --- | --- | --- | --- |
| Objeto Indireto (O.I.) | Verbo (VTI ou VTDI) | Completa a regência verbal com preposiçãoa | Obedeci ao menino | Obedeci-lhe prontamente. |
...
| --- | --- | --- |
| Estrutura Oracional | Aceita duas opções de transitividade independentes e alternativas. | Exige simultaneamente dois complementos (1 O.D. + 1 O.I.) na mesma oração. |
| Variação de Sentido | Frequente (ex.:entender algo vs. entender de algo). | Não há alteração semântica; a moldura sintática é fixa. |
...
| --- | --- | --- | --- | --- |
| Implicar | VTD (sem preposição) | Acarretar, ter como consequência (implicou a demissão). | VTI pronominal (em) | Envolver-se (implicou-se em confusão). |
| Entender | VTD (sem preposição) | Compreender, assimilar intelectualmente (entendi o texto). | VTI (com preposição de) | Ter conhecimento, competência (entende de leis). |
| Aspirar | VTI (com preposição a) | Almejar, pretender (aspira ao cargo). | VTD (sem preposição) | Inalar, sorver (aspirou o perfume). |
| Assistir | VTI (com preposição a) | Ver, presenciar (assistiu ao jogo). | VTD / VTI | Socorrer, amparar (assistiu o paciente). |
...
- Resultado: O pronome lhe liga-se ao substantivo respeito, exercendo a função de Complemento Nominal (C.N.).
...
- Resultado: lhe exerce a função de Adjunto Adnominal (Adj.Adn.).

- Estrutura: "Obedeceu-me."
- Análise Sintática: Obedecer é VTI (obedeceu ao menino).
- Resultado: O pronome coringa me assume a função de Objeto Indireto (O.I.).

- Estrutura: "Viu-me."
- Análise Sintática: Ver é VTD (viu o menino).
- Resultado: O pronome coringa me assume a função de Objeto Direto (O.D.).
...
3. Eu o ajudei naquela árdua tarefa. (Ajudar é VTD na norma padrão $\rightarrow$ o; rejeita o eufônico lhe).
4. Por que um mendigo dormindo incomodou-os tanto? (Incomodar é VTD $\rightarrow$ os; rejeita o eufônico lhes).
5. A minha resposta não o satisfez. (Satisfazer é VTD $\rightarrow$ o).
6. Seu pai, que é libanês..., o presenteou com o Maverick 74 da família. (Presentear é VTD com adjunto com com $\rightarrow$ o).
...
- Sentença Reestruturada ("Cada Um na Sua"): Eu vi o filme [O.D.] e gostei dele [O.I.]. (Correta).
...
- Gabarito C: "mesmo quando lhe causa incômodo".
...
- Gabarito: CERTO. (Quem precisa, precisa de algo $\rightarrow$ regência posposta).
...
- Gabarito: ERRADO. (A preposição de é regida pelo verbo posposto surge).
...
- Gabarito: ERRADO. (Altera o sentido de "não ter conhecimento/domínio" para "não compreender").
...
- Gabarito: CERTO. (Investir rege em; aonde exige preposição a e movimento).
...
Ao concluir o estudo desta unidade, o estudante deve dominar as seguintes competências práticas e teóricas:

1. Diferenciação Mecânica e Precisa de 3ª Pessoa: Identificar que o, a, os, as são complementos diretos (sem preposição) e que lhe, lhes são complementos indiretos/preposicionados, sabendo que lhe pode ser Objeto Indireto, Complemento Nominal ou Adjunto Adnominal de posse.
2. Operação dos Pronomes Coringas: Analisar que me, te, se, nos, vos não possuem transitividade própria, espelhando fielmente a regência do verbo ou nome com o qual se combinam.
3. Manejo Seguro da Bitransitividade: Aplicar a regra da não duplicidade em verbos VTDI (dar, oferecer, informar, presentear, reduzir, acarretar), garantindo que a presença de um O.D. imponha pronome indireto (lhe) e vice-versa.
4. Aplicação do Paralelismo Sintático em Coordenação: Reconhecer e reescrever frases com verbos coordenados de regências distintas pela regra "cada um na sua" (Vi o filme e gostei dele).
5. Domínio de Verbos Notáveis e Peculiaridades Semânticas: Dominar as regências de implicar (VTD no sentido de acarretar), aspirar (VTI com a no sentido de almejar), ansiar (VTI com por ou de), assistir (VTI com a no sentido de presenciar), namorar (VTD sem com) e entender (VTD = compreender vs. VTI com de = conhecer).
6. Morfossintaxe e Colocação Pronominal: Aplicar alomorfias (-lo, -no) somente em ênclise, reconhecer a força atrativa da próclise obrigatória (palavras negativas e conjunções subordinativas) e validar a próclise após orações intercaladas.
7. Regência em Orações Adjetivas com Relativos: Identificar que a preposição que precede o pronome relativo é regida exclusivamente pelo verbo ou nome posposto na oração adjetiva, dominando a comutabilidade entre no qual/em que e as restrições do pronome aonde.
8. Neutralização de Pegadinhas de Bancas: Desmontar armadilhas de eufonia, reconhecer que lhes retoma substantivos femininos plurais (às ordens) e refutar falsas alegações de "dupla regência" em verbos bitransitivos no Cebraspe.
