...
**Método central:** A concordância verbal parte da seta entre Ve e o núcleo de Su, mesmo quando o sujeito está distante, posposto, oculto ou possui estrutura complexa.
...
- Diferenciação entre classes de palavras (substantivo, adjetivo, pronome, preposição, verbo e conjunção).
...
│
└── 6. Regras Especiais Complementares
 ├── Partícula de Realce (SE): Verbos de ação espontânea/tempo decorrido (concordância na ordem direta)
 ├── Nomes Geográficos no Plural: Sem artigo = singular; Com artigo = plural
 ├── Pronome Relativo 'QUE': Verbo concorda em número e pessoa com o termo antecedente
 └── Infinitivo Flexionado: Facultativo quando o sujeito plural já foi expresso na oração principal
```
...
- *Contraexemplo (erro grave):* *"Podem fazerem verificações"* ou *"Deve existirem dúvidas"* (Incorreto: nunca se flexiona o verbo principal em locuções verbais desse tipo).
...
```text
 QUADRO SINÓPTICO: PA vs. PIS
┌──────────────────────────────────────────────┬──────────────────────────────────────────────┐
│ PRONOME APASSIVADOR (SE = PA) │ ÍNDICE DE INDETERMINAÇÃO DO SUJEITO (PIS) │
├──────────────────────────────────────────────┼──────────────────────────────────────────────┤
│ Transitividade: VTD ou VTDI │ Transitividade: VI, VTI ou VL │
│ Estrutura: Voz Passiva Sintética │ Estrutura: Sujeito Indeterminado │
│ Conversão: Admite Passiva Analítica │ Conversão: NÃO admite Passiva Analítica │
│ Sujeito: Sujeito Paciente Expresso │ Sujeito: Indeterminado (sem núcleo explícito)│
│ Concordância: Flexiona (Singular ou Plural) │ Concordância: Invariável (3ª Pessoa Singular)│
│ Exemplo: "Alugam-se salas" (= salas são...) │ Exemplo: "Precisa-se de médicos" │
└──────────────────────────────────────────────┴──────────────────────────────────────────────┘
```
...
- **Invariabilidade de "Trata-se de":** A estrutura *"trata-se de"* é formada por `VTI + PIS`. A forma plural *"tratam-se de"* constitui erro gramatical absoluto.
...
- *"[Demonstrar seus sentimentos e emoções] não é fácil."* $\rightarrow$ *"Isso não é fácil."* (Incorreto grafar *"não são fáceis"*).
- *"As inquietações que coube a ela [registrar em seu livro]..."* $\rightarrow$ *"Isso coube a ela"* (O sujeito de *caber* é a oração *registrar as inquietações*, exigindo o singular *coube*).
...
A tabela abaixo consolida as regras fundamentais do grupo `esta unidade` para consulta rápida:

| Fenômeno Gramatical | Estrutura Sintática | Regra Canônica de Concordância |
| :------------------------------------------ | :--------------------------------------------- | :---------------------------------------------------------------------------------------- |
| **Sujeito Simples** | `Núcleo único não preposicionado + Verbo` | Verbo concorda em número e pessoa com o núcleo. |
| **Locução Verbal** | `Verbo Auxiliar + Verbo Principal Nominal` | Verbo Auxiliar flexiona; Verbo Principal permanece invariável. |
| **Verbos Dar / Bater / Soar** | `Verbo + Horas` vs. `Instrumento + Verbo` | Concorda com o instrumento (sem prep.) ou com as horas (com prep. no relógio). |
| **Sujeito Composto Posposto** | `Sujeito Composto + Verbo` | Flexão obrigatória no**plural** (Concordância Total/Lógica). |
| **Sujeito Composto Anteposto** | `Verbo + Sujeito Composto` | **Dupla opção:** Concordância Atrativa (1º núcleo) ou Total (plural). |
| **Pessoas Gramaticais** | Pronomes retos distintos no sujeito | $1ª > 2ª > 3ª$ (ex.: *eu + ele = nós* $\rightarrow$ 1ª do plural). |
| **Conectivo COM (sem vírgula)** | `Substantivo 1 + com + Substantivo 2` | **Dupla opção:** Plural (aditivo) ou Singular (ênfase no 1º termo). |
| **Conectivo COM (com vírgula)** | `Subst. 1, com Subst. 2, + Verbo` | Termo entre vírgulas é Adjunto Adverbial$\rightarrow$ Verbo no **singular**. |
| **Conectivo OU (Exclusão)** | `Núcleo 1 + ou + Núcleo 2` | Ação restrita a um único agente$\rightarrow$ Verbo no **singular**. |
| **Conectivo OU (Adição)** | `Núcleo 1 + ou + Núcleo 2` | Ação cumulativa/compartilhada$\rightarrow$ Verbo no **plural**. |
| **Partícula Apassivadora (PA)** | `VTD / VTDI + SE + Sujeito Paciente` | Voz Passiva Sintética$\rightarrow$ Verbo concorda com o Sujeito Paciente. |
| **Índice de Indeterminação (PIS)** | `VI / VTI / VL + SE` | Sujeito Indeterminado$\rightarrow$ Verbo fixo na **3ª pessoa do singular**. |
| **Locução "Trata-se de"** | `Trata-se de + Complemento` | `VTI + PIS` $\rightarrow$ **Sempre singular** (*"tratam-se de"* não existe). |
| **Partícula de Realce (SE)** | Verbo de ação espontânea / tempo | `SE` expletivo $\rightarrow$ Verbo concorda com o sujeito na ordem direta. |
| **Sujeito Oracional** | `[Oração Subjetiva / Infinitivo] + Verbo` | Equivale ao pronome*"ISSO"* $\rightarrow$ Verbo na **3ª pessoa do singular**. |
| **Nome Geográfico no Plural** | Nome de lugar no plural | Sem artigo =**singular**; Com artigo = **plural**. |
| **Pronome Relativo QUE** | `Antecedente + que + Verbo` | Verbo concorda com o termo antecedente recuperado pelo pronome. |
...
```text
 ...
 │ ▼
 ▼ [ Teste da Passiva Analítica ]
 [ SE = P.I.S. / I.I.S. ] (Converter em "Ser + Particípio")
 │ │
 ▼ ┌─────────┴─────────┐
 ...
 ▼ ▼ ▼
 ┌────────────────────────┐ ┌──────────────────┐ ┌───────────────────────┐
 │ VERBO FIXO NA 3ª DO │ │ SE = P.A. │ │ Verbo de ação │
 │ SINGULAR │ │ (Passiva Sint.) │ │ espontânea / tempo? │
 └────────────────────────┘ └────────┬─────────┘ └──────────┬────────────┘
 │ │
 ▼ ▼
 ┌──────────────────┐ ┌───────────────────────┐
 │ VERBO CONCORDA │ │ SE = REALCE │
 │ COM O SUJEITO │ │ (Ordem direta: verbo │
 │ PACIENTE │ │ concorda com sujeito)│
 └──────────────────┘ └───────────────────────┘
```
...
3. **Teste da exclusão:** Pergunte se a condição é exclusiva de um só agente (ex.: ser presidente, ser a capital, casar com alguém). Se sim $\rightarrow$ Valor de Exclusão $\rightarrow$ **Verbo no singular**.
4. **Teste da adição:** Substitua mentalmente `ou` por `e`. Se ambos puderem praticar a ação concomitantemente sem contradição semântica $\rightarrow$ Valor de Adição $\rightarrow$ **Verbo no plural**.
...
### 8.1. Sujeito Anteposto vs. Sujeito Posposto ao Verbo

| Critério | Sujeito Composto Anteposto (`Suj. Comp. + Verbo`) | Sujeito Composto Posposto (`Verbo + Suj. Comp.`) |
|:----------------------------------------- |:--------------------------------------------------------------------- |:------------------------------------------------------------------------- |
| **Concordância Total / Lógica** | **Obrigatória no plural** (*"Fernanda e Guilherme estavam"*). | **Permitida no plural** (*"Estavam Fernando e Guilherme"*). |
| **Concordância Atrativa / Parcial** | **Proibida** na norma-padrão. | **Permitida com o 1º núcleo** (*"Estava Fernando e Guilherme"*). |
| **Base Teórica** |  |  |
...
|:----------------------------------------- |:--------------------------------------- |:------------------------------------------------------------- |:------------------------------------------ |
| `Subst. 1 com Subst. 2` (Sem vírgula) | Sujeito Composto / Termo Associado | **Dupla opção:** Singular enfático ou Plural aditivo. | *"O rei com a rainha chegou / chegaram."* |
| `Subst. 1, com Subst. 2,` (Com vírgula) | **Adjunto Adverbial de Companhia** | **Exclusivamente Singular** (concorda com Subst. 1). | *"O rei, com a rainha, chegou."* |
...
|:------------------------------- |:------------------------------------------------------------------- |:------------------------------------------------------------------- |
| **Existência Semântica** | O agente**existe** na realidade (*alguém*, *todo mundo*). | O agente**não existe** no plano real nem sintático. |
| **Tipologia Verbal** | `3ª pessoa plural` OU `VI/VTI/VL + SE (PIS)`. | Verbos impessoais (*haver* = existir; fenômenos meteorológicos). |
| **Comportamento Verbal** | Fixo na 3ª do plural ou 3ª do singular com`SE`. | Fixo na**3ª pessoa do singular** (*Havia problemas*). |
| **Base Teórica** |  |  |
...
- **Itens e Resoluções:**
 ...
 2. *"**Fazem** muito barulho aqueles meninos."* (Núcleo: *meninos* [plural]).
 3. *"**Faltam** vinte minutos para as dez horas."* (Núcleo: *minutos* [plural]).
 4. *"**Devem** existir dúvidas sobre aquele ponto..."* (Locução: *dúvidas* é sujeito $\rightarrow$ *devem existir*).
...
Ao concluir o grupo temático `esta unidade`, o estudante deve dominar os seguintes pilares da concordância verbal:

1. **Identificar com precisão o sujeito:** Superar a intuição sonora pela aplicação do método do *farejador de núcleo*, eliminando qualquer termo preposicionado da posição de sujeito.
2. **Flexionar corretamente locuções verbais:** Aplicar a flexão de tempo, modo, número e pessoa exclusivamente ao verbo auxiliar, preservando a forma nominal do verbo principal.
3. **Manejar o posicionamento do sujeito composto:** Reconhecer que o verbo anteposto autoriza a concordância atrativa com o primeiro núcleo ou total no plural, enquanto o verbo posposto impõe plural total obrigatório.
4. **Interpretar conectivos associativos:** Avaliar o impacto das vírgulas na preposição `COM` e discriminar contextualmente se a conjunção `OU` expressa exclusão mútua (verbo singular) ou adição inclusiva (verbo plural).
5. **Dominar a taxonomia da partícula SE:** Identificar instantaneamente se o `SE` atua como Pronome Apassivador (permitindo passiva analítica e concordância com o sujeito paciente) ou Índice de Indeterminação do Sujeito (travando o verbo na 3ª do singular), reconhecendo ainda as partículas de realce em ações espontâneas.
6. **Reconhecer o Sujeito Oracional:** Neutralizar armadilhas de falsos plurais aplicando o teste de substituição por *"ISSO"*, travando o verbo principal na 3ª pessoa do singular.
