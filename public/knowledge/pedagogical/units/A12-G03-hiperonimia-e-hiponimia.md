...
│
└── 5. Aplicações Lexicográficas e em Provas
 ├── Estrutura de Verbetes: [Entrada] — [Hiperônimo de Abertura] + [Particularizações]
 ├── Direcionalidade Obrigatória: [Específico -> Geral = Hiponímia] vs. [Geral -> Específico = Hiperonímia]
 └── Distinção CEBRASPE: Adjetivação Sintagmática (Qualificação) ≠ Coesão Lexical por Hiperonímia
```
...
A chave de validação consiste na proposição afirmativa: `[Termo A] é um TIPO DE [Termo B]`.
Se a proposição for verdadeira, $A$ é categoricamente o **hipônimo** e $B$ é o **hiperônimo**.
...
```text
 ...
 │
 ┌───────────────────▼────────────────────┐
 │ HIPÔNIMO (Específico) │ ──> Teste: "Pássaro é um
 │ ex.: Pássaros, Carro, Gelatina │ TIPO DE animal"
 └────────────────────────────────────────┘
```
...
### Tabela Sintética de Relações e Conceitos Canônicos
...
| **Sinonímia Contextual** | Relação de equivalência de sentido entre termos que operam no**mesmo patamar semântico**. | Não há subordinação de classe nem relação de "tipo de" ($A = B$). |  |
| **Campo Semântico (Anáfora Associativa)** | Conexão referencial estabelecida por termos do**mesmo domínio temático** sem sinonímia ou inclusão. | As palavras compartilham o mesmo universo discursivo sem relação de "tipo de". |  |
| **Anáfora Especificativa** | Retomada anafórica em que um**hipônimo específico particulariza um hiperônimo geral** antecedente. | Sentido: Geral$\rightarrow$ Específico ($HIPER \longrightarrow HIPO$). |  |
| **Retomada por Hiperônimo** | Retomada anafórica em que um**hiperônimo geral engloba um hipônimo específico** antecedente. | Sentido: Específico$\rightarrow$ Geral ($HIPO \longrightarrow HIPER$). |  |
...
| **Regra da Direcionalidade** | A classificação semântica é regida estritamente pelo papel do**primeiro termo** citado no comando. | Específico$\rightarrow$ Geral = Hiponímia; Geral $\rightarrow$ Específico = Hiperonímia. |  |
...
```text
 ...
 ▼
 ┌───────────────────────────────────────────┐
 │ Passo 1: Avaliar Equivalência Semântica │
 │ T1 e T2 estão no mesmo patamar │
 │ de sentido no contexto da frase? │
 └─────────────────────┬─────────────────────┘
 │
 SIM ───────────┴─────────── NÃO
 │ │
 ▼ ▼
 ┌──────────────────┐ ┌──────────────────┐
 │ SINONÍMIA │ │ Ir para Passo 2 │
 │ (ex.: ceia/jantar│ └─────────┬────────┘
 │ telas/quadros) │ │
 └──────────────────┘ ▼
 ┌───────────────────────────────────────────┐
 │ Passo 2: Aplicar Teste Lógico "TIPO DE" │
 │ "T1 é tipo de T2" OU "T2 é tipo de T1"?│
 └─────────────────────┬─────────────────────┘
 │
 SIM ───────────┴─────────── NÃO
 │ │
 ▼ ▼
 ┌──────────────────────────────────┐ ┌──────────────────────────────────┐
 │ Passo 2b: Checar Direcionalidade│ │ Passo 3: Avaliar Campo Semântico │
 │ - Se T1=Específico e T2=Geral: │ │ T1 e T2 pertencem ao mesmo tema │
 │ ──> HIPONÍMIA │ │ sem relação taxonômica de gênero?│
 │ - Se T1=Geral e T2=Específico: │ └──────────────────┬───────────────┘
 │ ──> HIPERONÍMIA │ │
 └──────────────────────────────────┘ SIM ───────────┴─────────── NÃO
 │ │
 ▼ ▼
 ┌──────────────────┐ ┌──────────────────┐
 │ CAMPO SEMÂNTICO │ │ Relações Lógicas │
 │ Anáfora │ │ Distintas │
 │ Associativa │ │ (Antonímia, │
 │ (ex.: febre/ │ │ Paronímia, │
 │ doença; pista/ │ │ Coordenação) │
 │ aeroporto) │ └──────────────────┘
 └──────────────────┘
```
...
### Tabela Comparativa de Fenômenos Semântico-Coesivos
...
| **Pegadinha Frequente** | Inversão da ordem pedida no comando. | Achar que exige sinônimo perfeito fora do texto. | Achar que*febre* é espécie de *doença*. | Confundir sumarização com mera sinonímia. |
