...
**Conexão forte:** Na revisão e reescrita, compare mapas antes e depois da pontuação para verificar correção, escopo e preservação ou alteração de sentido.
...
Para a plena compreensão deste módulo consolidado, o estudante deve dominar previamente:

1. Sintaxe da Oração e do Período: Distinção entre termos essenciais (sujeito, predicado), integrantes (objetos direto e indireto) e acessórios (adjunto adverbial, aposto); estrutura do período composto por coordenação e subordinação.
...
PONTUAÇÃO APLICADA: REVISÃO, CRITÉRIOS DE BANCAS E REESCRITA │ ├── 1. VÍRGULA E SINAIS DE ISOLAMENTO BILATERAL │ ├── Conjunção "E": Proibida (sujeitos iguais + aditivo puro) | Facultativa (valor conclusivo ou adversativo) │ ├── Adjunto Adverbial Final: Ordem direta -> Vírgula facultativa │ ├── Segmentos Intercalados (Adv / Expl): Exigem vírgulas casadas ou travessões duplos (simetria obrigatória) │ ├── Parênteses: Exclusivos para informações estritamente acessórias (removíveis sem truncamento) │ └── Oração Adjetiva Reduzida: Restritiva (sem vírgula / "somente") vs. Explicativa (entre vírgulas / totalidade) │ ├── 2. DOIS-PONTOS E TRAVESSÕES │ ├── Dois-Pontos: │ │ ├── Com Verbo de Elocução (dicendi): Introduz fala direta / citação │ │ ├── Sem Verbo de Elocução: Esclarecimento, especificação ou enumeração │ │ └── Tríplice Distinção: Exemplo (rol aberto, sem "e") | Enumeração (rol fechado por "e") | Explicação (abstrato) │ └── Travessões: │ ├── Simples no Início de Linha: Marcação de mudança de interlocutor em diálogos │ ├── Simples em Final de Frase: Substitui dois-pontos explicativos (: Explicação. -> — Explicação.) │ ├── Duplos Intercalados: Inserção de Informação (fato objetivo) vs. Comentário (opinião/juízo de valor) │ └── Não se confunde com citação de autoria quando o texto usa aspas │ ├── 3. PONTO E VÍRGULA E SEGMENTAÇÃO DE PERÍODO │ ├── Ponto e Vírgula: │ │ ├── Função 1: Separar blocos de enumerações complexas (itens com vírgulas internas) │ │ ├── Função 2: Separar orações coordenadas independentes (não subordinadas) │ │ ├── Conectivo "Agora": Atua como coordenativo adversativo (= mas/porém) -> admite ponto e vírgula │ │ └── Regra Negativa Absoluta: Ponto e vírgula NUNCA decorre de intercalação │ └── Ponto Final: │ ├── Conclusão/fechamento integral de período gramatical (não indica interrupção de pensamento) │ └── Substituição de travessão/vírgula por ponto final exige autonomia sintático-semântica da nova oração │ ├── 4. ASPAS E VALORES DISCURSIVOS │ ├── Citação Direta / Texto Alheio: Transcrição literal (evita plágio em bancas como a FGV) │ ├── Títulos de Obras Literárias: Identificação formal │ ├── Ênfase / Realce Semântico: Acentua o valor significativo de termo no debate (não é ironia) │ ├── Sentido Conotativo / Figurado: Termo deslocado de seu sentido denotativo literal │ ├── Expressões Populares / Metáforas: Mesmo com "no dizer do povo", mantêm finalidade figurada │ └── Hierarquia Gráfica: Aspas simples (' ') dentro de aspas duplas (" ") │ └── 5. INTERAÇÕES SISTÊMICAS E REESCRITA ├── Proibição Absoluta: VTD + Conjunção Integrante ("que" = "isso") não admite vírgula nem dois-pontos ├── Supressão de Travessões: Não pode ser "pura"; exige reposição de vírgulas de isolamento └── Colocação Pronominal: Troca de "e" por ";" cria limite oracional e veda próclise no início da nova oração
...
- Contraexemplo (Adição pura com mesmo sujeito): "Chegou cedo, e sentou na primeira fileira." (Incorreto na norma culta estrita).
...
3. Certifique-se de que há pontuação no início E no fim do segmento (duas vírgulas, dois travessões ou parênteses).
4. Aplique o teste de supressão : a leitura da frase ignorando o bloco intercalado deve manter a integridade sintática da oração principal.
...
3. Se a substituição "Verbo + isso" for perfeita, o "que" é conjunção integrante e a oração é subordinada substantiva.
4. Conclusão: é vedado qualquer sinal de pontuação entre a forma verbal e o conectivo "que".
...
1. A primeira vírgula separa duas orações coordenadas assindéticas (sua conversa é instrutiva / seu silêncio, formativo).
...
- Se o termo for um verbo de elocução (ex.: "disse mais uma vez: sou vigorosa..."), os dois-pontos introduzem uma fala direta.
- Se o termo for uma expressão nominal genérica (ex.: "ganhou sentido positivo, invejável: fazer sucesso na internet..." ou "Diz-se da melhor companhia: sua conversa..."), os dois-pontos introduzem a explicação/especificação da passagem antecedente.
...
1. Exemplificação: Amostra concreta e ilustrativa de uma ideia geral; trata-se de um rol aberto que não esgota a totalidade dos elementos (conectados apenas por vírgulas, sem conjunção final).
...
| --- | --- | --- | --- |
| Exemplificação | Concreta / Ilustrativa | Rol aberto (separado apenas por vírgulas, sem "e") | "Comprei frutas: banana, maçã." |
| Enumeração | Exaustiva / Finita | Rol fechado (presença da conjunção"e" no fim) | "Comprei frutas: banana e maçã." |
| Explicação | Abstrata / Teórica | Paráfrase ou definição conceitual | "Definiu a meta: vencer as eleições." |
...
- Exemplo validado (Cebraspe): "Nesse cenário, surgem as chamadas ações afirmativas: medidas políticas que visam acabar..." $\rightarrow$ "Nesse cenário, surgem as chamadas ações afirmativas — medidas políticas que visam acabar...".
...
1. Separar itens em enumerações complexas, notadamente quando os itens são extensos ou já possuem vírgulas internas.
2. Separar orações coordenadas independentes (não subordinadas), especialmente quando longas ou com conectivos deslocados.
...
- Proposição A: ausência de culpa / peso mínimo.
- Proposição B: presença de culpa / implicações graves.

O termo "agora" atua contrapondo os fatos (= mas/porém). Por se tratar de oração coordenada (e não de oração subordinada adverbial temporal), é gramaticalmente legítimo substituir o ponto final por ponto e vírgula: "...ela é mínima; agora, dizer...".
...
1. Citação Direta: Reprodução literal da fala de personagem ou de argumento de autoridade.
2. Títulos de Obras: Delimitação de títulos de livros, poemas ou produções artísticas.
3. Ênfase / Valor Significativo: Destacar uma palavra-chave crucial no debate, cuja importância o autor deseja sublinhar (não se confunde com ironia).
4. Sentido Conotativo / Figurado: Indicar que a palavra foi deslocada de seu sentido denotativo literal (translação metafórica).
5. Expressões Populares / Metáforas Regionais: Sinalização de termos informais ou metafóricos (ex.: "seca encarrilhada"), cuja finalidade é figurada, mesmo que sucedidos pela expressão "no dizer do povo".
6. Ironia: Emprego deliberado da palavra para significar o oposto de sua literalidade denotativa.
...
- Exemplo (FCC / Paulo Freire): "...Eis aí a concepção 'bancária' da educação..." dentro de um longo bloco de citação já aberto por aspas duplas.
...
Para consulta rápida e precisa, consolidam-se abaixo as regras canônicas do grupo:

1. Vírgula antes do "E":

1. Adjunto Adverbial Final: Posicionado no fim da oração na ordem direta $\rightarrow$ Vírgula facultativa .
2. Intercalações Adverbiais/Explicativas: Exigem isolamento bilateral obrigatório por vírgulas casadas ou travessões duplos .
3. Oração Adjetiva (Particípio):

1. Verbo Transitivo Direto + Oração Substantiva: Proibição absoluta de pontuação entre VTD e "que" integrante .
2. Vírgula Vicária: Sinaliza a elipse/zeugma de verbo antecedente em oração coordenada .
3. Parênteses: Uso restrito a informações acessórias cuja supressão não quebra a estrutura sintática .
4. Travessão Duplo: Inserção de Informação = Fato objetivo; Inserção de Comentário = Opinião/julgamento .
5. Dois-Pontos: Antecedidos por verbum dicendi $\rightarrow$ introduzem fala; antecedidos por nome/ideia geral $\rightarrow$ introduzem explicação .
6. Exemplo vs. Enumeração vs. Explicação:

1. Dois-Pontos $\rightarrow$ Travessão Simples: Válido em fim de período (:.... $\rightarrow$ —....) .
2. Ponto e Vírgula: Usado exclusivamente em enumerações com vírgulas internas e orações coordenadas; nunca decorre de intercalação .
3. "Agora" Adversativo: Quando equivale a mas/porém, encabeça oração coordenada e aceita ponto e vírgula antecedente .
4. Segmentação por Ponto Final: Exige que a nova oração possua verbo e sentido autônomo completo .
5. Ponto e Vírgula e Próclise: A troca de "e" por ";" cria início de nova oração, vedando a próclise e exigindo ênclise .
...
1. Se a banca afirmar que são "exemplos", mas a lista é fechada com "e", marque o item como ERRADO.
...
1. Verifique se os blocos constituem enumeração de itens com vírgulas internas ou orações coordenadas independentes:
...
3. Se fechar no ponto final, substitua os dois-pontos por um travessão simples (—....).
4. Confirme que a substituição preserva a gramática e o sentido, agregando ênfase.
...
3. Se a oração não depender sintaticamente de outra oração ausente para fazer sentido, valide a divisão do período.
...
3. Se houver pronome átono em próclise logo após a pontuação forte sem a presença de palavra atrativa, classifique a reescrita como INCORRETA por violação da colocação pronominal.
...
| Sinal / Fenômeno | Função Normativa Primária | Aplicação Típica em Prova | Pegadinha Recorrente |
| --- | --- | --- | --- |
| Vírgula antes de "E" | Separa orações aditivas com sujeitos diferentes ou com valor conclusivo/adversativo. | "Chegou atrasado, e não se inteirou" (conclusivo = facultativa). | Achar que vírgula antes de "e" com mesmo sujeito é sempre proibida. |
| Parênteses | Isola dados estritamente acessórios. | Inserção de comentários removíveis sem prejuízo sintático. | Isolar verbos ou complementos essenciais da oração principal. |
| Travessão Duplo | Isola intercalações explicativas ou enfáticas. | Inserção de Fato (Informação) vs. Comentário (Opinião). | Confundir isolamento de termo explicativo com fala de personagem. |
| Dois-Pontos | Anuncia fala direta, enumeração ou explicação. | Apósverbum dicendi (fala); após termo geral (explicação). | Confundir enumeração (fechada por "e") com exemplificação (rol aberto). |
| Ponto e Vírgula | Separa enumerações complexas ou orações coordenadas. | Lista de crimes com vírgulas internas; oração com "agora" adversativo. | Julgar que ponto e vírgula é causado por intercalação interna. |
| Aspas Duplas / Simples | Delimita citações, títulos, conotação e destaque semântico. | Termos metafóricos ou ênfase no debate ideológico. | Classificar automaticamente qualquer palavra aspeada como "ironia". |
| Ponto Final | Conclui e encerra períodos gramaticais completos. | Fracionamento de períodos extensos com orações autônomas. | Julgar que ponto final atua para "interromper pensamentos". |
...
Ao concluir este módulo consolidado de revisão e reescrita, o estudante domina com precisão cirúrgica:

1. O manejo de conectivos e vírgulas polissêmicas: O reconhecimento da vírgula facultativa antes de "e" com valor conclusivo ou adversativo, a obrigatoriedade de vírgulas casadas para orações adverbiais intercaladas e a pontuação opcional em adjuntos adverbiais finais na ordem direta.
2. O domínio dos sinais de isolamento e esclarecimento: O emprego restrito de parênteses para termos puramente acessórios, o uso de travessões duplos para inserir fatos (informações) ou comentários (opiniões), a distinção estrutural entre travessão de diálogo e de intercalação, e a conversão de dois-pontos finais em travessão simples.
3. A distinção semântica fina em listas e discursos: A diferenciação categórica entre exemplificação (rol aberto sem "e"), enumeração (rol exaustivo fechado com "e") e explicação abstrata no Cebraspe.
4. As regras estruturais de ponto e vírgula e ponto final: O entendimento de que ponto e vírgula atua exclusivamente em enumerações com vírgulas internas e em orações coordenadas (nunca em intercalações), a caracterização de "agora" como conjunção adversativa, e o critério de autonomia oracional para segmentação por ponto final.
5. As funções expressivas das aspas e elipses verbais: A identificação de termos conotativos/figurados, títulos, ênfase no debate ideológico, prevenção de plágio (texto alheio), subordinação de aspas simples, além da caracterização da vírgula vicária marcando zeugma verbal.
6. A avaliação sistêmica em questões de reescrita: A proibição absoluta de pontuação entre VTD e oração subordinada substantiva integrante, a invalidade da supressão pura de travessões em intercalações, e os impactos fatais da troca de "e" por ponto e vírgula na colocação pronominal em início de oração.
