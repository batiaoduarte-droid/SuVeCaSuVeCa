...
- ➔ 1. VÍRGULA E SINAIS DE ISOLAMENTO BILATERAL
➔ Conjunção "E": Proibida (sujeitos iguais + aditivo puro) | Facultativa (valor conclusivo ou adversativo)
- ➔ Adjunto Adverbial Final: Ordem direta -> Vírgula facultativa
➔ Segmentos Intercalados (Adv / Expl): Exigem vírgulas casadas ou travessões duplos (simetria obrigatória)
- ➔ Parênteses: Exclusivos para informações estritamente acessórias (removíveis sem truncamento)
➔ Oração Adjetiva Reduzida: Restritiva (sem vírgula / "somente") vs. Explicativa (entre vírgulas / totalidade)
- ➔ 2. DOIS-PONTOS E TRAVESSÕES
➔ Dois-Pontos:
- ➔ Com Verbo de Elocução (dicendi): Introduz fala direta / citação
➔ Sem Verbo de Elocução: Esclarecimento, especificação ou enumeração
- ➔ Tríplice Distinção: Exemplo (rol aberto, sem "e") | Enumeração (rol fechado por "e") | Explicação (abstrato)
➔ Travessões:
- ➔ Simples no Início de Linha: Marcação de mudança de interlocutor em diálogos
➔ Simples em Final de Frase: Substitui dois-pontos explicativos (: Explicação. -> — Explicação.)
- ➔ Duplos Intercalados: Inserção de Informação (fato objetivo) vs. Comentário (opinião/juízo de valor)
➔ Não se confunde com citação de autoria quando o texto usa aspas
- ➔ 3. PONTO E VÍRGULA E SEGMENTAÇÃO DE PERÍODO
➔ Ponto e Vírgula:
- ➔ Função 1: Separar blocos de enumerações complexas (itens com vírgulas internas)
➔ Função 2: Separar orações coordenadas independentes (não subordinadas)
- ➔ Conectivo "Agora": Atua como coordenativo adversativo (= mas/porém) -> admite ponto e vírgula
➔ Regra Negativa Absoluta: Ponto e vírgula NUNCA decorre de intercalação
- ➔ Ponto Final:
➔ Conclusão/fechamento integral de período gramatical (não indica interrupção de pensamento)
- ➔ Substituição de travessão/vírgula por ponto final exige autonomia sintático-semântica da nova oração
➔ 4. ASPAS E VALORES DISCURSIVOS
- ➔ Citação Direta / Texto Alheio: Transcrição literal (evita plágio em bancas como a FGV)
➔ Títulos de Obras Literárias: Identificação formal
- ➔ Ênfase / Realce Semântico: Acentua o valor significativo de termo no debate (não é ironia)
➔ Sentido Conotativo / Figurado: Termo deslocado de seu sentido denotativo literal
- ➔ Expressões Populares / Metáforas: Mesmo com "no dizer do povo", mantêm finalidade figurada
➔ Hierarquia Gráfica: Aspas simples (' ') dentro de aspas duplas (" ")
- ➔ 5. INTERAÇÕES SISTÊMICAS E REESCRITA ➔ Proibição Absoluta: VTD + Conjunção Integrante ("que" = "isso") não admite vírgula nem dois-pontos ➔ Supressão de Travessões: Não pode ser "pura"; exige reposição de vírgulas de isolamento ➔ Colocação Pronominal: Troca de "e" por ";" cria limite oracional e veda próclise no início da nova oração
