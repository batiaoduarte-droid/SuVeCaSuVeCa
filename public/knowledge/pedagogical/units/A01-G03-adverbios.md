...
```text
 ...
 │ │
 ADJETIVO ADVÉRBIO
 (Classe Variável) (Classe Invariável)
 • Atribui qualidade/estado • Função Sintática: Adjunto Adverbial
 • Função: Adjunto Adnominal / • Valores Circunstanciais:
 Predicativo ├─ Se ligado a ADJ / ADV ──► INTENSIDADE (Intensificador)
 • NÃO expressa circunstância └─ Se ligado a VERBO:
 ├─ Modo (Jeito: "como?")
 ├─ Tempo (Quando? / Frequência)
 ├─ Lugar (Onde?)
 ├─ Afirmação / Negação
 └─ Intensidade
```
...
|:------------------------------------- |:---------------------------------------------- |:---------------------------------------------------------- |
| **Teste do Plural** | "Elas estão**bem**." (Termo invariável) | "Elas estão**felizes**." (Termo flexiona em número) |
...
- *vital* conecta-se a *papel* (substantivo).
- Logo, *vital* é **adjetivo**.
...
| **Dupla Face de 'Nunca'** | O vocábulo*nunca* expressa simultaneamente valor de negação e de tempo. |  |
...
### Tabela Comparativa de Contrastes Gramaticais Críticos
...
| **Sufixo '-mente'** | **Modo** (*rapidamente* = jeito rápido) | **Tempo / Frequência** (*raramente* = poucas vezes no tempo) | Perguntar: Indica o*jeito* de fazer ou *quando/com que frequência* ocorre? |
