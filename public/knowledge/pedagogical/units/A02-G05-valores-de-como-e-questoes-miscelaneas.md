...
│
└── 4. ZONAS DE ALTO RISCO E DISTINÇÕES CRÍTICAS
 ├── Porquanto (Causa/Explicação) vs. Conquanto (Concessão)
 ├── Posto que (Concessão) vs. Visto que (Causa)
 ├── Posição do "Pois" (Início de oração com intercalação vs. Posposto ao verbo)
 └── Pontuação Exemplificativa (como X vs., por exemplo, X)
```
...
```text
CONJUNÇÕES CAUSAIS: já que | visto que | uma vez que | porque | porquanto | COMO
...
```text
Construção com "como": descrição de ações, como "subindo as escadas"...
...
### Quadro sintético de valores do conectivo "Como"
...
### Tabela comparativa de armadilhas semânticas frequentes em bancas
