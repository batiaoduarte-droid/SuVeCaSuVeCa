...
```text
Emprego da Crase em Palavras no Plural (Padrão Cebraspe)
 ├── 1. Regente exige preposição "a" + Palavra feminina no plural
 ...
 │ └── "à" no singular antes de plural (Erro crasso: "a" no singular não admite crase diante de plural)
 ├── 2. Critérios de Avaliação nas Questões de Concurso
 │ ├── "O acento é obrigatório em 'às'?" -> Sim, pois não se pode simplesmente remover o acento deixando "as" (artigo puro).
 │ ├── "A inserção de crase em 'a' singular é facultativa/correta?" -> Errado, é proibida ("nem a pau").
 │ └── "A substituição de 'a' por 'às' mantém a correção gramatical?" -> Certo, ambas as formas são gramaticalmente válidas.
 └── 3. Supressão de Termos Intervenientes e Regência
 └── "subjacente a todas as relações" -> ao suprimir "a todas", "subjacente" rege diretamente "as relações" -> "subjacente às relações" (crase torna-se obrigatória).
```
...
```text
 ...
 +----------------------------+----------------------------+
 | |
 REGRA 1: FENÔMENO FONÉTICO (FF) REGRA 2: LOCUÇÕES FEMININAS (LF)
 (Preposição A + Artigo/Pronome A) (Expressões pluriverbais de núcleo feminino)
 | |
 | +--------------+--------------+
 | | |
...
```text
 ...
 ┌────────────────┬──────────────┴───────────────┬────────────────┬────────────────┐
 ▼ ▼ ▼ ▼ ▼
2 REGRAS 3 CASOS DE CRASE CRASE NO PLURAL CRASE PROIBIDA CRASE E PARALELISMO
FUNDAMENTAIS FACULTATIVAS (Regras de Uso) (Vedações do (Simetria Sintática)
 │ Acento)
 ├─────────────────────────────┐
 ▼ ▼
FENÔMENO FONÉTICO (FF) LOCUÇÃO FEMININA (LF)
(Fusão da Preposição A) (Adverbial, Prepositiva,
 │ Conjuntiva)
 ├─► + A (Artigo Feminino)
 ├─► + A QUAL / AS QUAIS (Pronomes Relativos)
 ├─► + AQUILO / AQUELE(A) (Pronomes Demonstrativos)
 └─► + A (Pronome Demonstrativo antes de 'QUE' ou 'DE')
```
...
(Preposição 'a' + Artigo/Pronome 'a') (Acento Grave Fixo / Obrigatório)
 ...
 │ └─ Exemplo: "deixar o pão à porta" (deixar + a + a porta) │ └─ Lugar/Direção: "à direita", "à porta"
 │ ├─ Locuções Temporais Femininas
 └─ Atenção ao Contexto: │ └─ Tempo: "à hora da passagem do ano"
 └─ "Deixar a" (proximidade) vs. "Deixar em" (dentro de) ├─ Expressões Cristalizadas Femininas
 │ └─ "trazer à tona" (em evidência)
 └─ Contraposição Obrigatória:
 └─ Núcleo Masculino: "a granel" (SEM crase)
```
...
└── 4. Exercícios de Fixação
 ├── Identificação de ocorrências facultativas vs. obrigatórias vs. proibidas
 └── Validação de regência e classes gramaticais
```
...
No quadro digital, ao lado de `FF`, A abordagem didática abre uma chave e escreve:
`A + { A (ARTIGO) | A QUAL / AS QUAIS | AQUILO / AQUELE(A) | A (PRON. DEMONS.) ANTES DE QUE / DE }`.
...
- Sublinhado no verbo *deixar*;
- Traçado de seta conectando *deixar* ao segmento *à porta*;
...
- Destaque no verbo *ver*;
- Destaque em *a cara do mascate* como objeto direto;
- Delimitação de *à porta* e associação com a sigla `LF` (Locução Feminina).
...
- Marcação do gênero masculino da palavra *granel*.
...
- *Ante a ausência do réu.* (Correto, sem crase).
