...
• despercebido (não notado, invisível) vs. desapercebido (desprevenido, desprovido)
• auferir (obter, colher lucros/vantagens) vs. aferir (medir, calibrar, conferir)
• diferir (distinguir, divergir, adiar) vs. deferir (aprovar, conceder, despachar favoravelmente)
• preito (homenagem, reverência, tributo) vs. pleito (eleição, disputa, contenda judicial)
• tráfico (comércio ilegal, mercancia) vs. tráfego (trânsito, circulação de veículos)
• emigrar (sair da pátria de origem) vs. imigrar (entrar em país estrangeiro)
• expirar (soltar o ar, findar prazo) vs. inspirar (puxar o ar, incutir/despertar ideias)
• conjuntura (situação atual, contexto) vs. conjectura (suposição, divagação, hipótese)
• acidente (imprevisto grave/destrutivo) vs. incidente (imprevisto leve/sem danos)
• descrição (ato de descrever, detalhar) vs. discrição (reserva, prudência, recato)
• acerca de (a respeito de, sobre) vs. cerca de (aproximadamente)
```
...
### Quadro Sinótico de Relações Léxico-Semânticas
...
[Início da Análise da Sentença]
 │
 ▼
Passo 1: Testar a Via 1 (Palavra Isolada / Polissemia Pura)
 └─ Há vocábulo polissêmico ou homônimo sem restrição no cotexto? (ex.: "banco", "cachorro")
 ├─ SIM ──> Classificar como AMBIGUIDADE LEXICAL / IMPRECISÃO VOCABULAR.
 └─ NÃO ──> Avançar para o Passo 2.
 │
 ▼
Passo 2: Testar a Via 2 (Conectivos Específicos "DE" e "E")
 ├─ Há preposição "DE" ligada a substantivo abstrato de ação?
 │ └─ O termo preposicionado pode ser Agente (Adj. Adnominal) E Paciente (Compl. Nominal)?
 │ └─ SIM ──> Classificar como INDEFINIÇÃO AGENTE / PACIENTE.
 └─ Há conjunção "E" ligando sujeitos com verbo transitivo/recíproco?
 └─ Pode indicar Soma Aditiva independente OU Ação Conflituosa/Recíproca mútua?
 └─ SIM ──> Classificar como MAU USO DA COORDENAÇÃO.
 │
 ▼
Passo 3: Testar a Via 3 (Setas de Referenciação Estrutural)
 ├─ Há forma nominal de GERÚNDIO (-ndo) ou PARTICÍPIO (-do) com duplo sujeito possível?
 │ └─ SIM ──> Classificar como MAU USO DAS FORMAS NOMINAIS.
 ├─ Há pronome RELATIVO ("que") com duplo antecedente ou POSSESSIVO ("seu") com 2 sujeitos de 3ª pessoa?
 │ └─ SIM ──> Classificar como MAU USO / MÁ COLOCAÇÃO DE PRONOMES.
 └─ Há modificador flutuante entre verbo e substantivo (Adjetivo vs. Advérbio) ou entre dois verbos?
 └─ SIM ──> Classificar como MÁ COLOCAÇÃO DOS TERMOS.
 │
 ▼
[Fim do Diagnóstico]
```
...
### Tabela 1 — Ambiguidade Lexical vs. Ambiguidade Estrutural
...
| :---------: | :---------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------- |
| **1** | *A mãe viu o filho chegando em casa bem tarde.* | O gerúndio*chegando* pode referir-se à *mãe* ou ao *filho*. |
| **2** | *A abordagem didática de Maria terminou a aula fazendo apontamentos no seu caderno.* | O possessivo*seu* pode remeter a *Maria* ou ao *professor*. |
| **3** | *Falei com a chefe que estava com vertigem.* | *Que* oscila entre pronome relativo (*a qual*) e conjunção integrante (*isso*). |
| **4** | *João e Maria se casaram.* | Conectivo*e* oscilando entre casamento mútuo (reciprocidade) ou com terceiros (soma). |
| **5** | *Chamar os bombeiros já ficou na moda.* | Advérbio*já* flutuando entre *chamar* (chamar agora) e *ficou na moda*. |
...
| **8** | *Júlio conversou com Paulo sentado no muro.* | O particípio*sentado* pode concordar com *Júlio* ou com *Paulo*. |
...
- Em **A**, *eminente poeta* está perfeito (ilustre).
- Em **B**, progredir na vida exige o homófono **ascender** (com sc).
- Em **C**, não ser notado exige o parônimo **despercebido**.
- Em **D**, sentido de existência de alcance exige o homófono **haja** (com h).
- **Gabarito oficial:** **A**
...
**Desenvolvimento e Resolução:**
*Sessão*, *seção* e *cessão* possuem idêntica realização fonética (`som =`) e grafias distintas (`grafia ≠`). São estritamente **homônimas homófonas**.
...
- O item **C** opõe *eminentes* (ilustres) a *iminentes* (prestes a acontecer), constituindo **parônimos**.
- O item E opõe *mangas* (fruta) a *mangas* (roupa), que são homônimos perfeitos.
- **Gabarito oficial:** **C**
...
- Em **A**, *descrição* (detalhar) e *discrição* (reserva) são **parônimos**.
- Em C, *sede* (/ê/) e *sede* (/é/) são homônimos homógrafos.
...
- Em B, *o moral* (masculino) = ânimo/disposição.
- Em C, *conserto* (com s) = reparo.
- Em D, *conjuntura* = contexto/situação.
- **Gabarito oficial:** **A**
...
- Em A, *acidentes* está correto (eventos graves/fortuitos da natureza).
- **Gabarito oficial:** **B**
...
**Desenvolvimento e Resolução:**
O item define corretamente palavras homógrafas (mesma grafia), mas traz um exemplo falso: *sessão* (com s inicial) e *cessão* (com c inicial) possuem grafias manifestamente distintas, configurando **homófonas**, e não homógrafas.
...
- **Cessão (Inicia com C):** Lembrar do verbo **C**eder $\rightarrow$ *ato de transferir direitos ou posse*.
