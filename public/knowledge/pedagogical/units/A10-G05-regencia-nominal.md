...
```text
SINTAXE DE REGÊNCIA
 ├── Regência Verbal (Verbo -> Complementos Diretos / Indiretos)
 └── REGÊNCIA NOMINAL (Nome -> Complemento Nominal)
 │
 ...
 │
 └── 4. Fenômenos de Intersecção
 ├── Determinação Semântica: Prep. simples (genérico) vs. Prep. + Artigo / Crase (determinado)
 ├── Pronome Possessivo Feminino: Artigo facultativo (de nossas vs. das nossas)
 └── Paralelismo Sintático: Recurso de ênfase (FGV) vs. Vício de redação (CEBRASPE)
```
...
| **** | Obrigatoriedade de "De" em Completivas | Substantivos abstratos exigem a preposição*de* antes de orações introduzidas por *que* (*dúvida de que*). |  |
...
| **** | Crase em Regência Nominal | Preposição`a` exigida pelo nome + artigo feminino `a` resulta em crase obrigatória (`à`). |  |
| **** | Regência do Verbo*Preferir* | VTDI: exige OD sem preposição e OI com preposição`a` (rejeita "mais que", "do que"). |  |
...
4. **Confrontar com o Regime Padrão Dicionarizado:** Verificar se a preposição usada é a exigida pela norma culta ou se pertence ao rol de dupla regência legítima.
5. **Verificar a Ocorrência de Fusão com Artigo (Crase ou Combinação):** Se o nome reger `a` e o termo regido for substantivo feminino com artigo, validar a crase (`à`); se masculino, validar a combinação (`ao`).
...
| **Semântica de Preposições** | **Sobre:** indica posição superior, incidência de impacto ou assunto (*impacto sobre*). | **Sob:** indica posição inferior física, tutela ou subordinação (*sob o comando*). | O impacto de um projeto ou evento incide*sobre* a sociedade (não *sob*). |
...
- **Item C:** *"Ficou furioso _____ os alunos."* $\rightarrow$ **com** (*furioso* = Adjetivo).
- **Item D:** *"Não houve a necessidade _____ que outros clientes fossem contatados."* $\rightarrow$ **de** (*necessidade* = Substantivo).
- **Item E:** *"Foi anunciada a ampliação _____ nossas reservas..."* $\rightarrow$ **de** ou **das** (*ampliação* = Substantivo; artigo facultativo diante de possessivo).
- **Item F:** *"Não há dúvida _____ que os números são bons."* $\rightarrow$ **de** (*dúvida* = Substantivo).
- **Item G:** *"Tinha aversão _____ pessoas superficiais."* $\rightarrow$ **a** ou **por** (*aversão* = Substantivo de dupla regência; rejeita `de`).
- **Item H:** *"Era propenso _____ maus hábitos."* $\rightarrow$ **a** ou **para** (*propenso* = Adjetivo de dupla regência).
- **Item I:** *"Estamos imunes _____ crises existenciais."* $\rightarrow$ **a** ou **de** (*imunes* = Adjetivo de dupla regência).
