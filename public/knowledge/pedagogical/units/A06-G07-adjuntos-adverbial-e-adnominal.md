...
>
> Compreender e definir os princípios normativos e conceituais de Adjuntos Adverbial e Adnominal. Aplicar regras canônicas e procedimentos decisórios de Adjuntos Adverbial e Adnominal na resolução de questões. Analisar o papel de Adjuntos Adverbial e Adnominal no eixo Método central e reconhecer armadilhas e variações de bancas.
...
Como termo acessório, o adjunto adverbial não é exigido pela transitividade direta ou indireta do verbo. Ele responde a perguntas periféricas sobre o evento: Quando? Onde? Como? Por que motivo? Para que fim? Com quem? Sob que condição?. Para sistematizar o vasto rol de circunstâncias adverbiais cobradas em provas e correlacioná-las à morfologia das conjunções e à sintaxe do período composto, consolida-se o mnemônico 6C + FTP:

- 6C (Seis circunstâncias iniciadas pela letra C):

1. Causa (morrem de exaustão);
2. Condição (sem estudo, não haverá aprovação);
3. Concessão (apesar da chuva, saímos);
4. Comparação (agia como um mestre);
5. Consequência (comeu a ponto de passar mal);
6. Conformidade (agiu segundo as regras).

- FTP (Três circunstâncias iniciadas por F, T, P):

1. Finalidade (estudou para a prova);
2. Tempo (sairemos amanhã / pela manhã);
3. Proporção (à medida que subia, cansava).
...
| --- | --- | --- |
| Status Sintático | TermoAcessório (periférico / dispensável). | TermoIntegrante (complemento obrigatório por regência). |
| Natureza da Preposição | PreposiçãoNocional (carrega semântica própria de modo, causa, tempo, lugar). | PreposiçãoGramatical (mero conector imposto pelo verbo transitivo indireto). |
...
| Predicações Compatíveis | Coexiste com qualquer verbo (VI, VTD, VTI, VTDI, VL). | Ocorre exclusivamente com verbosVTI e VTDI. |
...
| --- | --- | --- |
| Natureza na Predicação | Termoacessório dispensável. | Termoessencial integrante da predicação verbo-nominal. |
...
1. Diferenciação entre Adjunto Adverbial e Objeto Indireto:
