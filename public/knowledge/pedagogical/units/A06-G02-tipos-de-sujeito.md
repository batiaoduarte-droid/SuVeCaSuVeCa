...
**Método central:** O sujeito é reconstruído pela concordância e pelo sentido, podendo ser anteposto, posposto, oculto, indeterminado ou inexistente.
...
* **Morfologia Básica:** Identificação de classes de palavras (substantivo, pronome pessoal, pronome demonstrativo, pronome indefinido, numeral, preposição e conjunção integrante).
...
```text
 ...
 ┌──────────────────────────────────┴──────────────────────────────────┐
 ▼ ▼
 PROTOCOLO OPERACIONAL TAXONOMIA DOS TIPOS
 ┌─────────────────────────┐ │
 │ Passo 1: Interrogar V. │ ┌───────────────────────┼───────────────────────┐
 │ "Quem/O que é que + V?" │ ▼ ▼ ▼
 └─────────────┬───────────┘ SUJEITO DETERMINADO SUJEITO INDETERMINADO SUJEITO INEXISTENTE
 │ (Agente Identificável) (Existe, mas oculto) (Oração Sem Sujeito)
 ▼ │ │ │
 ┌─────────────────────────┐ ┌────────┼────────┐ ┌───────┼───────┐ ┌───────┼───────┐
 │ Passo 2: Extrair Núcleo │ │ │ │ │ │ │ │ │ │
 │ • Substantivo/Pron/Num │ Simples Composto Oculto 3ª P. 3ª Sing Inf. Haver Fazer Fenôm.
 │ • VEDAÇÃO DE PREPOSIÇÃO │ (1 Núc.) (2+ Núcs)(Elípt.) Plural + SE Impes. (Exist.)(Tempo) Natureza
 └─────────────┬───────────┘ │ │ │ (s/ ctx) (IIS) ("Estudar") │ │ (Literal)
 │ │ │ │ │ │ │ │ │ │
 ▼ └────────┼────────┘ └───────┼───────┘ └───────┼───────┘
 ┌─────────────────────────┐ │ │ │
 │ Passo 3: Classificar │ ▼ ▼ ▼
 │ (Contagem / Estrutura) │ Sujeito Oracional Marca Formal da Verbos Impessoais
 └─────────────────────────┘ (Oração Subjetiva) Língua Escrita (3ª P. Singular)
 • Teste do "ISSO" (Norma Culta) [Exceção: Sentido Figurado
 • Contém VERBO próprio -> Sujeito Simples]
```
...
Análise Sintática: "Alguém" = Núcleo Único do Sujeito ──> SUJEITO SIMPLES
```
...
1. **Via 1 (Verbo na 3ª pessoa do plural sem referente):** O verbo é conjugado no plural sem que haja sujeito explícito na oração e sem que exista referente em orações anteriores imediatas (*"Esqueceram de trancar a porta"* / *"Chegaram cedo"*). Se houver referente textual, o sujeito é oculto.
...
1. **Verbo HAVER com sentido de existir, ocorrer, suceder ou acontecer:** Permanece na 3ª pessoa do singular. O termo substantivo que o acompanha exerce a função de **Objeto Direto**, e não de sujeito (*"Há muita gente na praia"* / *"Houve algumas rebeliões"* / *"porque há muita concorrência e maior seleção"*).
2. **Verbos FAZER e HAVER indicando tempo decorrido ou clima:** Permanece estritamente na 3ª pessoa do singular (*"Faz três anos que estudo"* / *"Faz dois anos de sua partida"* / *"Há dois meses não o vejo"*). O tempo decorrido não é sujeito; é vedada a flexão no plural como *"fazem duas horas"*.
...
```text
Passo 1: Identificar a estrutura: "É imprescindível analisar os impactos da carga tributária"
Passo 2: Substituir a oração subordinada por "ISSO": "É imprescindível ISSO"
Passo 3: Reorganizar na ordem direta: "ISSO é imprescindível"
Passo 4: Confirmar verbo no interior do bloco ("analisar") ──> SUJEITO ORACIONAL (Verbo "é" na 3ª Sing.)
```
...
```text
 ...
 ┌────────────────────────────────┴────────────────────────────────┐
 ▼ ▼
SE = PRONOME APASSIVADOR (PA) SE = ÍNDICE DE INDETERMINAÇÃO (IIS)
• Verbo: VTD ou VTDI • Verbo: VTI, VI ou VL
• Termo seguinte: NÃO PREPOSICIONADO • Termo seguinte: PREPOSICIONADO
• Função do termo: SUJEITO PACIENTE (Determinado) • Função sintática: SUJEITO INDETERMINADO
• Conversão: Admite Passiva Analítica • Conversão: NÃO admite Passiva Analítica
• Exemplo: "Alugam-se casas" (= Casas são alugadas) • Exemplo: "Trata-se de problemas"
 │ │
 └────────────────────────────────┬────────────────────────────────┘
 ▼
 MARCA DE FORMALIDADE DA LÍNGUA ESCRITA
 (Assertiva canônica da banca Cebraspe)
```
...
Esta seção sumariza as formulações canônicas definitivas estabelecidas na matriz comparativa da professora Adriana Figueiredo:

| Tipologia do Sujeito | O Sujeito Existe? | Subdivisões Formais | Regra Canônica / O que Decorar | Exemplo Canônico de Fixação | ID de Detalhamento |
|:------------------------------------------------------ |:------------------------: |:---------------------------------------------------------------------------------------------------- |:------------------------------------------------------------------------------------------------------------------------------------------------------------------- |:----------------------------------------------------------------------- |:----------------: |
| **1. Sujeito Simples** | **SIM** | Não há subdivisões | Apresenta**exatamente um único núcleo** substantivo explícito (inclusive se for pronome indefinido). | `[O menino] chegou.[Alguém] acordou cedo.` |  |
| **2. Sujeito Oculto (Elíptico)** | **SIM** (escondido) | **(I) Desinencial****(II) Não-desinencial** | **(I)** Verbo na 1ª pessoa (*EU / NÓS*) marcado na terminação.**(II)** Verbo na 3ª pessoa do singular cujo **referente está no contexto**. | `Estou feliz.` / `Estamos felizes.João chegou. [Ele] Estava feliz.` |  |
| **3. Sujeito Composto** | **SIM** | Não há subdivisões | Apresenta**dois ou mais núcleos** explícitos coordenados (*e, nem, ou*). | `[João e Maria] chegaram.Andavam [bois, vacas e bezerros].` |  |
| **4. Sujeito Indeterminado** | **SIM** (alguém) | **(I) 3ª P. Plural****(II) 3ª P. Sing. + SE****(III) Infinitivo Impessoal** | O agente existe, mas a estrutura**não identifica quem é** (sem referente prévio, com IIS ou infinitivo). | `Chegaram cedo.Precisa-se de vendedores.É bom estudar.` |  |
| **5. Sujeito Inexistente (Oração Sem Sujeito)** | **NÃO** | **(I) Haver = Existir****(II) Fenômenos da Natureza****(III) Haver/Fazer = Tempo** | Construído com**Verbos Impessoais** (sem pessoa/agente). Verbo fixo na 3ª do singular. | `Há alunos aqui.Chove muito.` (literal)`Faz dois anos que estudo.` |  |
| **6. Sujeito Oracional** | **SIM** (`= ISSO`) | **(I) Oração Desenvolvida****(II) Oração Reduzida** | **É aquele que tem verbo**. Oração subordinada substantiva subjetiva. Verbo principal na 3ª sing. | `Basta [que você estude]. (= ISSO)Basta [estudar muito]. (= ISSO)` |  |
...
* **Passo 2:** Substituir todo o bloco oracional subordinado subsequente pela palavra **ISSO**.
* **Passo 3:** Reorganizar a oração na ordem direta mental: `ISSO + [verbo principal] + [predicativo/complemento]`.
* **Passo 4:** Formular a pergunta: *"O que é que [verbo principal]?"* $\rightarrow$ Resposta: *"ISSO"*.
* **Passo 5:** Inspecionar o interior do bloco substituído por *ISSO*. Se contiver um verbo próprio (conjugado ou no infinitivo), o sujeito está formalmente comprovado como **Sujeito Oracional**.
...
* **Passo 2 (Isolamento Oracional):** Isolar visualmente a oração pertencente ao `Verbo 1`, ignorando momentaneamente as orações vizinhas.
* **Passo 3 (Análise Autônoma):** Interrogar o `Verbo 1` e classificar o seu sujeito de forma independente.
* **Passo 4 (Transição Ordenada):** Avançar para o `Verbo 2`, traçar novos limites oracionais e classificar o seu respectivo sujeito sem contaminar a análise anterior.
...
### Tabela de Contrastes Fundamentais na Sintaxe do Sujeito

| Estrutura Analisada | Classificação do Sujeito | Justificativa Sintático-Semântica Decisiva | Diferencial em Prova de Concurso |
|:-------------------------------------------------------- |:----------------------------------- |:------------------------------------------------------------------------------------------------------- |:-------------------------------------------------------------------------------- |
| *"Eles acordaram cedo."* | **Sujeito Simples** | O pronome*Eles* está expresso e grafado na oração como núcleo único. | Verbo na 3ª do plural com pronome explícito não é sujeito indeterminado. |
| *"Eles acordaram e saíram."* (análise de *saíram*) | **Sujeito Oculto (Elíptico)** | O verbo*saíram* não traz pronome ao lado, mas seu referente anafórico (*Eles*) está no contexto. | A existência de âncora textual prévia impede a indeterminação do sujeito. |
| *"Saíram cedo."* (frase isolada) | **Sujeito Indeterminado** | Verbo na 3ª pessoa do plural desprovido de sujeito expresso e sem referente no texto. | Factualmente alguém saiu, mas a sintaxe não revela quem é. |
| *"Alguém acordou cedo."* | **Sujeito Simples** | O termo*Alguém* está grafado na frase como núcleo único (pronome indefinido substantivo). | O pronome é indefinido na morfologia; o sujeito é simples na sintaxe. |
| *"Chove muito na cidade."* | **Sujeito Inexistente** | Verbo de fenômeno meteorológico empregado em seu sentido literal e denotativo. | Verbo impessoal; ausência ontológica e gramatical de agente. |
| *"Choveram elogios para ele."* | **Sujeito Simples** | Verbo meteorológico em sentido figurado/conotativo (significa*ocorrer em abundância*). | O termo*elogios* é o núcleo do sujeito simples; o verbo flexiona no plural. |
| *"Houve duas rebeliões."* | **Sujeito Inexistente** | Verbo*haver* com sentido de existir/ocorrer é impessoal; *duas rebeliões* = Objeto Direto. | Invariável no singular; erro grave tentar pluralizar (*"houveram"*). |
| *"Ocorreram duas rebeliões."* | **Sujeito Simples** | Verbo*ocorrer* é verbo pessoal pleno; *duas rebeliões* = Sujeito Simples posposto. | Flexiona compulsoriamente no plural para concordar com o núcleo*rebeliões*. |
| *"Alugam-se casas."* | **Sujeito Simples (Paciente)** | Verbo transitivo direto com*se* PA; equivale a *"Casas são alugadas"*. | Há sujeito expresso (*casas*); o verbo concorda obrigatoriamente no plural. |
| *"Precisa-se de vendedores."* | **Sujeito Indeterminado** | Verbo transitivo indireto com*se* IIS; o termo *de vendedores* é objeto indireto. | O termo é preposicionado e não pode ser sujeito; o verbo fica fixo no singular. |
...
* **Classificação:** **Sujeito Simples** (com flexão obrigatória no plural).
...
* **Julgamento e Justificativa:** O verbo *haver* está empregado no sentido de *existir*, operando como verbo impessoal sem sujeito gramatical.
* **Gabarito:** **CERTO**.
...
* **Julgamento e Justificativa:** A oração subordinada reduzida *analisar os impactos...* funciona como sujeito oracional (*"Isso é imprescindível"*). Todo sujeito oracional impõe ao verbo regente a flexão na 3ª pessoa do singular.
* **Gabarito:** **CERTO**.
...
* **Descrição do atalho:** Memorize o mantra operacional da disciplina:
 ...
 > Se o bloco identificado como sujeito contém um verbo em seu interior (desenvolvido ou reduzido no infinitivo), a classificação sintática é compulsoriamente **Oracional**, independentemente da presença de conjunção integrante.
 >
...
Ao concluir o estudo deste grupo temático consolidado, o estudante adquire as seguintes competências operacionais:
