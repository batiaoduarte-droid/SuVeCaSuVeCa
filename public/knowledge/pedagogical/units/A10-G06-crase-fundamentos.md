...
**Método central:** A crase é verificada depois do mapa: primeiro confirme a exigência da preposição a pelo regente; depois teste se o termo seguinte admite artigo a.
...
│
└── 3. Introdução à Crase Proibida
 └── Princípio: "Um 'A' sozinho não faz fenômeno" (ausência de artigo antes de verbos, masculinos, etc.)
```

---
```text
CRASE: FENÔMENO FONÉTICO (Preposição 'A' + Artigo 'A' / Pronome 'Aquele'/'A qual')
 ├── Axioma Fundamental: "Um 'A' sozinho não faz fenômeno"
 │
 ├── CASOS DE CRASE PROIBIDA
 │ ├── GRUPO 1: Proibição por ausência/rejeição do ARTIGO DEFINIDO FEMININO (A = apenas preposição)
 │ │ ├── 1. Antes de palavras masculinas (ex.: "igual a Camões", "a pé")
 │ │ ├── 2. Antes de verbos no infinitivo (ex.: "disposto a lutar")
 │ │ ├── 3. Antes de pronomes indefinidos (ex.: "corresponder a qualquer realidade")
 │ │ ├── 4. Antes de pronomes demonstrativos (este/esta, esse/essa) (ex.: "referiu-se a este livro")
 │ │ ├── 5. Antes de artigo indefinido (ex.: "candidatei-me a um cargo")
 │ │ ├── 7. Nomes de lugar que não admitem artigo (ex.: "fui a São Paulo" -> "voltei de")
 │ │ │ └── Exceção: Topônimo com determinante semântico (ex.: "à Londres dos meus sonhos")
 │ │ └── 8. Paralelismo no uso do artigo / Palavras repetidas / Expressões simétricas
 │ │ ├── Palavras repetidas (ex.: "cara a cara", "frente a frente" -> sem artigo)
 │ │ └── Expressões temporais/estruturais simétricas (ex.: "de segunda a sexta" vs. "da terça à sexta")
 │ │
 │ └── GRUPO 2: Proibição por rejeição da PREPOSIÇÃO 'A' (A = apenas artigo)
 │ └── 6. Após preposição essencial (ex.: "desde as oito horas", "ante a ausência", "para a festa", "após a aula")
 │ └── Única exceção: Preposição ATÉ (admite preposição 'a' posposta -> crase facultativa)
 │
 └── MÉTODO RESOLUTIVO EM 2 PASSOS (APLICAÇÃO PRÁTICA EM 10 ITENS)
 ├── Passo 1: Verificar regência do termo anterior (exige preposição 'a'?)
 └── Passo 2: Verificar aceitação de artigo definido pelo termo posterior (admite 'a/as'?)
```

---
```text
CRASE (Duas Grandes Regras)
 ├── I. FENÔMENO FONÉTICO (Fusão da Preposição 'a' + 'a' inicial) [Foco deste material]
 │ ├── 1. Preposição 'A' + Artigo Definido 'A(S)' (~80% dos casos de prova)
 │ │ ├── Passo 1: Termo Anterior exige Preposição 'A'?
 │ │ │ └── Teste Prático: Substituir por masculino ("menino" / "clube") -> se der "AO", exige preposição.
 │ │ └── Passo 2: Termo Posterior aceita Artigo 'A'?
 │ │ ├── Substantivos Comuns: Iniciar frase com a palavra em função de sujeito (ex.: "A terra é boa").
 │ │ └── Nomes de Lugar (Topônimos): Mnemônico do verbo 'voltar'
 │ │ ├── "Volta DA" -> Tem artigo -> CRASE ("Voltei da Itália" -> "Retornou à Itália")
 │ │ └── "Volta DE" -> Sem artigo -> SEM CRASE ("Voltei de Brasília" -> "Retornou a Brasília")
 │ ├── 2. Preposição 'A' + Pronomes Demonstrativos 'aquele(s)', 'aquela(s)', 'aquilo'
 │ ├── 3. Preposição 'A' + Pronome Relativo 'a qual', 'as quais'
 │ └── 4. Preposição 'A' + Pronome Demonstrativo 'A' (antes de 'que' e 'de')
 └── II. LOCUÇÃO FEMININA (Adverbiais, Prepositivas, Conjuntivas) [Anunciada para blocos seguintes]
```
...
│
└── 5. CASO ESPECIAL: PRONOMES INDEFINIDOS NO PLURAL (ex: 'várias')
 ├── Repelem o artigo definido
 └── Admitem unicamente a preposição simples 'a' (proibido 'à' e 'às')
```
...
A abordagem didática enfatiza que "crase não é acento" e faz o aluno repetir: "Crase não é acento. É resultado do fenômeno fonético que é a fusão, a fusão que culmina no acento grave indicativo de crase".

esquemas de abertura com o título **FENÔMENO FONÉTICO** e anotações manuscritas progressivas: `(= fusão)`, `acento grave indicativo de crase`.
...
- Sua ocorrência depende rigorosamente de dois requisitos simultâneos e complementares:
 ...
 2. **Presença de artigo definido feminino `a(s)`:** O termo seguinte precisa ser uma palavra feminina que admita a anteposição do artigo definido.
- Fórmula estrutural: $\text{Preposição } a + \text{Artigo } a(s) = \text{à(s)}$.

A abordagem didática explica: "O primeiro caso de fenômeno fonético é o mais comum [...] ele incide em 80% dos casos de concurso [...] que é o fenômeno da preposição com artigo".
...
Anotação no esquemas: `2) Com nomes de lugar: use VOLTAR -> Volta DA (CRASE NO 'A') / Volta DE (SEM ARTIGO: SEM CRASE)`.
Exemplos escritos:
...
"Dá muito nervoso no aluno o aquele, crase no aquele [...] porque a gente decorou que masculino não aceita crase [...] Mas é porque o aquele já começa com A. Se ele já começa com A, mais o A preposição, crase."

esquemas com esquema da caixa `PREPOSIÇÃO + PRONOMES DEMONSTRATIVOS` conectada a `Dirigiu-se àquelas pessoas em tom emocionado`.
...
- **Método de substituição:** Para testar a ocorrência dessa crase, substitui-se o `a` antes de `que`/`de` por `aquela`. Se a frase mantiver sentido perfeito com "a aquela", a crase está plenamente justificada.

"Esse 2B poucos falam, poucos livros, mas cai de vez em quando numa questões ferradas, sabe, de concurso alto nível [...] O 2B não é difícil não, é só você ver que existe um aquele/aquela implícito antes de que e de [...] É preposição A mais o A pronome demonstrativo."

esquemas anotado: `2.b) Prep. A + a (demonstrativo implícito, ANTES DE QUE/DE) (= aquela)`.
Exemplos analisados no quadro:
- `Vestia uma camisa igual à do irmão` ($a \text{ [prep.]} + a \text{ [demonstrativo = aquela]} \rightarrow \text{à}$).
- `Referiu-se à que saiu` ($a \text{ [prep.]} + a \text{ [demonstrativo = aquela]} \rightarrow \text{à}$).
- `Obedeceu à que estava de blusa azul` ($a \text{ [prep.]} + a \text{ [demonstrativo = aquela]} \rightarrow \text{à}$).
...
esquemas com a caixa `PREPOSIÇÃO + PRONOME RELATIVO A QUAL / AS QUAIS` e o exemplo `Essa é uma questão à qual o governo deveria dar mais atenção`.
...
> *"Aprendemos que o fenômeno fonético é a contração da preposição A com, normalmente, um artigo... Se o A estiver sozinho, ou seja, se ele for só artigo ou só preposição... um A sozinho não faz fenômeno."*

A abordagem didática anota no esquemas de abertura:
$$\text{"Um } \underline{\text{A}} \text{ sozinho não faz fenômeno" (FF: } \text{A} + \text{A)}$$
E classifica no topo: `* só preposição / artigo`.
...
3. **Antes de pronomes indefinidos:** Pronomes de sentido vago/indefinido como *qualquer*, *alguém*, *ninguém*, *tudo*, *cada* não aceitam artigo definido (ex.: *corresponder a qualquer realidade*).
4. **Antes de pronomes demonstrativos (este, esta, esse, essa, isto, isso):** Não aceitam artigo anteposto no padrão culto (ex.: *referiu-se a este livro*). Não se diz *"a essa blusa é bonita"*.
5. **Antes de artigo indefinido (um, uma):** Dois determinantes de naturezas opostas (definido e indefinido) não se justapõem (*a + uma* não existe no padrão regular como fusão craseada).
6. **Antes de nomes de lugar que não aceitam artigo:** Cidades e estados que se combinam com preposição simples *de* (ex.: *fui a São Paulo* $\rightarrow$ *voltei de São Paulo*).

> *"Por que que eu não uso crase antes de palavras masculinas? Porque palavras masculinas não aceitam artigo... Verbo não aceita artigo... Qualquer é um pronome indefinido que não aceita artigo... Esses pronomes demonstrativos não aceitam artigo."*

esquemas com a lista de proibições anotado com a explicação em tinta vermelha: `A crase é proibida porque o ARTIGO é proibido (A é só preposição)`.
...
```text
Paralelismo do Uso do Artigo:
[SEM ART. + SEM ART.] = Eles se encontraram frente a frente. (frente = sem art. / a = só prep. / frente = sem art.)
[SEM ART. + SEM ART.] = A aula será de segunda a sexta. (de = só prep. / a = só prep.)
[COM ART. + COM ART.] = A aula será da terça à sexta. (da = de + a / à = a + a)
```
...
Anotação em destaque no esquemas `#ANOTAAÍ`:
$$\text{MANTRA: "A única preposição que aceita outra depois dela é a prep. ATÉ"}$$
Com listagem das preposições essenciais (`a, ante, até, após, com, contra, de, desde, para, perante...`) e a anotação: `Caso 6: O acento é proibido porque a PREPOSIÇÃO é proibida`.
...
Anotação manuscrita comparativa ao lado do item 8 dos exercícios:
$$\text{* foram à Londres dos meus sonhos} \rightarrow \text{Voltei DA Londres dos meus sonhos (com det. } \rightarrow \text{ com art.)}$$
...
A abordagem didática pontua: *"só existem duas regras de crase: o fenômeno fonético e a locução feminina."*.

Escrita manuscrita no topo do esquemas `#ANOTAAÍ`:
- `CRASE -> DUAS REGRAS`
- `(I) FENÔMENO FONÉTICO`
- `(II) LOCUÇÃO FEMININA`
...
Havendo a junção `a (prep.) + a (art.)`, resulta a forma grafada com o acento grave indicador de crase: `à` ou `às`.

*"80% dos casos de prova estão aqui, nesta primeira situação: preposição A mais artigo A."*.

Inscrição manuscrita no canto superior direito e no ramo (1) do esquema:
- `* 80% dos casos`
- `(1) PREP. A + ARTIGO A`
...
*"Como fazer isso? A gramática diz que você deve (...) começar uma frase com a palavra na função de sujeito e colocar artigo antes. (...) Por exemplo, quero saber se terra aceita artigo: A terra é boa. Pronto."*.

Anotação no esquema:
- `Como saber se o termo seguinte aceita artigo A?`
- `COMECE UMA FRASE COM A PALAVRA na função de sujeito e coloque ARTIGO antes.`
- Exemplo: `A terra é boa. -> L ARTIGO`
...
Anotações no esquemas:
- `NOME DE LUGAR -> VOLTAR`
- `VOLTA DA = Artigo (ex.: VOLTEI DA ITÁLIA -> à Itália)`
- `VOLTA DE = Sem artigo (ex.: VOLTEI DE BRASÍLIA -> a Brasília)`
...
3. **Preposição `A` + Pronome Relativo `A QUAL` / `AS QUAIS`:** Fusão da preposição regida pelo verbo da oração adjetiva com o artigo `a` que integra o pronome relativo (*à qual*, *às quais*).
4. **Preposição `A` + Pronome Demonstrativo `A` (antes de *que* ou *de*):** Fusão da preposição com o pronome demonstrativo `a` (equivalente a *aquela* ou *aquela coisa*), gerando *à que* ou *à de*.

*"O fenômeno fonético (...) vai ser o encontro da preposição A: com o artigo, que são 80% dos casos; (...) com o aquela, o A de aquele e o A de aquilo (...); com o Azinho do a qual ou as quais (...); e com o pronome demonstrativo a antes de que e de."*.

Ramos preenchidos no esquema da lousa digital:
- `(1) PREP. A + ARTIGO A (antigo 80%)`
- `(2) PREP. A + Aquela, Aquele, Aquilo`
- `(3) PREP. A + A qual / As quais`
...
A abordagem didática detalha:
> *"Antes de plural, é muito comum o artigo ser facultativo. Por quê? Porque muitas vezes, quando eu uso o artigo, eu uso porque eu quero especificar. E quando eu não uso, eu uso porque eu quero generalizar... Se de um lado eu tenho um verbo que pede preposição (...) do outro eu tenho o artigo 'as': 'a' preposição mais 'as' artigo, acento no 'às'. Nesse caso, estou dizendo: 'obedecia a ordens específicas'. Mas eu também posso dizer 'obedecia a ordens' sem artigo... Nesse caso, esse 'a' seria só preposição, com sentido geral de quaisquer ordens."*

O esquemas estruturado exibe sob o título "OPÇÕES CORRETAS":
...
3. **Coerência Textual (Lógica do texto):** **Mantém-se**. Coerência diz respeito à logicidade e não contradição do discurso. Uma frase que passa de sentido específico para sentido geral continua fazendo sentido e mantendo harmonia lógica.

> *"A troca de uma forma pela outra: Mantém a correção gramatical? Sim, as duas são corretas. Mantém o sentido? Não, altera o sentido (semântica). Num caso estou especificando, no outro estou dizendo quaisquer ordens. E se a banca me pergunta da coerência? Mantém a coerência! Coerência é lógica. Alterar coerência é ficar sem sentido, incoerente. E nos dois casos a coerência é mantida."*

Anotação manuscrita esquemática elaborada no esquemas:
`text
(*) DICA:
A troca de uma forma pela outra:

```text

---

## Estruturas Incorretas Diante de Palavras no Plural

### Definição ou ideia central
Existem duas configurações estruturais que violam a norma culta ao se tentar formular frases com termos regentes preposicionados diante de substantivos femininos no plural: a crase no singular (`à`) e o artigo feminino plural isolado (`as` sem acento).

### Explicação detalhada
1. **Erro 1: Crase no singular antes de plural (`à + substantivo plural`):**
 - *Exemplo incorreto:* `Obedecia à ordens.`
 - *Causa do erro:* A crase singular pressupõe `a (preposição) + a (artigo singular)`. Não é gramaticalmente possível associar um artigo feminino singular `a` a um substantivo plural `ordens`. Diante de plural, a crase só pode ocorrer se estiver no plural (`às`).
 - *Mnemônico canônico:* "Crase no singular antes de plural, nem a pau!"
2. **Erro 2: Apenas artigo plural sem preposição (`as + substantivo plural`):**
 - *Exemplo incorreto:* `Obedecia as ordens.`
 - *Causa do erro:* Se o verbo ou nome regente exige a preposição `a`, o vocábulo `as` sem acento representa unicamente o artigo definido. A ausência da preposição configura grave **erro de regência**.

> *"O que eu não posso antes do plural? Não posso duas coisas: Primeiro erro é usar crase no singular antes do plural... Daí aquele: antes do plural, crase no singular, nem a pau! (...) E o segundo erro, que cai muito em prova: obedecer as ordens. Esse 'as' sem acento é só artigo. Quando eu uso só o 'as', significa que faltou a preposição. Erro de regência!"*

O esquemas de "OPÇÕES INCORRETAS" exibe:
- `1. Obedecia à ordens.` $\rightarrow$ Anotado: `CRASE NO SINGULAR ANTES DO PLURAL! (ERRO!)`.
- `2. Obedecia as ordens.` $\rightarrow$ Anotado: `SÓ ARTIGO AS! (ERRO DE REGÊNCIA: FALTOU A PREPOSIÇÃO!)`.

---

## Crase Diante de Pronomes Indefinidos Plurais ('Várias')

### Definição ou ideia central
Palavras com sentido intrinsecamente indefinido, como o pronome indefinido `várias`, repelem o artigo definido `as`, inviabilizando a crase mesmo quando o termo regente exige a preposição `a`.

### Explicação detalhada
Ao aplicar o teste de aceitação de artigo (iniciar uma oração com o termo como sujeito: `Várias barragens são boas`), constata-se que a inserção do artigo definido feminino plural gera agramaticalidade (`*As várias barragens são boas` é semanticamente inadequado porque o artigo define o que o pronome indefiniu).
Portanto:

- A única forma correta é o emprego da **preposição simples `a`** (`alude a várias barragens`).
- O emprego de `à várias` constitui duplo erro: crase no singular antes de plural e crase antes de pronome indefinido.
- O emprego de `às várias` também é incorreto por incompatibilidade entre o artigo definido e o pronome indefinido.

> *"Várias barragens é indefinido. Se eu coloco o artigo 'as várias barragens são boas', não fará sentido, a frase ficará errada, porque estou falando de várias. Isso é um pronome que generaliza. Se eu coloco o artigo definido, estou definindo algo que não pode ser definido... É por isso que o certo seria sem artigo: 'o relatório alude a várias barragens', só preposição."*

Resolução manuscrita no esquemas do item 4:
- Identificação da regência: `alude a` $\rightarrow$ `ao menino` (pede preposição `A`).
- Teste de sujeito no canto superior: `Várias barragens são boas.` (rejeição de `*As várias...`).
- Correção explícita grafada: `alude a várias barragens` (rotulado como `SÓ PREP.`).
...
[Início: Frase com termo regente + 'a'/'à'/'às'/'as' + substantivo plural]
 │
 ▼
 Passo 1: Verificar a Regência do Termo Anterior
 (Substituir mentalmente o termo regido por substantivo masculino singular,
 ex: "ao menino" ou "ao regulamento")
 │
 ┌────────────────┴────────────────┐
 ▼ ▼
 Exige Prep. 'A' NÃO Exige Prep. 'A'
 │ │
 ▼ ▼
Passo 2: Analisar a Grafia Crase Proibida!
do 'A' Diante do Plural
 │
 ┌──────┼────────────────────────┬────────────────────────┐
 ▼ ▼ ▼ ▼
"às" "a" "à" "as"
 │ │ │ │
 ▼ ▼ ▼ ▼
[A+AS] [A + Ø] [ERRO 1] [ERRO 2]
CERTO CERTO ERRADO ERRADO
(Sent. (Sent. (Crase singular (Só artigo;
Espec.) Geral) antes de plural) faltou prep.)
```
...
- **Resposta final:** `a` (sem acento indicativo de crase). Justificativa visual: circular `homens` (masculino).
...
- **Resposta final:** `à` (com acento grave).
...
- **Resposta final:** `a` (sem acento indicativo de crase). Justificativa visual: circular `fazer` (verbo).
...
- **Resposta final:** `a` (sem acento indicativo de crase). Justificativa visual: circular `uma` (artigo indefinido).
...
- **Contraponto pedagógico demonstrado:** Se houvesse determinante (*Os cientistas foram _____ Londres dos meus sonhos*), o retorno seria *Voltei da Londres dos meus sonhos*, resultando em crase obrigatória (*à Londres dos meus sonhos*).
- **Resposta final:** `a` (sem acento indicativo de crase). Justificativa visual: circular `Londres` (lugar sem artigo).
...
- **Desenvolvimento do raciocínio:**
 1. *Regência verbal:* Quem dedica, dedica algo a alguém (`Dedicou sua vida ao menino`). Exige preposição `a`.
 2. *Termo seguinte:* `crianças abandonadas` é substantivo feminino no plural.
 3. *Estrutura grafada:* Utilizou-se `a` no singular e sem acento grave.
 4. *Classificação morfológica:* O `a` é exclusivamente preposição simples. O artigo `as` foi suprimido para imprimir sentido genérico (quaisquer crianças abandonadas).
- **Conceito demonstrado:** Emprego lícito de preposição simples antes de substantivo plural com valor semântico indeterminado.
...
- **Gabarito / Julgamento:** **CERTO**
- **Desenvolvimento do raciocínio:**
 1. *Regência nominal:* O adjetivo `favorável` rege complemento nominal introduzido pela preposição `a` (`favorável ao menino / ao projeto`).
 2. *Termo seguinte:* `propostas do diretor` aceita artigo definido `as` (`As propostas do diretor são boas`).
 3. *Fusão:* Preposição `a` + artigo `as` = `às`.
 4. *Valor semântico:* Trata-se de propostas específicas, determinadas pelo adjunto adnominal `do diretor`.
- **Conceito demonstrado:** Crase no plural decorrente de regência nominal com sentido especificado.
...
- **Desenvolvimento do raciocínio:**
 1. *Regência verbal:* O verbo pronominal `referir-se` rege preposição `a` (`refere-se ao menino`).
 2. *Estrutura grafada:* Grafou-se `as` sem acento indicativo de crase.
 3. *Identificação do erro:* O vocábulo `as` sem acento é mero artigo definido. Omitiu-se a preposição obrigatória exigida pelo verbo `referir-se`, gerando erro de regência.
 4. *Correções possíveis:*

- **Conceito demonstrado:** Inadmissibilidade do artigo plural desacompanhado de preposição quando o regente exige a preposição `a`.
...
- **Gabarito / Julgamento:** **ERRADO**
- **Desenvolvimento do raciocínio:**
 1. *Regência verbal:* O verbo `aludir` rege preposição `a` (`alude ao menino / ao fato`).
 2. *Termo seguinte:* `várias barragens` é encabeçado pelo pronome indefinido `várias`.
 3. *Erro 1 (Morfossintático):* Uso de crase no singular (`à`) antes de palavra no plural (`barragens`).
 4. *Erro 2 (Semântico):* Pronomes indefinidos repelem o artigo definido. O termo `várias barragens` não admite o artigo `as`.
 5. *Forma correta única:* `O relatório alude a várias barragens que estão vulneráveis.` (apenas a preposição `a`).
- **Conceito demonstrado:** Proibição de crase diante de pronomes indefinidos no plural.
...
- **Gabarito / Julgamento:** **CERTO**
- **Desenvolvimento do raciocínio:**
 1. *Análise sintática do regente:* O termo que rege `às terras` é o substantivo `direito` (e não o verbo `reconhece`, que rege `o direito` como objeto direto e `aos povos indígenas` como objeto indireto).
 2. *Regência nominal:* O nome `direito` rege complemento nominal introduzido por preposição `a` (`direito ao usufruto / ao benefício`).
 3. *Termo regido:* `terras que habitam` aceita o artigo `as` (`As terras são boas`).
 4. *Fusão:* Preposição `a` + artigo `as` = `às`.
- **Conceito demonstrado:** Identificação correta do termo regente nominal em períodos complexos e validação da crase no plural.
...
- **Gabarito / Julgamento:** **ERRADO**
- **Desenvolvimento do raciocínio:**
 1. *Regência verbal:* O verbo `ir` exige preposição `a` (`ir ao clube / ao cinema`).
 2. *Termo seguinte:* `festas` está no plural.
 3. *Identificação do erro:* Acentuou-se o `a` no singular (`à`) antes de um substantivo plural (`festas`), violando a regra de fusão fonética.
 4. *Correções possíveis:*

- **Conceito demonstrado:** Aplicação do princípio "crase no singular antes de plural, nem a pau".
...
- **Gabarito / Julgamento:** **CERTO**
- **Desenvolvimento do raciocínio:**
 1. *Regência verbal:* O verbo `submeter-se` exige preposição `a` (`submeto-me ao regulamento / ao diretor`).
 2. *Termo seguinte:* `ordens desse tipo` está no plural.
 3. *Estrutura grafada:* `a` singular e desprovido de acento grave.
 4. *Análise morfológica:* O termo `a` é exclusivamente preposição, sem artigo definido. A intenção é generalizar o sentido (não se submeter a quaisquer ordens daquele tipo).
- **Conceito demonstrado:** Validade do emprego de preposição simples singular antes de substantivo plural.
...
- **Gabarito / Julgamento:** **ERRADO**
- **Desenvolvimento do raciocínio:**
 1. *Regência nominal:* O adjetivo `atentos` exige preposição `a` (`atentos ao menino / ao chamado`).
 2. *Estrutura grafada:* Grafou-se `as` sem acento indicativo de crase.
...
- Exemplo: *Vou a Curitiba* $\rightarrow$ Volto *de* Curitiba (sem crase).
- Exemplo: *Vou a França* $\rightarrow$ Volto *da* França (com crase: *Vou à França*).
...
- Diante de demonstrativo: *"A essa blusa é minha"* $\rightarrow$ O `a` soa como interjeição de espanto (*Ah!*), e não artigo. Logo, *essa* rejeita artigo.
...
2. **Axioma das Preposições:** *"A única preposição que aceita outra depois dela é a preposição ATÉ"* (Após *desde*, *para*, *ante*, *após*, crase é proibida).
...
| Proposição da banca sobre trocar `às [pl.]` por `a [pl.]` | Julgamento | Justificativa |
|:--- |:--- |:--- |
| "A substituição preserva a correção gramatical do texto." | **CORRETO** | Ambas as estruturas são gramaticais. |
| "A substituição mantém o sentido original do texto." | **INCORRETO** | Altera de específico para geral. |
| "A substituição altera o sentido / relações semânticas." | **CORRETO** | Específico $\neq$ Geral. |
| "A substituição preserva a coerência do texto." | **CORRETO** | O texto mantém sua logicidade e não se torna contraditório. |
| "A substituição prejudica a coerência textual." | **INCORRETO** | O texto não perde o sentido lógico. |
...
- **Correção:** Entender que *acento grave* é o sinal gráfico, e *crase* é o processo fonético de contração.
...
- **Correção:** Os pronomes *aquele*, *aquela* e *aquilo* admitem acento grave em sua vogal inicial porque a fusão ocorre com a letra `A` do próprio pronome demonstrativo, independentemente de seu gênero gramatical.
...
- **Forma correta:** *Para a festa...*, *Ante a ausência...*, *Após a aula...* (sem acento grave).
...
- **Forma correta de pensar:** Coerência é harmonia lógica. Uma frase continua perfeitamente coerente ao se referir a uma categoria em geral (`a festas`), mesmo que antes se referisse a eventos determinados (`às festas`).
...
- **Consequência:** Não identificar o erro de regência verbal/nominal.
...
Neste material inicial sobre o emprego do acento indicativo de crase, A abordagem didática desconstrói o modelo tradicional de ensino baseado na memorização de extensas listas desconexas e estrutura a matéria sobre duas regras centrais. O foco integral da aula recai sobre a **Regra 1: O Fenômeno Fonético (FF)**, que se define pela fusão física e sonora de duas vogais `a` ($a + a = à$).
...
---
O conteúdo integrado da Aula 10 encerra a análise teórica da crase fundamentada no conceito de fenômeno fonético (fusão de preposição `a` com artigo definido feminino `a` ou pronomes específicos).

A abordagem didática demonstra que as situações clássicas de crase proibida não devem ser decoradas como uma lista desconexa, mas compreendidas sob duas categorias:
1. **Proibição por inexistência de artigo:** Casos 1 (palavras masculinas), 2 (verbos), 3 (pronomes indefinidos), 4 (pronomes demonstrativos *este/esse*), 5 (artigo indefinido), 7 (topônimos que repelem artigo) e 8 (palavras repetidas/paralelismo). Em todos eles, o vocábulo regido não aceita determinante feminino, restando apenas a preposição `a`.
2. **Proibição por inexistência de preposição `a`:** Caso 6 (após outras preposições essenciais como *desde*, *ante*, *para*, *após*). Como não se acumulam preposições, o `a` existente é apenas artigo definido. A única preposição que admite outra após si é *até* (gerando crase facultativa).

A aula estabelece ainda o princípio do paralelismo sintático para expressões simétricas e a regra de especificação de topônimos por determinantes semânticos. Por fim, o método em dois passos é consolidado através da resolução analítica de 10 exercícios de fixação, nos quais os alunos são guiados a circular visualmente os termos impeditivos de artigo.

---
nesta unidade, A abordagem didática apresenta uma revisão estrutural definitiva sobre o **Fenômeno Fonético da Crase**, responsável por cerca de 80% das incidências em provas.
...
---
O estudo da crase diante de termos no plural assenta-se na compreensão da natureza facultativa do artigo definido feminino plural `as`. Quando o termo regente (seja ele um verbo como *obedecer*, *dedicar*, *aludir*, *referir-se*, seja um nome como *favorável*, *direito*, *atento*) exige a preposição `a`, o candidato tem à sua disposição duas — e apenas duas — construções gramaticalmente irrepreensíveis:

1. **`Termo Regente + às + Substantivo Plural`:** Resulta da soma da preposição `a` com o artigo `as`. Emprega-se quando o substantivo possui sentido restrito, específico ou determinado.
2. **`Termo Regente + a + Substantivo Plural`:** Resulta do emprego exclusivo da preposição `a`, sem artigo. Emprega-se quando o substantivo é tomado em sentido amplo, genérico ou indeterminado (equivalendo a *quaisquer*).

Por outro lado, constituem erros gravíssimos em concursos:
- **Usar `à` no singular antes de plural** (ex: `*obedecia à ordens`), por violação direta da concordância e da fusão fonética;
- **Usar `as` sem acento grave** (ex: `*obedecia as ordens`), por configurar erro de regência pela omissão da preposição obrigatória.

Em provas de reescritura, a substituição de `às [termo]` por `a [termo]` **mantém a correção gramatical**, **mantém a coerência lógica**, mas **altera as relações semânticas (o sentido)**. Por fim, diante de pronomes que repelem o artigo definido, como o pronome indefinido *várias*, a crase é sempre proibida, sendo lícito unicamente o emprego da preposição pura `a`.
