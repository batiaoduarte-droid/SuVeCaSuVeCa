# Ponto e Vírgula Dois-Pontos e Aspas
...
**Conexão forte:** Ponto e vírgula e dois-pontos dependem da relação entre segmentos; aspas também envolvem citação, destaque e sentido, exigindo camada discursiva além do mapa.
...
- **Sintaxe do Período Composto:** Diferenciação nítida entre orações coordenadas (independentes) e orações subordinadas substantivas, adjetivas e adverbiais (dependentes).
- **Morfologia Verbal:** Reconhecimento obrigatório de verbos e locuções verbais para validar a existência de orações (frases verbais).
...
```text
 ...
 ┌───────┴────────┐ ┌───────┴────────┐ ┌───────┴────────┐
 ▼ ▼ ▼ ▼ ▼ ▼
PERMITIDO PROIBIDO FUNÇÃO TEXTUAL SINTAXE ESTRITA HIERARQUIA 4 CASOS DE USO
 • Enumerações • Subordinação • Citação / • Par terminal: • Duplas: Geral 1. Citação / Fala /
 • Coordenadas (6Cs + FTP) Discurso Só com ponto • Simples: Mudança de
 (sindéticas • Orações Direto final (:....) Apenas no Interlocutor
 ou assindé- reduzidas • Explicação / • Proibido interior de 2. Desvios Lexicais:
 ticas) causais Esclarecimento intercalação aspas duplas Estrangeirismo
 • Reescritura • Início com (Diz o mesmo; medial (VOLP), Neologismo,
 explicativa próclise não é ideia (:..., / Arcaísmo, Gíria
 (com coord.) (; te deixa) nova; "pois"):... —) 3. Destaque / Ênfase
 • Enumeração • Reescritura: (Ressalte)
 (Desdobra em Equivale à 4. Sentido Diverso
 2+ elementos) vírgula (Figurado / Rela-
 terminal tivizado) vs.
 Ironia (Setas)
```
...
- *Reescritura com valor adversativo:* `...ela é mínima. Agora, dizer...` $\rightarrow$ `...ela é mínima; agora, dizer...` (onde *Agora* equivale a *porém*).
...
1. Introduzir **citação textual / discurso direto**;
...
- **3. Enumeração:** Apresenta uma sequência coordenada de itens que desdobram formalmente um conjunto ou hiperônimo citado previamente.
...
$$
\begin{aligned}
\text{Axioma 1: } & \text{EXPLICAÇÃO} \neq \text{ENUMERAÇÃO} \\
\text{Axioma 2: } & \text{EXPLICAÇÃO} = \text{ESCLARECIMENTO} \\
\text{Axioma 3: } & \text{EXPLICAR} = \text{JUSTIFICAR} = \text{DIZER O MESMO} \neq \text{IDEIA NOVA}
\end{aligned}
$$
...
3. **Axioma 3 (Explicação $\neq$ Ideia Nova / Progressividade Textual):** A explicação/justificativa pós-dois-pontos mantém-se estritamente circunscrita aos limites semânticos do antecedente. Ela **não avança na progressão temática**, não acrescenta evento novo independente e não amplia o universo temático; ela apenas reitera, elucida ou justifica o que já foi dito ("dizer o mesmo").
...
3. **Mudança de Interlocutor (Termo FGV/Vunesp):** Sinaliza a alternância da voz do autor principal para uma entidade discursiva externa.
4. **Distinção Crítica de Suporte:** *Citação de outro texto* exige suporte escrito prévio (artigo, livro); *Marcar a fala de alguém* aplica-se a declarações orais concedidas a repórteres.
...
$$
\begin{aligned}
\text{Polo Conotativo (Não Literal): } & \text{Sentido Figurado} = \text{Sentido Conotativo} = \text{Relativizar o Significado} = \text{Diverso do Usual} \\
\text{Polo Denotativo (Literal): } & \text{Sentido Real} = \text{Sentido Denotativo} = \text{Sentido Literal} = \text{Sentido do Dicionário}
\end{aligned}
$$
...
3. **O Método das Setas Contextuais:** A ironia não pode ser presumida. Para ser validada em prova, o texto deve conter uma pista textual explícita de polaridade contrária. Ao ligar a palavra aspeada ao seu referente por uma seta, se o referente contradisser o sentido literal (ex.: *"brilhante"* $\longrightarrow$ *"ao dizer aquela asneira"*), comprova-se a **ironia**. Se apenas houver transposição de sentido sem contradição, trata-se de **sentido figurado**.
...
| Sinal / Fenômeno | Regra Canônica Consolidada | Proibição / Limite Absoluto | ID Detalhado |
|:------------------------------------- |:----------------------------------------------------------------------------------------------------------------- |:--------------------------------------------------------------------------------------------------------- |:------------------------------- |
| **Ponto e Vírgula (`;`)** | Separa itens de enumeração e orações coordenadas (sindéticas ou assindéticas). | **Nunca** separa orações subordinadas (`6Cs + FTP`) ou estruturas reduzidas causais. |  |
| **Ponto e Vírgula + Próclise** | O ponto e vírgula gera pausa forte inicial de oração coordenada. | **Proibido** iniciar oração pós-ponto e vírgula com pronome oblíquo átono (próclise). |  |
| **Reescritura Explicativa** | Permutabilidade mútua entre vírgulas, parênteses, travessões, dois-pontos e ponto e vírgula (em coordenadas). | Nuances expressivas de ênfase (travessão) ou isolamento (parênteses) não alteram o sentido gramatical. |  |
| **Dois-Pontos (Função)** | Introduz citação textual/discurso direto, explicação/esclarecimento ou enumeração. | Explicação pós-dois-pontos**não** introduz ideia nova; enumeração exige $\ge 2$ elementos. |  |
| **Dois-Pontos (Estrutura)** | Exige encerramento obrigatório no ponto final do período (`:....`). | **Proibido** em intercalações mediais fechadas com vírgula ou travessão (`:,` ou `: —`). |  |
| **Aspas Simples (`' '`)** | Emprego exclusivo no interior de trechos já delimitados por aspas duplas. | Não se empregam aspas duplas dentro de aspas duplas nem aspas simples isoladas. |  |
| **Aspas em Citações** | Isolam citação direta, transcrição literal, fala oral e mudança de interlocutor. | Se a fala for reproduzida em paráfrase autoral (sem aspas), trata-se de explicação do autor. |  |
| **Aspas em Estrangeirismos** | Obrigatórias em vocábulos estrangeiros não aportuguesados pelo VOLP (ABL). | Dispensáveis em estrangeirismos já dicionarizados como substantivos/adjetivos vernáculos. |  |
| **Aspas de Destaque** | Conferem relevo tipográfico/metalinguístico em sentido literal. | Tríade de sinonímia de prova:*Destacar = Enfatizar = Ressaltar*. |  |
| **Aspas em Sentido Figurado** | Indicam sentido conotativo, desvio do usual e relativização semântica. | Não confundir com ironia. Só é ironia se houver termo contextual contradizendo o sentido literal. |  |
| **Conotativo vs. Coloquial** | Conotativo = desvio semântico/metáfora; Coloquial = inadequação de registro/informalidade. | Categorias mutuamente excludentes em itens de concurso. |  |
...
```text
 ...
 │
 ▼
 A oração subsequente é SUBORDINADA adverbial (6Cs + FTP)
 ou oração reduzida com valor causal/condicional?
 ├─► SIM ──► [ MARCAR ERRADO / PROIBIDO ]
 └─► NÃO
 │
 ▼
 A estrutura é uma ENUMERAÇÃO ou ORAÇÃO COORDENADA?
 ├─► NÃO ──► [ MARCAR ERRADO ]
 └─► SIM
 │
 ▼
 [ CHECAGEM DA TRAVA DE COLOCAÇÃO PRONOMINAL ]
 Ao retirar o conectivo, a oração após ';' inicia-se
 por pronome oblíquo átono (ex: '; te deixa')?
 ├─► SIM ──► [ MARCAR ERRADO (Erro de Próclise) ]
 └─► NÃO ──► [ MARCAR CERTO (Validação Concluída) ]
```
...
2. **Aplicar a fórmula do nexo causal:** Substitua mentalmente a construção pela estrutura:
 $$
 \text{"O fato de [Causa] faz com que [Efeito]"}
 $$
3. **Avaliar o resultado:** Se a frase fizer sentido pleno sob essa fórmula, comprova-se a subordinação adverbial causal. Logo, **o ponto e vírgula é absolutamente proibido**.
...
```text
 ...
 │
 ▼
 O trecho é reprodução da fala/texto de outrem?
 ├─► SIM ──► [ CITAÇÃO DIRETA / DISCURSO DIRETO ]
 └─► NÃO
 │
 ▼
 É estrangeirismo, neologismo, arcaísmo ou gíria?
 ├─► SIM ──► [ DESVIO LEXICAL / NEOLOGISMO / ESTRANGEIRISMO ]
 └─► NÃO
 │
 ▼
 ...
 │
 └─► Inexiste figura ou contradição, apenas realce do vocábulo?
 (Ex.: Diga-me "como" direi isso)
 └─► [ DESTAQUE / ÊNFASE / RESSALTE ]
```
...
### Tabela 8.1 — Ponto e Vírgula vs. Dois-Pontos vs. Travessão vs. Parênteses

| Sinal | Função Primária | Comportamento em Intercalação Medial | Comportamento em Final de Período | Reescritura Típica |
|:------------------------------------ |:----------------------------------------------------------------- |:--------------------------------------------------------------- |:---------------------------------------------------------------- |:------------------------------------------------------- |
| **Ponto e Vírgula (`;`)** | Pausa intermediária entre orações coordenadas e itens de lista. | Não se aplica (separa orações completas). | Válido se anteceder oração coordenada final. | Permuta com ponto final ou vírgula em coordenadas. |
| **Dois-Pontos (`:`)** | Anuncia citação, explicação ou enumeração. | **Proibido** (`:...,` e `:... —` são inválidos). | **Obrigatório** terminar no ponto final (`:....`). | Permuta com vírgula terminal ou travessão explicativo. |
| **Travessões (`—... —`)** | Isola elementos explicativos conferindo ênfase. | **Válido** em pares simétricos no meio do período. | Válido como travessão simples final (`... — explicação.`). | Permuta com parênteses, vírgulas ou dois-pontos. |
| **Parênteses (`(... )`)** | Isola comentários acessórios conferindo distanciamento. | **Válido** em pares simétricos no meio do período. | Válido isolando termo ao final do período. | Permuta com travessões ou vírgulas explicativas. |
...
|:-------------------------------------- |:------------------------------------------------------------ |:--------------------------------------------------------------- |:--------------------------------------------------------- |
| **Explicação / Esclarecimento** | Unidade de sentido que traduz ou justifica o antecedente. | Mantém o mesmo escopo temático (**não** é ideia nova). | Admite inserção da conjunção*"pois"*. |
| **Enumeração** | Lista coordenada de$\ge 2$ elementos específicos. | Desdobra as partes componentes de um hiperônimo. | Exige termos separados por vírgula/conectivo aditivo. |
| **Citação / Discurso Direto** | Transcrição literal das palavras de terceiro. | Introduz voz alheia (mudança de interlocutor). | Exige aspas (`"..."`) ou travessão + verbo *dicendi*. |
| **Ideia Nova (Progressão)** | Proposição inédita que faz a narrativa/argumento avançar. | Expande o universo factual do texto. | **Não** ocorre após dois-pontos explicativos. |
...
|:-------------------------------------- |:------------------------------------------------------- |:--------------------------------------------------------- |:------------------------------------------------- |
| **Ironia** | Dizer o oposto do que se pensa. | Pista contextual explícita de polaridade contrária. | *Você foi "brilhante" ao dizer aquela asneira.* |
| **Sentido Figurado (Conotativo)** | Transposição metafórica / relativização semântica. | Uso do vocábulo fora da acepção estrita do dicionário. | *Os juízes "traduzem" a fala dos réus.* |
| **Destaque / Ênfase** | Relevo tipográfico/metalinguístico literal. | Palavra usada em sentido denotativo real. | *Diga-me "como" direi isso a ela.* |
| **Linguagem Coloquial (Popular)** | Inadequação de registro sociolinguístico. | Termo informal da oralidade inserido no padrão culto. | *A "gente" é feliz.* |
...
- C) *O gerente informou...: "O valor mensal vai depender..."* (Citação/Fala).
- D) *Muitos itens já fizeram papel de dinheiro: o sal (...), a cachaça (...) e o bacalhau (...).* (CORRETA: o hiperônimo "Muitos itens" desdobra-se em 3 elementos).
- E) *O tabaco também já foi usado...: no século XVIII, o estado...* (Explicação).
- **Gabarito:** **Letra D**.
...
- **Correção da Intercalação:** *“Os jogos deste ano já produziram uma nova modalidade olímpica, apagamento de tocha, que provocou polêmicas, ontem.”* (ou entre travessões/parênteses).
...
- $\text{Citação Direta} = \text{Discurso Direto} = \text{Fala de Alguém} = \text{Mudança de Interlocutor}$
...
- ** — Ponto e Vírgula (`;`):** Sinal intermediário entre vírgula e ponto final, restrito a enumerações e orações coordenadas .
- ** — Dois-Pontos (`:`):** Sinal prospectivo que anuncia citação, explicação/esclarecimento ou enumeração, exigindo encerramento no ponto final .
- ** — Aspas Duplas (`" "`):** Sinal de pontuação duplo de padrão geral para citações diretas, desvios lexicais, ênfases e sentido figurado .
- ** — Aspas Simples (`' '`):** Sinal tipográfico auxiliar empregado exclusivamente no interior de segmentos já delimitados por aspas duplas .
- ** — Esclarecimento / Explicação:** Ato discursivo de justificar ou traduzir o referente antecedente mantendo a mesma substância conceitual ("dizer o mesmo"), sem avanço temático .
- ** — Enumeração:** Desdobramento de um termo hiperônimo em lista coordenada de dois ou mais itens hipônimos específicos .
- ** — Progressividade Textual (Ideia Nova):** Mecanismo discursivo que introduz informações ou eventos inéditos fazendo o texto avançar temática ou temporalmente .
- ** — Citação Direta / Transcrição:** Reprodução exata e literal de palavras de outro texto ou autor (*ipsissima verba*), marcada graficamente por aspas .
- ** — Mudança de Interlocutor:** Expressão técnica de bancas que identifica a transição da voz do autor principal para a voz de um terceiro citado entre aspas .
- ** — Relativização Semântica (Sentido Figurado / Conotativo):** Emprego de um vocábulo com significado subjetivo, atenuado ou metafórico fora de sua acepção literal do dicionário .
- ** — Sentido Denotativo (Literal / Real):** Significado básico, objetivo e dicionarizado da palavra .
...
Ao concluir este grupo temático, o estudante domina a arquitetura funcional dos sinais especiais de pontuação:

1. **Ponto e Vírgula:** Deve ser visualizado como uma barreira que conecta coordenadas ou organiza enumerações. Diante de qualquer nexo causal, condicional ou temporal (grupo `$6C_s + FTP$`), seu uso é vedado. Em reescrituras, a troca de conjunção aditiva por ponto e vírgula exige checagem obrigatória de ênclise para não incorrer em próclise inicial proibida.
2. **Dois-Pontos:** Operam em dois níveis: sintático (só abrem estruturas que terminam no ponto final, sendo nulas em intercalações mediais) e textual (subdividem-se em citação literal com aspas, explicação unívoca que diz o mesmo sem trazer ideia nova e enumeração que desdobra um hiperônimo em múltiplos componentes).
3. **Aspas:** Funcionam sob hierarquia estrita (simples dentro de duplas) e organizam-se em quatro gavetas conceituais: discurso alheio (mudança de interlocutor), desvios lexicais (estrangeirismos sob a regra do VOLP, neologismos, gírias), destaque tipográfico e desvio semântico.
4. **Resolução de Provas:** O estudante não cai na armadilha da "ironia universal", aplicando o método das setas para exigir contradição textual explícita, e distingue com segurança o registro popular/coloquial do sentido conotativo/relativizado.
