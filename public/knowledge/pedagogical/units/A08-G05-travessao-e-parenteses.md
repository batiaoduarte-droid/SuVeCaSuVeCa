...
- Reescrita com parênteses: Machado de Assis (grande romancista brasileiro) também escreveu contos.

- Expandido por:  (Regra de Cumulação de Pontuação).
...
1. Não ocorre duplicidade viciosa: cada sinal atende a um comando sintático distinto e autônomo.
...
- Uso Incorreto (Isolando objeto indireto): "...cadernos que pertencera (à minha avó Belmira)." $\rightarrow$ Viola a regência do verbo transitivo indireto pertencer.

- Operacionalizado por:  (Teste da Retirada).
...
3. Enumeração ≠ Exemplificação: A enumeração lista a totalidade dos elementos do conjunto (exaustão). A exemplificação cita amostras representativas de uma categoria mais ampla (relação de hiponímia, presença comum de "etc." ou conectivos exemplificativos).
...
| --- | --- | --- | --- |
| Explicação | Abstrato / Conceitual | Termo único ou oração com conector causal (pois) | Não (Diz o mesmo com outras palavras) |
| Enumeração | Concreto / Descritivo | Lista TODOS os componentes do conjunto | Não (Apenas lista as partes) |
...
- Exemplificação: Mídias sociais — Facebook, Twitter etc. — moldam a opinião pública.
- Comentário do autor: Nossa cabeça espia — se não naufragamos de vez —, assediada por pensamentos.
...
- Operacionalizado por:  (Eliminação por Sinonímia).
...
1. Ironia (Conceito Estrito): Figura de linguagem cuja definição técnica exige dizer rigorosamente o oposto do que se pensa. Não se confunde com crítica social direta, desabafo, zombaria ou inconformismo.

---

## Regras decisivas

| ID Canônico | Conceito / Regra | Formulação Sintética Canônica |
| --- | --- | --- |
|  | Ponto Final e Aspas | Citação é a frase inteira$\rightarrow$ Ponto DENTRO ("....."); Citação é parte da frase $\rightarrow$ Ponto FORA ("....".). |
|  | Citação Direta vs. Indireta | Citação Direta = Discurso Direto = Transcrição (Cópia fiel com aspas); Citação Indireta = Paráfrase (Sem aspas). |
|  | Reescritura com Duplo Travessão | [— X —] $\leftrightarrow$ [, X,] $\leftrightarrow$ [( X )] preservam sentido proposicional e correção; alteram apenas ênfase. |
|  | Sequência —... —, | Abertura com travessão e fechamento com vírgula (—...,) é ERRO; Travessão seguido de vírgula (—... —,) é CORRETO por cumulação sintática externa. |
|  | Isolamento Parentético | Parênteses isolam termos sintaticamente acessórios (secundários); não podem segmentar termos integrantes ou orações dependentes. |
|  | Tipologia Textual | Explicação (abstrato / diz o mesmo)$\neq$ Enumeração (todos os elementos) $\neq$ Exemplificação (amostra concreta / "etc.") $\neq$ Comentário (ideia nova opinativa). |
|  | Dicotomia FGV do Conteúdo | Explicação / Esclarecimento = "Diz o mesmo"; Ampliação / Nova Ideia = "Diz além" (soma dados ou dimensões temporais). |
|  | Função Resumitiva | Função Resumitiva = Função Recapitulativa (condensa elementos anteriores em palavra-síntese). |
|  | Conceito Estrito de Ironia | Ironia = Dizer tecnicamente o oposto do que se pensa. Crítica direta sem inversão de sentido NÃO é ironia. |
...
1. Passo 4 (Confirmação da Correção):
...
1. Passo 4 (Checagem Fato vs. Opinião): Se o conteúdo for científico/objetivo, elimine alternativas contendo modalizadores subjetivos (opinião, crítica, reflexão).
...
3. Teste 2 (Ampliação): O trecho contém conectivo aditivo (e), introduz um dado inédito ou soma uma nova perspectiva temporal (passado + presente)? Se SIM $\rightarrow$ Ampliação / Acréscimo de Nova Ideia.
...
| Sinal Gráfico | Estrutura | Grau de Ênfase / Relevo | Função Discursiva | Impacto no Sentido Proposicional |
| --- | --- | --- | --- | --- |
| Travessões Duplos | — X — | Máxima / Destaque | Põe a informação em evidência / Chama atenção visual do leitor. | Mantido (Sentido inalterado) |
...
| --- | --- | --- | --- |
| Explicação / Aposto | Reformulação conceitual no plano abstrato. | Diz o mesmo com outras palavras; sem ideia nova. | A inflação — aumento de preços — corrói a renda. |
| Enumeração | Listagem de todos os componentes da totalidade. | Desdobramento exaustivo das partes de um todo. | As estações — primavera, verão, outono e inverno. |
| Exemplificação | Apresentação de instâncias concretas não exaustivas. | Transita do abstrato para amostras práticas (etc.). | Redes sociais — Facebook, Instagram etc. |
...
| --- | --- | --- | --- |
| Retificação vs. Ratificação | Retificação: Corrigir erro prévio. | Ratificação: Confirmar / reiterar o mesmo. | Retificar anula o termo anterior; ratificar valida e repete o que foi dito. |
| Ainda (Tempo vs. Intensidade) | Tempo: Continuidade no presente (agora, até hoje). | Intensificação: Aumento de grau (muito mais). | Testar troca pormuito mais (intensidade) versus atualmente/neste momento (tempo). |
| Ironia vs. Crítica Direta | Ironia: Dizer o oposto do que se pensa. | Crítica Direta: Expressar julgamento negativo literal. | Na ironia há inversão semântica obrigatória; na crítica direta o autor diz textualmente o que pensa. |
...
- Justificativa: Trata-se de discurso direto / transcrição literal da fala do entrevistado, equivalendo à marcação da fala de uma personagem real.

Avaliar a justificativa do emprego dos travessões no período: “Nessa tona das águas da vida, por cima da qual nossa cabeça espia — se não naufragamos de vez —, somos assediados por pensamentos nem sempre muito inteligentes ou positivos sobre nós mesmos.”
...
Identificar a função dos travessões empregados no trecho: “Naquele ano, por indicação médica — pois adoecera por excesso de trabalho —, Marx deixou Berlim...”
...
- E) Exemplificativa. (Incorreta: não cita casos empíricos).

Analisar a função do travessão no trecho: “Eles podem estar envolvidos em uso passivo da rede social — que ocorre quando você passa muito tempo no Facebook, Twitter e Instagram vendo comentários, fotos e postagens de outras pessoas, e não publicando nada próprio nem se envolvendo em conversas. (...)”
...
Indicar o valor do segmento sublinhado entre travessões: “Em certas sociedades primitivas o tempo gasto nas preliminares do casamento — corte, namoro, noivado etc. — era abreviado.”
...
- E) Uma enumeração de todas as preliminares citadas. (Distrator forte: enumerar exige trazer todas as partes; o "etc." impede a alegação de que foram citadas "todas").
...
- C) ...confissão que se (juntava na linha adiante) com o preço... $\rightarrow$ Incorreta (deixa a oração subordinada sem verbo).
- D) ...cadernos que pertencera (à minha avó Belmira). $\rightarrow$ Incorreta (isola o objeto indireto obrigatório).
- E) ...via minha mãe (recorrer a esse caderno) para conferir... $\rightarrow$ Incorreta (isola a oração substantiva que complementa via).
...
- C) inserir um sinônimo para explicar o sentido de um termo. (Pegadinha: barbeiro não é sinônimo de doença de Chagas; é o vetor da doença).
...
- C) uma intensificação de um fenômeno. (Incorreta: o 'ainda' aqui expressa tempo presente, não aumento de grau).
- D) uma ironia sobre o fato citado. (Incorreta: o autor expressa fato literal sem inversão de sentido).
- E) uma confirmação de algo já dito. (Incorreta: confirmar/ratificar é dizer o mesmo; somar o presente ao passado é ampliar).
...
1. Alternativas que apresentem "marcar a fala" e "mudança de interlocutor" para o mesmo trecho anulam-se mutuamente.
...
Ao concluir o estudo desta unidade (Travessão e Parênteses), o estudante deve ter consolidado as seguintes competências práticas e teóricas:

1. Dominar o Regime de Aspas e Citações: Reconhecer que citação direta é transcrição literal de discurso direto (com aspas ou travessão) e citação indireta é paráfrase. Aplicar com precisão a regra do ponto final: dentro das aspas quando a citação constituir a oração inteira e fora quando for apenas fração da frase.
2. Operar a Reescritura Sintática sem Confundir Sentido e Ênfase: Saber que comutar travessões duplos, vírgulas duplas e parênteses mantém rigorosamente a correção gramatical e o sentido proposicional. Reconhecer a gradação expressiva: travessão enfatiza, parênteses atenuam e vírgulas são neutras.
...
1. Mapear Relações Coesivas pelo Método das Setas: Traçar setas para diferenciar hipônimos/exemplos de sinônimos literais.
2. Dominar o Vocabulário Semântico de Bancas: Discernir retificação (correção) de ratificação (confirmação); distinguir o ainda temporal do ainda intensificador; e aplicar a definição rigorosa de ironia (dizer o oposto do que se pensa).
