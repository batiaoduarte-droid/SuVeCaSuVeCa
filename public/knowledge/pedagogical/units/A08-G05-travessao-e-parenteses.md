...
│
└── 5. Semântica Crítica para Bancas Examinadoras
 ├── Ampliação (= Dizer mais / Somar dados)
 ├── Retificação (= Corrigir erro prévio) ≠ Ratificação (= Confirmar / Reiterar)
 ├── Operador "Ainda": Intensificação (Grau: "ainda mais") vs. Tempo (Persistência: "ainda atrai")
 └── Ironia Estrita (= Dizer o oposto do que se pensa; NÃO é simples crítica ou desabafo)
```
...
### Tabela de regras e formulações canônicas
...
### 8.1. Matriz de Expressividade e Comutabilidade na Reescritura
...
| **Ainda (Tempo vs. Intensidade)** | **Tempo:** Continuidade no presente (*agora, até hoje*). | **Intensificação:** Aumento de grau (*muito mais*). | Testar troca por*muito mais* (intensidade) versus *atualmente/neste momento* (tempo). |
