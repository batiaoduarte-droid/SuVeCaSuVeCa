...
(este/isto, esse/isso, aquele/aquilo) (que, quem, cujo, onde, o qual, etc.)
 │ │
 ┌───────┼───────────────────┐ ┌───────┼───────────────────┐
 ▼ ▼ ▼ ▼ ▼ ▼
ESPAÇO TEMPO TEXTO RECONHECIMENTO ANTECEDENTE DISTINÇÕES DE
(Dêixis)(Dêixis) (Endofórica) (2 Passos) (Natureza Subst.) "QUE"
 │ │ │ │ │ │
 ├1ª P. ├Presente/ ├Anáfora (esse/isso) ├Retoma ├Substantivo ├C. Integrante
 │(Este) │Fut. Próx. (Este) ├Catáfora (este/isto) │Subst. ├Pron. Subst. │("ISSO")
 │ ├Passado ├2 Antecedentes: └Troca │(o/os = aquilo) ├Realce ("é que")
 ├2ª P. │Recente (Esse) │ Este (próximo) / Aquele (distante) por ├Numeral Subst. ├Prep. Acidental
 │(Esse) ├Pass./Fut. └3+ Antecedentes: "o qual" │("milhões") └Consecutiva
 │ │Distante (Aquele) Este (último) / Aquele (1º) / Ordinais (meio) └Cujo: Posposto (Concord.) ("tanto... que")
 └3ª P. vs Antecedente (Ref.)
 (Aquele)
```
...
- O termo *o/os* não é artigo, mas sim um **pronome demonstrativo** equivalente a *aquilo / aqueles*.
- Como esse pronome não acompanha nenhum substantivo, ele atua como **pronome substantivo**.
- O pronome relativo *que* tem como antecedente direto exatamente esse pronome demonstrativo *o/os*.
...
```text
[ Termo ANTECEDENTE (Possuidor) ] <--- (Retoma) --- [ CUJO / CUJA ] --- (Concorda) ---> [ Termo POSPOSTO (Possuído) ]
...
### Tabela de Consulta Canônica Rápida

| Fenômeno Gramatical | Regra Canônica / Estrutura |
| :-------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Pergunta Tripla** | Antes de escolher o demonstrativo, identificar se o plano é**Espaço**, **Tempo** ou **Texto**. |
...
| **Reconhecimento do Relativo** | Retoma antecedente de natureza substantiva + troca-se por*o qual, a qual, os quais, as quais*. |
...
| **Paralelismo Sintático ("e que...")** | Relativos coordenados pela conjunção*e* compartilham obrigatoriamente o mesmo antecedente. |
| **Pronome "Cujo/Cuja"** | Concorda com o substantivo**posposto** (possuído), mas retoma o **antecedente** (possuidor). |
...
### 8.1. Matriz dos Demonstrativos: Espaço vs Tempo vs Texto
...
|:-------------------------------- |:---------------------------------------------------------- |:-------------------------------------------- |:---------------------------------------------------------------------- |
| **Pronome Relativo** | Retoma antecedente substantivo; introduz oração adjetiva. | Substituir por**o qual / a qual**. | *"A própria terra **que** piso..."* |
| **Conjunção Integrante** | Conecta oração substantiva sem valor anafórico. | Substituir a oração por**ISSO**. | *"Eu vi **que** a senhora tem uma sapucaia."* |
...
3. *`Guilherme, traga-me ____ CD que ficou lá no seu quarto.`* $\rightarrow$ **aquele** (Espaço: distante de ambos / advérbio *lá*).
4. *`Um dia ____ visitarei o seu pai.`* $\rightarrow$ **destes** (Tempo: futuro do presente *visitarei* $\rightarrow$ futuro próximo).
5. *`Guilherme, dê-me ____ CD que está aí.`* $\rightarrow$ **esse** (Espaço: perto do ouvinte / advérbio *aí*).
6. *`Luísa, ____ é o Eduardo?`* $\rightarrow$ **esse / aquele** (Espaço: descontextualizado; se perto de Luísa = *esse*, se longe de ambos = *aquele*).
7. *`____ ano que passou foi muito proveitoso para mim.`* $\rightarrow$ **Esse** (Tempo: pretérito recente *passou*).
8. *`____ mês que passou choveu muito. Para ____ mês que começa, porém, a previsão é de tempo bom.`* $\rightarrow$ **Nesse / este** (Tempo: passado recente vs futuro próximo).
9. *`____ aqui são meus primos.`* $\rightarrow$ **Estes** (Espaço: proximidade do falante / advérbio *aqui*).
10. *`O que foi combinado aqui não pode sair d____ sala.`* $\rightarrow$ **desta** (Espaço: sala do falante / advérbio *aqui*).
11. *`"Qual é a relação entre dialética e retórica? A ____ pergunta Aristóteles responde..."`* $\rightarrow$ **essa** (Texto: anáfora / resgate de pergunta anterior).
12. *`____ é a pergunta que Aristóteles responde desde a primeira frase: qual é a relação...?`* $\rightarrow$ **Esta** (Texto: catáfora / anúncio prévio).
13. *`Ao comparar os diversos rios do mundo com o Amazonas, defendia a proeminência d____ sobre cada um d____.`* $\rightarrow$ **deste / daqueles** (Texto: 2 antecedentes $\rightarrow$ *deste* retoma *Amazonas* [mais próximo]; *daqueles* retoma *rios* [mais distante]).
14. *`Atentem para ____: o que se guarda no coração jamais se esquece.`* $\rightarrow$ **isto** (Texto: catáfora antecipatória).
15. *`Homens e mulheres participaram das festividades. ____, simpáticas e elegantes. ____, sérios e tímidos.`* $\rightarrow$ **Estas / Aqueles** (Texto: 2 antecedentes $\rightarrow$ *Estas* = *mulheres* [próximo]; *Aqueles* = *Homens* [distante]).
