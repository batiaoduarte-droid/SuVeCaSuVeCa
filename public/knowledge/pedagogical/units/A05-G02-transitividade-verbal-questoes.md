# Transitividade Verbal - Questões
...
**Método central:** Nas questões de transitividade, reconstrua o mapa completo e teste regência, indispensabilidade e sentido, sem atribuir ao verbo uma etiqueta fixa.
...
1. COMPLEMENTOS VERBAIS 2. TERMOS DE ESTADO / LIGAÇÃO 3. COMPORTAMENTOS ESPECÍFICOS
 ...
 │ ├─ Sem preposição obrigatória │ ├─ Sempre Verbo de Ligação (V.L.) │ ├─ HAVER (=existir): VTD / Sem Sujeito / Rege O.D.
 │ ├─ Pronomes oblíquos: o, a, os, as │ └─ Exige Predicativo do Sujeito │ └─ EXISTIR: V.I. / Com Sujeito obrigatório
 │ └─ Artigos (um/uma/a) NÃO são prep. │ │
 │ ├─► ESTAR / FICAR ├─► LOCUÇÃO VERBAL (Mantra 7)
 ├─► Objeto Indireto (O.I.) │ ├─ Com P.Suj. => Verbo de Ligação │ ├─ Auxiliar: flexão de tempo/modo/número
 │ ├─ Com preposição regida pelo verbo │ └─ Sem P.Suj. (Lugar) => Verbo Intrans. │ └─ Principal: define a transitividade (VTD/VTI)
 │ ├─ Destinatário / Paciente puro │ │
 │ └─ Pronome "lhe" (= a ele/ela) ├─► Predicativo do Objeto (P.O.) └─► VERBO CHAMAR (=denominar)
 │ │ ├─ Qualifica o Objeto Direto ├─ Objeto Direto (ser denominado)
 └─► ADJUNTO ADVERBIAL (Mantra 10) │ └─ Pode vir com prep. "de" (não é OI!) └─ Predicativo do Objeto (nome/título atribuído)
 ├─ "Cheiro de Circunstância" │
 └─ Tempo, lugar, modo, causa, etc. └─► Voz Passiva Pronominal
 └─ VTD + SE => Paciente vira Sujeito
```
...
No esquemas da Questão 01, A abordagem didática anota sobre a opção E: `V.T.D.I.` sobre *Empresta*, `O.I.` sobre *me*, `O.D.` sobre *trezentos mil réis* e `(Tu)` como sujeito oculto.
...
> *“Preposições, muitas vezes, trazem cheiro de circunstância. Você não precisa saber do que é, basta sentir o cheiro.”*

Casos consolidados analisados no bloco:

1. *"...fazer sem um ponto de interrogação"*: *sem* introduz **condição** $\rightarrow$ **Adjunto Adverbial de Condição** (*fazer* = V.T.D. na voz passiva).
2. *"...olhar-lhe para a cara"*: *para* indica **direção/localização** $\rightarrow$ **Adjunto Adverbial de Lugar** (*olhar* = V.I.).
3. *"...serve para pescar"*: *para* indica **finalidade** $\rightarrow$ **Adjunto Adverbial de Finalidade** (*servir* = V.I.).
4. *"...lançais o anzol de repente"*: *de repente* indica **modo** $\rightarrow$ **Adjunto Adverbial de Modo** (*lançais* = V.T.D.).
5. *"...retornava a cada Natal"*: *a* indica **tempo** $\rightarrow$ **Adjunto Adverbial de Tempo** (*retornava* = V.I.).
6. *"...comprar a preço justo"*: *a* indica **preço** $\rightarrow$ **Adjunto Adverbial de Preço**.
7. *"...Ele nada faz sem motivo"*: *sem* indica **modo/ausência de causa** $\rightarrow$ **Adjunto Adverbial de Modo** (*faz* = V.T.D., *nada* = O.D.).

Em contrapartida, no **Objeto Indireto puro**, a preposição não traz valor adverbial circunstancial, conectando o verbo a um paciente/destinatário:

- *"...o recado chegara a ele"*: *a ele* completa o verbo *chegar* (regência para pessoas na FCC) sem ideia de lugar físico $\rightarrow$ **Objeto Indireto**.

> "Preposições trazem muitas vezes cheiro de circunstância... Mesmo que você não saiba do que, você tem que sentir o cheiro... Sentiu o cheiro, vai ser adjunto adverbial." (`006`: )

Registro do quadro digital:
`MANTRA (10): Preposições, muitas vezes, trazem cheiro de circunstância (você não precisa saber do que é, basta sentir o cheiro).`
...
- Aplicação do **Mantra 11**:
 > *“Com o verbo SER, eu sou feliz! Sempre de ligação!”*
 >

Exemplos dissecados:

- *"A verdade é apenas uma."* $\rightarrow$ *A verdade* = Sujeito; *é* = V.L.; *apenas uma* = **Predicativo do Sujeito**.
- *"Quase todos os caracóis eram viúvos de suas lesmas."* $\rightarrow$ *eram* = V.L. (estado permanente); *viúvos* = **Predicativo do Sujeito**.
- *"Wagner era um homem casado."* $\rightarrow$ *era* = V.L.; *um homem casado* = **Predicativo do Sujeito**.
- *"é filho de Elis Regina..."* $\rightarrow$ *é* = V.L.; *filho de Elis Regina...* = **Predicativo do Sujeito**.

> "Com o verbo SER, eu sou feliz! Porque ele será sempre verbo de ligação!... O problema é perceber que é verbo SER... Não é 'é o quê', é verbo ser, verbo de ligação." (`006`:; `007`: )
...
|:---------------- |:---------------- |:-------------------------------------------- |:--------------- |:----------------------------- |
| **HAVER** | Existir / Ocorrer | **Inexistente** (Oração Sem Sujeito) | **V.T.D.** | **Objeto Direto (O.D.)** |
| **EXISTIR** | Existência | **Obrigatório** (Concorda com o verbo) | **V.I.** | **Sujeito** |

> "O haver quando é igual a existir ele é sem sujeito e VTD; o existir não, o existir tem sujeito, é com sujeito e intransitivo." (`007`: )
...
- *João ficou feliz.* $\rightarrow$ *feliz* = Predicativo do Sujeito (estado) $\rightarrow$ *ficou* = **V.L.**.
- *João ficou no curso.* $\rightarrow$ *no curso* = Adjunto Adverbial de Lugar $\rightarrow$ *ficou* = **V.I.**.

> "Para que o verbo seja de ligação, o predicativo do sujeito é obrigatório! Quando é que ele vai ser intransitivo? Se não houver predicativo do sujeito."
...
- **Gabarito:** **D** (*O verbo "cresceu" é intransitivo*)
...
- **Julgamento:** **ERRADO**.
- **Justificativa:** O complemento de *temos* é a oração reduzida inteira (*o 'jeitinho' virando corrupção*). Dentro dela, *'jeitinho'* é o sujeito e *'corrupção'* é o predicativo do sujeito de *virando* (V.L.).
...
- **Julgamento:** **ERRADO**.
- **Justificativa:** *sem motivo* é introduzido por preposição com valor circunstancial de modo/ausência, operando como **Adjunto Adverbial de Modo**. O complemento direto de *faz* é a palavra *nada*.
...
- **Julgamento:** **ERRADO**.
- **Justificativa:** O verbo *existir* é intransitivo e possui **Sujeito**. O segmento nominal concorda com a locução verbal e exerce a função de **Sujeito**, e não de complemento.
...
- **Julgamento:** **ERRADO**.
- **Justificativa:** O objeto direto não é elíptico (é o pronome relativo *que*); o sujeito (*eu*) é quem está elíptico. O termo *de a ordem das propriedades* não é objeto indireto, mas sim **Predicativo do Objeto**.
...
O bloco consolidado  estabelece uma metodologia pragmática e rigorosa para o domínio da predicação e transitividade verbal em concursos públicos:

1. **Bancas Variadas (AOCP, FUNDEP, VUNESP):** Fixação do **Mantra 10** (*"Preposições trazem cheiro de circunstância"* $\rightarrow$ Adjuntos Adverbiais) e do **Mantra 11** (*"Com o verbo SER, eu sou feliz!"* $\rightarrow$ Verbo de Ligação e Predicativo do Sujeito). Distinção rigorosa entre artigos e preposições e análise de sujeito estendido.
2. **Fundação Carlos Chagas (FCC):** Aplicação de transitividade em textos complexos. Confirmação de que o pronome *lhe* atua como Objeto Indireto e que termos preposicionados sem valor circunstancial voltados a pessoas (*"chegara a ele"*) são tratados como O.I. Consolidação da regra do verbo *Haver* (V.T.D., oração sem sujeito, rege O.D.) e do comando da transitividade pelo verbo principal em locuções verbais (Mantra 7).
3. **CEBRASPE / CESPE:** Aplicação do "Método das Setas" para rastreamento de relações sintáticas diretas. Confirmação de que verbos como *estar* e *ficar* tornam-se Intransitivos quando acompanhados de lugar; diferenciação de sujeitos de verbos intransitivos (*existir*) rotulados como complementos; e desmistificação da sintaxe do verbo *chamar* (V.T.D. + O.D. + Predicativo do Objeto).
