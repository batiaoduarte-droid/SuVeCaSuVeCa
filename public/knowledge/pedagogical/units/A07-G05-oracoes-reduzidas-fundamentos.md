# Orações Reduzidas - Fundamentos

---

> **Objetivo de aprendizagem**
>
> Compreender e definir os princípios normativos e conceituais de Orações Reduzidas - Fundamentos. Aplicar regras canônicas e procedimentos decisórios de Orações Reduzidas - Fundamentos na resolução de questões. Analisar o papel de Orações Reduzidas - Fundamentos no eixo Método central e reconhecer armadilhas e variações de bancas.
...
**Método central:** Orações reduzidas são delimitadas pelas formas nominais e encaixadas no mapa conforme a função que exercem no período.
...
- Identificação de conjunções integrantes (que, se), conjunções subordinativas adverbiais e pronomes relativos (que, quem, cujo, onde, o qual).
...
text SINTAXE DO PERÍODO COMPOSTO: ORAÇÕES REDUZIDAS ├── 1. ESTATUTO COMPARATIVO │ ├── Oração Desenvolvida: Conectivo explícito + Verbo conjugado finito (Pres/Pret/Fut) │ └── Oração Reduzida: Sem conectivo oracional / Com preposição + Forma nominal do verbo │ ├── 2. MECANISMOS DE INTRODUÇÃO E CONECTIVOS │ ├── Introdução Assindética: Iniciação direta pelo verbo na forma nominal │ ├── Introdução Preposicional: Preposição pura (a, de, em, para, por, ao, sem) │ └── Oposição Estrutural de Locuções: │ ├── Locução Conjuntiva: Termina com conjunção ("que") -> Oração Desenvolvida │ └── Locução Prepositiva: Termina com preposição ("de", "para") -> Oração Reduzida │ ├── 3. PARADIGMA DAS FORMAS NOMINAIS │ ├── Infinitivo: Terminações em -ar, -er, -ir (pessoal ou impessoal) │ ├── Gerúndio: Terminações em -ando, -endo, -indo (-ndo) │ └── Particípio: Formas regulares (-ado, -ido) e irregulares (-to, -so, etc.) │ ├── 4. TAXONOMIA SINTÁTICA EM CONCURSOS (ESTRITAMENTE SUBORDINADAS) │ ├── Subordinadas Substantivas Reduzidas │ │ ├── Teste de Equivalência: Substituição integral pelo pronome neutro "ISSO" │ │ └── Funções Sintáticas: Subjetiva, O.D., O.I., C.N., Predicativa, Apositiva │ ├── Subordinadas Adjetivas Reduzidas │ │ ├── Critério Identificador: Modificação de Substantivo/Pronome Antecedente │ │ └── Subdivisão por Pontuação: Restritiva (sem vírgula) vs. Explicativa (pontuada) │ └── Subordinadas Adverbiais Reduzidas │ ├── Critério Identificador: Apreensão semântica de circunstância ("cheiro de circunstância") │ └── Tipologia Circunstancial: 9 tipos (6C + FTP) │ └── 5. FRONTEIRA E SEGMENTAÇÃO ORACIONAL ├── Oração Principal: Retém seus termos nucleares e complementos imediatos └── Oração Reduzida: Inicia-se estritamente na forma nominal ou na preposição introdutória
...
A oposição sistemática assenta-se em três eixos obrigatórios consolidados pelas fontes:

1. Mecanismo de Iniciação: Presença obrigatória de conjunção ou pronome relativo (desenvolvidas) versus ausência de conectivo ou introdução por preposição simples/locução prepositiva (reduzidas).
...
Para fins de resolução de provas de concurso público:

- Ao constatar a estrutura reduzida, a análise sintática bifurca-se unicamente nas três classes da subordinação:

1. Subordinada Substantiva: Desempenha função sintática substantiva em relação à oração principal.
2. Subordinada Adjetiva: Qualifica, restringe ou explica um substantivo antecedente da oração principal.
3. Subordinada Adverbial: Funciona como adjunto adverbial da oração principal, veiculando uma das circunstâncias lógicas do período composto.
...
O protocolo de identificação opera em dois passos:
...
- Análise sintática: "É preciso" é predicado nominal desprovido de sujeito na oração principal; o bloco "seguir em frente" exerce a função de sujeito.
- Nomenclatura oficial: Oração subordinada substantiva subjetiva reduzida de infinitivo.
...
A classificação obedece a dois critérios consecutivos:
...
As 9 circunstâncias adverbiais são memorizadas pela fórmula mnemônica 6C + FTP:

- 6 C:

1. Causal (motivo/fato gerador da ação principal);
2. Comparativa (relação de confronto ou analogia);
3. Concessiva (ideia de oposição, quebra de expectativa ou ressalva que não impede o fato);
4. Condicional (hipótese ou pré-requisito para a realização do fato);
5. Conformativa (acordo, adequação ou conformidade);
6. Consecutiva (efeito, resultado ou consequência).

- F: Final (objetivo, meta ou finalidade visada).
...
- Desdobramento semântico: Equivale a Quando a hora chegou / Assim que chegou a hora / Logo que a hora chegou.
- Valor circunstancial: Marco cronológico (tempo).
- Nomenclatura oficial: Oração subordinada adverbial temporal reduzida de particípio.
...
No exemplo Vi a menina chorando na praça:
...
3. Nunca transferir o núcleo substantivo que atua como complemento da oração principal para dentro do bloco da oração reduzida.
...
- Etapa de Base (Tipo 1): Domínio imediato dos testes diagnósticos (troca por "ISSO", vínculo a substantivo antecedente e apreensão de circunstâncias 6C + FTP), garantindo segurança na tipologia oracional e identificação da forma nominal empregada.
...
- Oração Desenvolvida: Oração estruturada com conectivo explícito (conjunção ou pronome relativo) e verbo flexionado em tempo e modo finitos .
- Oração Reduzida: Oração desprovida de conjunção e pronome relativo, com verbo em forma nominal (infinitivo, gerúndio ou particípio), podendo ser antecedida por preposição pura ou locução prepositiva .
...
3. Regra do Diagnóstico Adjetivo: Toda oração reduzida que modifica diretamente um substantivo/pronome antecedente sem conjunção é subordinada adjetiva (restritiva se não pontuada; explicativa se isolada por pontuação) .
...
1. Passo 2 — Delimitar a Fronteira Oracional:
...
1. Passo 3 — Executar a Triagem Diagnóstica Categorial:
...
1. Passo 4 — Redigir a Nomenclatura Padrão:
...
| Eixo de Comparação | Orações Desenvolvidas | Orações Reduzidas | Critério Decisivo de Reconhecimento |
| --- | --- | --- | --- |
| 1. Como se iniciam? | Com conectivo explícito: conjunções integrantes/adverbiais/coordenativas ou pronomes relativos. | Sem conectivo oracional (iniciação direta) ou introduzidas por preposição / locução prepositiva. | Inspecionar se há conjunção ou pronome relativo. Preposição pura não anula a redução. |
| 2. Como fica o verbo? | Verbo flexionado em tempos e modos finitos (Presente, Pretérito, Futuro do indicativo/subjuntivo/imperativo). | Verbo principal ou auxiliar sob forma nominal: Infinitivo (-r), Gerúndio (-ndo) ou Particípio (-do/-to). | Verificar a desinência verbal. Locuções verbais sem auxiliar conjugado mantêm a redução. |
| 3. Como se subdividem? | Coordenadas (5 tipos) e Subordinadas (Substantivas, Adjetivas, Adverbiais). | Estritamente Subordinadas na prática de concursos: Substantivas, Adjetivas e Adverbiais. | Em provas de concurso, descartar qualquer hipótese de oração coordenada reduzida. |

| Locução Empregada | Palavra Terminal | Natureza Sintática | Tipo de Oração Resultante | Estrutura Verbal Exigida | Exemplo Modelo |
| --- | --- | --- | --- | --- | --- |
| a fim de que / para que | Conjunção (que) | Locução Conjuntiva | Oração Desenvolvida | Verbo conjugado no subjuntivo | Estudo para que passe. |
| a fim de / para | Preposição (de / para) | Locução Prepositiva | Oração Reduzida | Verbo no infinitivo | Estudo para passar. |
| apesar de que | Conjunção (que) | Locução Conjuntiva | Oração Desenvolvida | Verbo conjugado no subjuntivo | Apesar de que estude, não passa. |
| apesar de | Preposição (de) | Locução Prepositiva | Oração Reduzida | Verbo no infinitivo | Apesar de estudar, não passa. |
...
| --- | --- | --- | --- | --- |
| Substantiva | Troca integral pelo pronome neutro"ISSO" | Exerce função sintática substantiva perante a oração principal | 6 funções: Subjetiva, O.D., O.I., C.N., Predicativa, Apositiva | É preciso seguir em frente. (= É preciso ISSO $\rightarrow$ Subjetiva) |
| Adjetiva | Seta referencial para oSubstantivo Antecedente | Qualifica, restringe ou explica núcleo nominal da oração principal | 2 tipos: Restritiva (sem vírgula) e Explicativa (pontuada) | Vi a menina chorando na praça. (chorando $\rightarrow$ menina $\rightarrow$ Restritiva) |
| Adverbial | Apreensão semântica do"Cheiro de Circunstância" | Exerce papel de adjunto adverbial do verbo da oração principal | 9 circunstâncias: Mnemônico6C + FTP | Chegada a hora, saiu rapidamente. (Chegada $\rightarrow$ Tempo) |
...
3. Determinação da Função Sintática: A oração principal "É preciso" compõe-se de verbo de ligação + predicativo, não possuindo sujeito interno. A oração subordinada atua como sujeito sintático.
4. Nomenclatura Consolidada: Oração subordinada substantiva subjetiva reduzida de infinitivo.
...
3. Identificação do Vínculo: O verbo "chorando" adjetiva diretamente o substantivo "menina" (menina que chorava).
4. Critério de Pontuação: A oração não está isolada por vírgula $\rightarrow$ Valor restritivo.
5. Nomenclatura Consolidada: Oração subordinada adjetiva restritiva reduzida de gerúndio.
...
3. Nomenclatura Consolidada: Oração subordinada adverbial temporal reduzida de particípio.
...
- Terminou em "que" $\rightarrow$ É Locução Conjuntiva $\rightarrow$ Oração desenvolvida (exige verbo conjugado).
- Terminou em preposição (de, a, para) $\rightarrow$ É Locução Prepositiva $\rightarrow$ Oração reduzida (exige verbo no infinitivo).
...
- 6 C: Causal, Comparativa, Concessiva, Condicional, Conformativa, Consecutiva.
- F: Final.
- T: Temporal.
- P: Proporcional.
...
- Regra de Correção: O adjunto adverbial de lugar "na praça" subordina-se unicamente ao verbo "chorando". A oração como um todo (chorando na praça) qualifica o substantivo "menina", possuindo natureza estritamente subordinada adjetiva restritiva.
...
- Regra de Correção: O termo "a menina" é o objeto direto da oração principal (Vi a menina). A oração subordinada inicia-se estritamente no gerúndio: [Vi a menina] / [chorando na praça].
...
Oração que se estrutura com conectivo subordinativo ou coordenativo explícito (conjunção integrante, conjunção adverbial, conjunção coordenativa ou pronome relativo) e apresenta seu verbo conjugado em tempo e modo finitos (presente, pretérito ou futuro). Relações: .

Oração que não é introduzida por conjunção subordinativa/coordenativa nem por pronome relativo, apresentando seu verbo sob uma das formas nominais (infinitivo, gerúndio ou particípio), podendo ser antecedida por preposição pura ou locução prepositiva. Relações: , .

Formas do paradigma verbal desprovidas de flexão modotemporal finita que assumem comportamento morfossintático análogo ao de nomes (substantivos, adjetivos ou advérbios): Infinitivo (-r), Gerúndio (-ndo) e Particípio (regular em -do ou irregular em -to/-so). Relações: , .

Expressão pluriverbal que desempenha função de conjunção, caracterizando-se por terminar obrigatoriamente em conjunção (que). Introduz orações desenvolvidas. Exemplos: a fim de que, para que, apesar de que. Relações: .

Expressão pluriverbal que desempenha função de preposição, caracterizando-se por terminar obrigatoriamente em preposição (de, a, para). Introduz orações reduzidas de infinitivo. Exemplos: a fim de, apesar de, para. Relações: .

Estrutura oracional que se liga no período composto sem conjunção subordinativa típica, iniciando-se por pronomes indefinidos, interrogativos ou demonstrativos (como ocorre doutrinariamente nas orações agente da passiva). Relações: .

Processo de reescrita sintática no qual uma oração reduzida é transformada em oração desenvolvida mediante a inserção da conjunção/pronome relativo cabível e a flexão do verbo para o tempo e modo finitos correspondentes. Relações: , .

---

## Síntese para recuperação ativa

Ao concluir o estudo desta unidade, o estudante deve dominar com precisão e segurança:

1. A Natureza da Redução Oracional: Compreender que a oração reduzida caracteriza-se pela eliminação de conjunções integrantes/adverbiais e pronomes relativos, apresentando o verbo sob a forma nominal de infinitivo, gerúndio ou particípio (regular em -ado/-ido ou irregular em -to/-so), sendo perfeitamente compatível com preposições introdutórias puras ou locuções prepositivas.
...
1. A Técnica de Segmentação Oracional: Identificar corretamente a barra de corte imediatamente antes do verbo na forma nominal ou da preposição, preservando os complementos pertencentes à oração principal.
2. A Tipologia de Questões de Prova: Distinguir a abordagem de reconhecimento e classificação sintática (Tipo 1) da abordagem de desenvolvimento e reescrita de orações (Tipo 2).
