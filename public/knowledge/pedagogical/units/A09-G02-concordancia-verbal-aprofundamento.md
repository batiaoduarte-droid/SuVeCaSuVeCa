# Concordância Verbal - Aprofundamento
...
**Método central:** O aprofundamento testa sujeito oracional, expressões quantitativas, relativos e outros controladores sem se deixar enganar pela proximidade linear.
...
MAPA CONCEITUAL DO CONHECIMENTO CONSOLIDADO (esta unidade)
- ➔ 1. ORAÇÃO SEM SUJEITO (VERBOS IMPESSOAIS) ➔ []
➔ Fenômenos Naturais (Literal = Impessoal vs. Figurado = Pessoal) []
- ➔ Verbo HAVER (= existir/ocorrer/acontecer) vs. EXISTIR/OCORRER []
➔ Verbos de Tempo Decorrido (HAVER, FAZER, IR) ➔ []
- ➔ Expressões Idiomáticas (CHEGA DE, BASTA DE, PASSA DE) ➔ []
➔ 2. SUJEITO ORACIONAL (ORAÇÃO SUBORDINADA SUBJETIVA) ➔ []
- ➔ Formas: Oração Desenvolvida (com C.I.) vs. Reduzida (Infinitivo)[]
➔ Estruturas Típicas: Unipessoais, VL + Predicativo, Voz Passiva []
- ➔ Caso Específico: Verbo PARECER e Falso Sujeito Anteposto ➔ []
➔ 3. SINTAXE DE LOCUÇÕES VERBAIS ➔ []
- ➔ Principal Impessoal (Haver/Fazer) → Auxiliar no SINGULAR
➔ Principal Pessoal (Existir/Precipitar-se) → Auxiliar FLEXIONA
- ➔ Locução Modal "Haver de + Infinitivo" (Haver Pessoal Auxiliar) []
➔ 4. ESTRUTURAS DE VOZ PASSIVA E INDETERMINAÇÃO DO SUJEITO ➔ []
- ➔ VTD + SE = Partícula Apassivadora (PA) → Concorda com Sujeito Paciente
➔ VTI + SE = Partícula de Indeterminação (PIS) → Verbo na 3ª pessoa do singular
- ➔ Estrutura "Trata-se de" (IIS invariável) ➔ []
➔ 5. MECANISMOS AVANÇADOS DE CONCORDÂNCIA E DESAMBIGUAÇÃO ➔ Pronome Relativo "Que" como Sujeito ("O que vem depois salva") [] ➔ Inversões Sintáticas, Núcleo Não Preposicionado e Pronome LHES [] ➔ Sujeito Composto Posposto (Concordância Total vs. Atrativa) ➔ [] ➔ Acentuação Diferencial (têm/vêm, convém/convêm) e Particípios ➔ []
...
A matéria organiza-se pedagogicamente em quatro blocos fundamentais:

1. Verbos indicadores de fenômenos naturais/meteorológicos;
2. Verbo haver com sentido de existir, ocorrer ou acontecer;
...
- Ação: Manter o verbo na 3ª pessoa do singular.
...
3. Verbo IR temporal: Quando indica transcurso temporal aproximado (Vai para dois anos que estudo, Vai para duas horas que cheguei), o verbo ir é impessoal e não admite plural ("vão para...").
...
- Sendo o principal pessoal (pesquisar, precipitar-se), o auxiliar haver recebe as desinências pessoais de concordância com o sujeito.
- Exemplos corretos: Aqueles que hão de pesquisar / haverão de pesquisar; Desavenças hão de precipitar-se.
...
- Desenvolvida: Convém [que estejas aqui na hora marcada]. $\rightarrow$ Isso convém.
- Reduzida: Convém [mudar as regras]. $\rightarrow$ Isso convém (o verbo não concorda com o substantivo plural interno regras).
- Reduzida: [Ter um cérebro relaxado] é importante. $\rightarrow$ Isso é importante.
- Reduzida: [Reduzir as distâncias] convém. $\rightarrow$ Isso convém.

As bancas examinadoras cobram sujeito oracional estruturado em três matrizes sintáticas principais na oração regente:

1. (A) Verbos Unipessoais: Verbos como convir, bastar, faltar, importar, parecer, cumprir, urgir, caber, constar.
...
- Matriz A: Não deve caber aos sociólogos [definir a estrutura]; Nem ao sonho cabe [fazer restrições].
- Matriz B: Está claro [que as reações não podem ser diferentes]; É necessário [valorizar a desconcentração].
- Matriz C: Sabe-se [que o prazo expirou]; Constata-se [que as crianças têm dificuldades].
...
1. O termo Os dois é o sujeito da locução verbal subordinada haviam acabado de comer (o auxiliar haviam está corretamente no plural).
2. O sujeito do verbo parecer é a oração inteira: [que os dois haviam acabado de comer a maçã proibida].
3. Teste do ISSO: [Que os dois haviam acabado...] parecia $\rightarrow$ Isso parecia.
4. É erro crasso escrever \*"Os dois pareciam que haviam acabado...", pois Os dois não é sujeito de pareciam.
...
- Concordância total: existiram (com ambos os núcleos).
- Concordância atrativa: como o núcleo mais próximo (boatos) é plural, o verbo também permanece no plural (existiram).
- Se o primeiro núcleo fosse singular (ex.: o boato e as mentiras), seriam válidas as formas existia (atrativa) ou existiam (total).
- Se substituído por haver, a pessoalidade é anulada: sempre houve boatos e mentiras.
...
3. Impossibilidade de "Lhes" como Sujeito: O pronome lhes é intrinsecamente preposicionado (equivale a a eles / para eles). Portanto, jamais pode exercer a função de sujeito gramatical.
...
A tabela abaixo sintetiza o inventário canônico de regras consolidadas desta unidade, servindo de consulta rápida e remetendo aos seus respectivos detalhamentos técnicos:

| Estrutura Sintática | Regra Canônica de Concordância | Justificativa Sintática |
| --- | --- | --- |
| Verbos de Fenômenos Naturais (Literal) | 3ª pessoa do singular (Invariável) | Oração sem sujeito |
| Verbos de Fenômenos Naturais (Figurado) | Flexiona com o sujeito explícito | Verbo pessoal transitivo/intransitivo |
| Haver (= existir, ocorrer, acontecer) | 3ª pessoa do singular (Invariável) | VTD impessoal (termo posposto é Objeto Direto) |
| Existir / Ocorrer / Acontecer | Flexiona com o sujeito | Verbos pessoais (termo associado é Sujeito) |
| Haver / Fazer / Ir (Tempo decorrido) | 3ª pessoa do singular (Invariável) | Verbos impessoais temporais |
| Chega de / Basta de / Passa de | 3ª pessoa do singular (Invariável) | Expressão impessoal (preposição veda sujeito) |
| Locução com Principal Impessoal | Verbo auxiliar na 3ª do singular | O auxiliar assume a impessoalidade do principal |
| Locução com Principal Pessoal | Verbo auxiliar flexiona com o sujeito | O principal transmite a exigência de sujeito |
| Haver de + Infinitivo (Modal) | Verbohaver flexiona no plural | Haver atua como verbo auxiliar pessoal |
| Sujeito Oracional (Desenvolvido/Reduzido) | 3ª pessoa do singular (Invariável) | Oração subjetiva equivale ao pronome neutro ISSO |
| Verbo Parecer + Oração Desenvolvida | Parecer na 3ª pessoa do singular | Oração inteira é o sujeito deparecer |
| VTD + SE + Expressão Temporal | Verbo flexiona no plural | Voz passiva sintética (tempo é Sujeito Paciente) |
| VTI + SE (ex: Trata-se de / Torce-se para) | 3ª pessoa do singular (Invariável) | Índice de Indeterminação do Sujeito (PIS) |
| Pronome Relativo "Que" Sujeito | Verbo concorda com o antecedente | O "que" substitui sintaticamente o referente |
| Pronome Oblíquo "Lhes" | Não altera a concordância do verbo | Pronome preposicionado (nunca é sujeito) |
...
3. Interrogação Específica: Formule a pergunta de localização: "O que é que [verbo 1]?" ou "Quem é que [verbo 1]?".
4. Extração do Núcleo Não Preposicionado: Localize a resposta. Descarte imediatamente todos os adjuntos introduzidos por preposição (de, com, em, entre, para, a).
5. Ajuste de Flexão: Force a concordância estrita entre o núcleo não preposicionado e o Verbo 1.
6. Iteração Subsequente: Avance para o Verbo 2 e repita o processo. Jamais permita que o sujeito da Oração 2 interfira na concordância do Verbo 1.
...
1. Como o pronome "ISSO" é singular neutro, fixe compulsoriamente o verbo principal na 3ª pessoa do singular.
...
1. Se o verbo for Transitivo Indireto (VTI) com preposição regida:
...
1. Conclua com segurança que o antecedente é critérios, exigindo a flexão no plural: se apresentam.
...
3. Ao trocar por EXISTIR: Identifique o substantivo associado: ele passa a ser o sujeito sintático. Flexione o verbo existir no singular ou no plural concordando com ele (Existiam dúvidas / Existem problemas).
...
| Parâmetro de Comparação | Verbo HAVER (= existir/ocorrer) | Verbo EXISTIR / OCORRER / ACONTECER |
| --- | --- | --- |
| Pessoalidade Sintática | Impessoal (sem sujeito) | Pessoal (possui sujeito expresso) |
| Regra de Concordância | Invariável na 3ª pessoa do singular | Flexiona regularmente com o sujeito |
| Função do Termo Associado | Objeto Direto (OD) | Sujeito Sintático da oração |
...
| --- | --- | --- |
| Posição na Estrutura | Verbo principal isolado ou último da locução | Verbo auxiliar (primeiro verbo da locução) |
| Valor Semântico | Existir, ocorrer, tempo decorrido | Dever, ter de, destinar-se, futuro |
...
| --- | --- | --- |
| Transitividade do Verbo | VTD ou VTDI | VTI, VI ou VL |
| Presença de Preposição | Termo não preposicionado | Termo obrigatoriamente preposicionado |
| Transposição Analítica | Possível (verbo ser + particípio) | Totalmente impossível |
...
| --- | --- | --- |
| Estrutura Interna | Núcleo nominal (sem verbo) | Possui forma verbal (desenvolvida ou reduzida) |
| Equivalência Prática | Eles / Elas / Aqueles | Pronome neutroISSO |
| Concordância do Verbo | Verbo flexiona no plural | Verbo permanece fixo na 3ª pessoa do singular |
| Exemplo de Contraste | Bastam amigos verdadeiros. | Basta que você tenha amigos verdadeiros. |

---

## Exemplos comentados

- Enunciado: 27)  bastante em alguns bairros. (Choveu/Choveram)
- Gabarito: Choveu
- Justificativa Pedagógica: O verbo exprime evento meteorológico real na natureza. Sendo impessoal, constitui oração sem sujeito e deve ser fixado na 3ª pessoa do singular.
- Contraste Didático: "Choveram reclamações no grupo" (Sentido figurado $\rightarrow$ reclamações é sujeito simples plural $\rightarrow$ verbo flexionado no plural).

- Enunciado: 28)  dúvidas sobre aquele ponto da matéria. (Havia/Haviam)
- Gabarito: Havia
- Justificativa Pedagógica: O verbo haver é empregado no sentido de existir. Sendo VTD impessoal, não possui sujeito; dúvidas é objeto direto. O verbo permanece na 3ª pessoa do singular.
- Contraste Didático: "Existiam dúvidas sobre aquele ponto" (O verbo existir é pessoal e concorda com o sujeito plural dúvidas).

- Enunciado: 29)  haver novas rebeliões. (Poderá/Poderão)
- Gabarito: Poderá
- Justificativa Pedagógica: Na locução poderá haver, o verbo principal é haver (= existir). Sendo o principal impessoal, o verbo auxiliar poder absorve a impessoalidade e fica obrigatoriamente no singular.
- Contraste Didático: "Poderão existir novas rebeliões" (Com o verbo principal pessoal existir, o auxiliar poderão flexiona-se no plural para concordar com o sujeito novas rebeliões).

- Enunciado: 30) Falei da angústia que  dias me devorava. (havia/haviam)
- Gabarito: havia
- Justificativa Pedagógica: Trata-se do verbo haver indicando tempo decorrido. Por ser impessoal, não se flexiona no plural, ficando restrito à 3ª pessoa do singular (havia dias).

- Enunciado: 31)  dez meses que não recebo notícias dele. (Faz/Fazem)
- Gabarito: Faz
- Justificativa Pedagógica: O verbo fazer na indicação de tempo transcorrido é impessoal e não admite plural.
- Exemplo Suplementar da Aula: "Vai para dois anos que o espero" (O verbo ir temporal também atua como impessoal e invariável no singular).

- Enunciados:
- 32)  reclamações. (Chega de/Chegam de)
- 33)  duas horas. (Passa de/ Passam de)
...
- Gabarito Oficial: C
- Justificativa Pedagógica: O verbo haver indica tempo decorrido. Ao ser substituído por fazer com idêntico valor semântico temporal, este herda a impessoalidade e deve fixar-se na 3ª pessoa do singular (faz).
...
- Gabarito Oficial: E
- Justificativa Pedagógica: A opção E apresenta inadequação porque na locução verbal com haver existencial (haver como principal impessoal), o verbo auxiliar deve ficar no singular: Deve haver, e nunca Devem haver.

- Item 34:  que estejas aqui na hora marcada. (Convém/Convêm) $\rightarrow$ Gabarito: Convém (Oração desenvolvida; acento agudo no singular).
- Item 35:  mudar as regras. (Convém/Convêm) $\rightarrow$ Gabarito: Convém (Oração reduzida de infinitivo; acento agudo no singular).
- Item 36:  que as reações não podem ser diferentes. (Está claro/Estão claros) $\rightarrow$ Gabarito: Está claro (VL + Predicativo neutro singular).
- Item 37: Os dois  que haviam acabado de comer a maçã proibida. (parecia/pareciam) $\rightarrow$ Gabarito: parecia (O sujeito de parecia é toda a oração subordinada; Os dois é sujeito exclusivo da locução haviam acabado de comer).
...
> "Quem manda é o principal, quem obedece é o auxiliar." Em qualquer questão envolvendo locução verbal, não olhe primeiro para o verbo auxiliar flexionado. Olhe diretamente para o último verbo da cadeia (o verbo principal). Se o principal for impessoal (haver existencial ou fazer temporal), trave o auxiliar no singular. Se o principal for pessoal (existir, ocorrer), faça o auxiliar concordar com o sujeito da oração.
...
- Como evitar: Aplique o princípio "Viva Cada Verbo". Se houver uma forma verbal (mudar, valorizar), o sujeito é oracional e o verbo regente deve ficar no singular: Basta mudar as regras, É necessário valorizar as ações.
...
- Como evitar: Haver (= existir) e fazer (tempo) são estritamente impessoais. Os substantivos associados são objetos diretos ou adjuntos. A flexão correta é sempre no singular: Havia muitas pessoas, Faz dez meses, Houve problemas.
...
- Como evitar: O auxiliar absorve a impessoalidade do verbo principal. O correto é: Deve haver soluções, Pode haver dúvidas, Vai fazer dois anos. (Contraste: Devem existir soluções).
...
- Como evitar: Empregado figurativamente, o verbo é plenamente pessoal e concorda com o sujeito: Choveram críticas ao projeto.
...
- Como evitar: Faça a transposição analítica: Dois anos foram completados. O termo temporal é o sujeito paciente; logo, o verbo flexiona no plural: Já se completaram dois anos, Contam-se dois anos.
...
- Como evitar: O verbo tratar-se de é VTI com PIS e fica sempre no singular: Trata-se de questões complexas.
...
- Como evitar: Os alunos é o sujeito da oração subordinada sabiam. O sujeito de parecer é a oração inteira (Isso parecia). A forma correta é: Parecia que os alunos sabiam a lição.
...
Ao concluir o estudo integrado desta unidade (Concordância Verbal — Aprofundamento), o estudante deve ter desenvolvido competência analítica para:

1. Reconhecer e conjugar verbos impessoais: Identificar orações sem sujeito estruturadas por fenômenos da natureza em sentido estrito (Choveu bastante), pelo verbo haver existencial (Havia dúvidas) ou temporal (Havia meses), pelos verbos fazer e ir temporais (Faz dez anos, Vai para duas horas) e pelas expressões idiomáticas preposicionadas (Chega de, Basta de, Passa de), mantendo todas na 3ª pessoa do singular.
2. Dominar a hierarquia sintática das locuções verbais: Aplicar o princípio de que o verbo principal rege as relações sintáticas e transmite sua impessoalidade para o verbo auxiliar (Deve haver soluções, Vai fazer dez anos), contrastando-o com locuções de verbos pessoais (Devem existir soluções, Passaram a existir fortunas).
3. Diferenciar o verbo "Haver" existencial do "Haver de" modal: Compreender que em perífrases como haver de pesquisar, o verbo haver é auxiliar pessoal e deve flexionar no plural com o sujeito (Eles hão de pesquisar).
4. Dominar o Sujeito Oracional: Identificar orações subordinadas subjetivas desenvolvidas (Convém que venhas) e reduzidas (Convém mudar as regras), aplicando o Macete do "ISSO" (Isso convém) para travar o verbo principal no singular e neutralizar a atração de substantivos plurais internos.
5. Aplicar o isolamento oracional ("Viva Cada Verbo"): Analisar períodos compostos fragmentando-os oração por oração, interrogando cada verbo isoladamente e desarmando falsos sujeitos antepostos (como na pegadinha de Parecia que os dois haviam acabado...).
6. Distinguir Voz Passiva Sintética de PIS e Tempo Puro: Reconhecer que VTD + SE com expressão temporal gera sujeito paciente no plural (Já se completaram dois anos), ao passo que VTI + SE gera sujeito indeterminado no singular (Torce-se para as crianças, Trata-se de problemas).
7. Desambiguar antecedentes com o pronome relativo "Que": Aplicar a regra "o que vem depois salva" para identificar com segurança o núcleo que rege a concordância em orações adjetivas complexas.
8. Identificar sujeitos pospostos e excluir termos preposicionados: Reconhecer que o núcleo do sujeito nunca vem regido de preposição, desconsiderar pronomes oblíquos preposicionados (lhes) na função de sujeito e ordenar períodos invertidos na ordem direta.
