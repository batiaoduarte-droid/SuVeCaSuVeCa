...
│
└── 5. MECANISMOS AVANÇADOS DE CONCORDÂNCIA E DESAMBIGUAÇÃO
 ├── Pronome Relativo "Que" como Sujeito ("O que vem depois salva") []
 ├── Inversões Sintáticas, Núcleo Não Preposicionado e Pronome LHES ─ []
 ├── Sujeito Composto Posposto (Concordância Total vs. Atrativa) ──── []
 └── Acentuação Diferencial (têm/vêm, convém/convêm) e Particípios ── []
```
...
EXISTIR / OCORRER ────────► PESSOAL (Flexiona) ──► Termo associado = SUJEITO
```
...
LOCUÇÃO VERBAL ──► Analisar o ÚLTIMO verbo (Principal)
 ├── Principal IMPESSOAL (Haver/Fazer) ──► Auxiliar FIXO no SINGULAR ("Deve haver problemas")
 └── Principal PESSOAL (Existir/Passar) ──► Auxiliar FLEXIONA com Sujeito ("Devem existir problemas")
```
...
- *Haver* é o **primeiro verbo** (auxiliar), enquanto o verbo pleno no infinitivo é o **principal**.
...
- O sujeito composto é *boatos e mentiras*.
...
| **Haver de + Infinitivo (Modal)** | Verbo*haver* flexiona no plural | *Haver* atua como verbo auxiliar pessoal |
...
| **Verbo Parecer + Oração Desenvolvida** | *Parecer* na 3ª pessoa do singular | Oração inteira é o sujeito de*parecer* |
...
### 1. HAVER Existencial vs. EXISTIR Pessoal
...
| **Equivalência Prática** | *Eles / Elas / Aqueles* | Pronome neutro**ISSO** |
...
- **Item 36:** `________ que as reações não podem ser diferentes. (Está claro/Estão claros)` $\rightarrow$ **Gabarito: Está claro** (VL + Predicativo neutro singular).
- **Item 37:** `Os dois ________ que haviam acabado de comer a maçã proibida. (parecia/pareciam)` $\rightarrow$ **Gabarito: parecia** (O sujeito de *parecia* é toda a oração subordinada; *Os dois* é sujeito exclusivo da locução *haviam acabado de comer*).
