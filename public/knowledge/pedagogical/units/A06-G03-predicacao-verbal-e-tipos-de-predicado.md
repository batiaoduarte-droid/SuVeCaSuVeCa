...
SUJEITO PREDICADO COMPLEMENTOS COMPLEMENTO ADJUNTO ADJUNTO
 ...
 │ (Atributo / Seta no Sujeito) │
 │ │ │
 │ ▼ │
 │ PREDICATIVO DO OBJETO ──────────────────────────────┼───────────────────┘
 │ (Atributo / Seta no Objeto) │
 │ │
 └──────────────────────────────────────────────────────────────────┘
```
...
TERMOS ESSENCIAIS TERMOS INTEGRANTES TERMOS ACESSÓRIOS
 ...
 ▼ ▼
 SUJEITO PREDICADO
 (Pode existir ou não existir) (Totalmente Obrigatório)
 │ │
 ┌────────────┴────────────┐ │
 ▼ ▼ ▼
 EXISTENTE INEXISTENTE "TUDO MENOS O SUJEITO"
 ├── Simples (Oração sem Sujeito / (Estruturado em torno
 ├── Composto Verbos Impessoais) do Verbo Central)
 ├── Oracional
 ├── Indeterminado
 └── Oculto / Desinencial
 │
 ┌─────────────────────┴─────────────────────┐
 ▼ ▼
 VERBOS DE AÇÃO / MOVIMENTO VERBOS DE ESTADO
 (Verbos Nocionais) (Verbos Relacionais)
 │ │
 ┌──────────┬─────┴────┬──────────┐ ▼
 ▼ ▼ ▼ ▼ VERBO DE LIGAÇÃO
 INTRANS. TRANS. TRANS. TRANS. (V.L.)
 (V.I.) DIRETO INDIRETO DIR. E IND. │
 │ (V.T.D.) (V.T.I.) (V.T.D.I.) ▼
 │ │ │ │ CRITÉRIO BIVALENTE:
 │ ▼ ▼ ▼ 1. Expressa ESTADO
 ...
 │
 ▼
TERMOS ACESSÓRIOS E QUALIFICADORES:
 ├── Adjunto Adverbial (Circunstância / Seta no Verbo / Dispensável)
 └── Predicativo do Sujeito / do Objeto (Estado / Seta no Nome / Invariável vs. Variável)
 ...
 (P.N.) (P.V.) (P.V.N.)
 │ │ │
 ├── Núcleo: Nome (Pred. Suj.) ├── Núcleo: Verbo de Ação ├── 2 Núcleos:
 └── Estrutura: VL + Pred. Suj. └── Estrutura: VI/VTD/VTI/VTDI Verbo de Ação + Predicativo
 sem Predicativo └── Predicativo do Sujeito OU
 Predicativo do Objeto
```
...
```text
 ...
 ┌────────────────────────┼────────────────────────┐
 ▼ ▼ ▼
TRANSITIVO DIRETO TRANSITIVO INDIRETO TRANSITIVO DIRETO E
 (V.T.D.) (V.T.I.) INDIRETO (V.T.D.I.)
 │ │ (ou Bitransitivo)
 ▼ ▼ │
Complemento SEM Complemento COM ▼
 preposição preposição Exige 2 complementos:
(Objeto Direto) (Objeto Indireto) 1 SEM prep. (O.D.)
 + 1 COM prep. (O.I.)
```
...
- Acoplado a **Verbo de Ligação** $\implies$ Predicado Nominal;
- Acoplado a **Verbo de Ação (VI, VTD, VTI, VTDI)** $\implies$ Predicado Verbo-Nominal.
...
```text
 ...
 │
 ▼
 Etapa 1: "VIVA O VERBO" (Isolar o Sujeito)
 Pergunta: "Quem é que / O que é que + VERBO?"
 │
 ▼
 Etapa 2: Delimitar o Predicado
 Equação: Predicado = Oração - Sujeito
 │
 ▼
 Etapa 3: "A GRANDE SACADA" (Natureza Verbal)
 O verbo expressa ESTADO ou AÇÃO/PROCESSO?
 │
 ┌─────────────────────┴─────────────────────┐
 ▼ ▼
 ...
 │ │
 ▼ ▼
 Tem Predicativo do Sujeito? Inspecionar termos pós-verbais
 ┌───────┴───────┐ ("Mantra do Contexto"):
 ▼ ▼ - Sem objeto (só adjunto adv.) ➔ VI
 [ SIM ] [ NÃO ] - Objeto sem preposição ➔ VTD
 │ │ - Objeto com preposição ➔ VTI
 ▼ ▼ - Ambos os objetos ➔ VTDI
 VERBO DE VERBO DE │
 LIGAÇÃO AÇÃO ▼
 (VL) (VI) Há termo adjetivo predicativo?
 │ │ │
 ▼ ▼ ┌─────────┴─────────┐
 PREDICADO PREDICADO ▼ ▼
 NOMINAL VERBAL [ NÃO ] [ SIM ]
 (PN) (PV) │ │
 ▼ ▼
 PREDICADO PREDICADO
...
| **Obrigatoriedade Sintática** | Termo acessório**dispensável** (pode ser suprimido). | Termo integrante**obrigatório** exigido pela regência. |
| **Efeito na Predicação** | O verbo permanece**Intransitivo (VI)**. | O verbo classifica-se como**Transitivo Indireto (VTI)**. |
