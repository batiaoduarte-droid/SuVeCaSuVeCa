...
const buildOfficialCorpusAnswerKey = async () => {
  const index = await readJson('public', 'knowledge', 'official-question-index.json');
  assert(Array.isArray(index.items) && index.items.length === 372, 'O índice oficial deve conter 372 questões.');
  const entries = index.items.map((item) => [item.questionId, item.officialProjection?.correctAnswer]);
  assert(entries.every(([id, answer]) => typeof id === 'string' && typeof answer === 'string'), 'Gabarito oficial incompleto.');
  assert(new Set(entries.map(([id]) => id)).size === entries.length, 'IDs oficiais duplicados no gabarito.');
  const answerKey = Object.fromEntries(entries);
  const source = `/* AUTO-GENERATED by scripts/build-deployment-shards.mjs. Do not edit. */\nexport const OFFICIAL_CORPUS_ANSWER_KEY = ${JSON.stringify(answerKey, null, 2)} as const;\n\nexport const OFFICIAL_CORPUS_VERSION = 'official-corpus-v1';\nexport const OFFICIAL_CORPUS_SAMPLE_SIZE = 10;\n`;
  await writeFile(resolve('functions', 'src', 'officialCorpus.generated.ts'), source);
  return entries.length;
};

const main = async () => {
  const [questions, profiles, knowledgeIndex, corpusAnswerCount] = await Promise.all([
    buildQuestionShards(),
    buildSemanticProfileShards(),
    buildKnowledgeIndexShards(),
    buildOfficialCorpusAnswerKey(),
  ]);
  console.log(JSON.stringify({
    ...
    officialCorpusAnswers: corpusAnswerCount,
  }, null, 2));
};
